...
"""
Tests that plugins.oppo's module-level bootstrap does not clobber an
...
locate('plugins.oppo.commands'), which needs the plugins.oppo package
importable under its real dotted name - re-running this module's top-level
code with __name__ == 'plugins.oppo', hitting the `else` branch. Simulated
...
class TestOppoStandaloneFlagNotClobbered(unittest.TestCase):
    def test_reload_does_not_reset_true_to_false(self):
        import plugins.oppo  # noqa: F401  (else branch runs, sets False - expected for normal import)
