...
class BackendLogging:
    ...
    @cherrypy.expose
    def logging_html(self, reload=None):
        """
        ...
        if reload is not None:
            ...
            for ln in loggerDict:
                ...
                li['name'] = l.name
                li['level'] = l.level
                li['handlers'] = l.handlers
                li['name'] = l.name
                loli.append(li)
            self.logger.warning("logging_html: loggerDict = {}".format(loggerDict))        
            self.logger.warning("logging_html: loli = {}".format(loli))        
            self.logger.warning("Reloading logging config")        
            self.plugin.get_sh().init_logging()
            self.logger.warning("--------------------   Logging config has been reloaded   --------------------")        
        ...
        return self.render_template('logging.html', updates=self.updates_allowed, loggers=loggers)
