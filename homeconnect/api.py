...
class HomeConnect:
    ...
    def connect(self):
        ...
        refresh_kwargs = {'client_id': self.client_id, 'client_secret': self.client_secret}
    ...
    def get_token(self, authorization_response):
        ...
        try:
            token = self._oauth.fetch_token(
                uri, authorization_response=authorization_response, client_secret=self.client_secret
            )
    ...
    def put(self, endpoint, data):
        ...
        res = self._oauth.put(
            uri,
            json.dumps(data),
            headers={'Content-Type': 'application/vnd.bsh.sdk.v1+json', 'accept': 'application/vnd.bsh.sdk.v1+json'},
        )
        if not res.content:
            return {}
        try:
...
class HomeConnectAppliance:
    ...
    def set_options_active_program(self, option_key, value, unit=None):
        ...
        if unit is None:
            ...
            _unit = {'unit': unit}
        return self.put(
            f'/programs/active/options/{option_key}', {'data': {'key': option_key, 'value': value, **_unit}}
        )
    ...
    def set_options_selected_program(self, option_key, value, unit=None):
        ...
        if unit is None:
            ...
            _unit = {'unit': unit}
        return self.put(
            f'/programs/selected/options/{option_key}', {'data': {'key': option_key, 'value': value, **_unit}}
        )

    def execute_command(self, command):
        """Execute a command."""
        return self.put(f'/commands/{command}', {'data': {'key': command, 'value': True}})
