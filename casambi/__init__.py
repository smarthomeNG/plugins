...
class Casambi(SmartPlugin):
    ...
    def getSessionCredentials(self):
        ...
        sessionrequest_response = self.session.post(
            urlCasambi + functionURL,
            data=dataBodyJson,
            headers={'X-Casambi-Key': self.api_key, 'content-type': 'application/json'},
            timeout=10,
            verify=False,
        )
    ...
    def controlDevice(self, item, id, key, error_count=0):
        ...
        if key == 'ON' or key == 'DIMMER':
            ...
            targetControls = {
                'ColorTemperature': {'value': sendValue},
                'Colorsource': {'source': 'TW'},
            }  # ColorTemperature value in Kelvins

        controlMsg = {'wire': self.wire, 'method': 'controlUnit', 'id': id, 'targetControls': targetControls}
        # convert to JSON string:
    ...
    def decodeEventData(self, receivedData):
        ...
        if method == '':
            ...
            self.logger.debug(
                f'Received {method} status from unit {unitID}, value: {dimValue}, vertical: {verticalValue}, cct: {cctValue}.'
            )
