...
class Philips_TV(SmartPlugin):
        ...
        if caller != self.get_shortname():
            if self.has_iattr(item.conf, 'philips_tv_tx_key'):
                ...
                if tx_key == 'MUTE':
                    ...
                    if not len(item()) >= 3:
                        self.logger.error(
                            f'HSV item {item.property.name} must be of type list with three entries (H,S,V). Aborting.'
                        )
                        return
                    ...
                    if (hue is None) or (saturation is None) or (brightness is None):
                        self.logger.error(
                            'Cannot find all neccessary list entries for hue command (hue, saturation and brightness). Aborting.'
                        )
                        return
    ...
    def pair_request(self, data, err_count=0):
        ...
        try:
            r = self.session.post(
                'https://' + str(self.ip) + ':1926/6/pair/request', json=data, verify=False, timeout=4
            )
    ...
    def pair_confirm(self, data, err_count=0):
        while err_count < 10:
            ...
            try:
                self.session.post(
                    'https://' + str(self.ip) + ':1926/6/pair/grant',
                    json=data,
                    verify=False,
                    auth=HTTPDigestAuth(self.deviceID, self.deviceKey),
                    timeout=2,
                )
    ...
    def completePairing(self, code):
        ...
        auth['auth_signature'] = self.create_signature(
            b64decode(self.secret_key), str(self.auth_Timestamp).encode() + str(code).encode()
        )
    ...
    def get(self, path, verbose=True, err_count=0, print_response=True):
        while err_count < int(self._num_retries):
            ...
            try:
                r = self.session.get(
                    'https://' + str(self.ip) + ':1926/6/' + str(path),
                    verify=False,
                    auth=HTTPDigestAuth(str(self.deviceID), str(self.deviceKey)),
                    timeout=2,
                )
            except Exception:
                err_count += 1
