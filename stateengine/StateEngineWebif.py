...
class WebInterface(StateEngineTools.SeItemChild):
    ...
    def __init__(self, abitem):
        ...
        self.__img_path = abitem.se_plugin.path_join(
            abitem.se_plugin.get_plugin_dir(), 'webif/static/img/visualisations'
        )
        ...
        self.__graph = pydotplus.Dot(
            'StateEngine',
            graph_type='digraph',
            splines='false',
            overlap='compress',
            compound='false',
            imagepath='{}'.format(self.__img_path),
        )
        self.__graph.set_node_defaults(
            color='lightgray', style='filled', shape='box', fontname='Helvetica', fontsize='10', pin='true'
        )
        self.__graph.set_edge_defaults(
            color='darkgray',
            style='filled',
            shape='box',
            labeldistance=2.5,
    ...
    def __repr__(self):
        return (
            'WebInterface item: {}, id {}'.format(self.__states, self.__name) if REQUIRED_PACKAGE_IMPORTED else 'None'
        )
    ...
    def _actionlabel(self, state, label_type, conditionset, active):
        ...
        action_tooltip = ''
        types = (
            [label_type] if label_type in ['actions_leave', 'actions_pass'] else ['actions_enter_or_stay', label_type]
        )
        tooltip_count = 0
        ...
        for label_type in types:
            for action in self.__states[state].get(label_type):
                ...
                fontcolor = (
                    ...
                    and ((cond1 and cond2 and _reason) or (_reason and label_type in ['actions_leave', 'actions_pass']))
                    else '#f4c430'
                    if _delay > 0 and active
                    else 'darkred'
                    if _delay < 0 and active
                    else '#303030'
                    if _issue
                ...
                if action1 in ['set', 'force set']:
                    ...
                    action3 = ''
                success_info = (
                    '<td width="26"><img src="sign_warn.png" /></td></tr>'
                ...
                if not action2 == 'None':
                    actionlabel += (
                        '<tr class="actionentry"><td align="center"><font color="{}">{} {} {} {}</font></td>'.format(
        ...
        actionlabel += '</table>>'
        actionend = '</table>>'
        actionlabel = '' if actionlabel == '{}{}'.format(actionstart, actionend) or actionlabel == '<>' else actionlabel
    ...
    def _conditionlabel(self, state, conditionset):
        ...
        conditionlist += '<tr class="conditionset"><td align="center" colspan="4" bgcolor="white">{}</td></tr>'.format(
            conditionset
        )
        tooltip_count = 0
                ...
                if cond1 and compare not in excluded_values:
                    if condition not in conditions_done:
                        ...
                        if current is not None and len(current) > 0:
                            ...
                            text = ' Not evaluated.'
                        conditionlist += (
                            '<tr class="conditionheader"><td align="center" colspan="2"><table border="0" cellpadding="0" '
                            ...
                            '<td width="40%"></td></tr></table></td></tr>'
                        ).format(condition.upper(), text)
                    ...
                    info_status_eval = info_status_eval[: self.__textlimit] + '.. &nbsp;' * int(
                        textlength > self.__textlimit
                    )
                    info_value = info_compare[: self.__textlimit] + '.. &nbsp;' * int(
                        len(info_compare) > self.__textlimit
                    )
                    ...
                    comparison = (
                        '&#62;='
                        if not min_none and compare == 'min'
                        else '&#60;='
                        if not max_none and compare == 'max'
                        else 'older'
                        if not agemin_none and compare == 'agemin'
                        else 'younger'
                        if not agemax_none and compare == 'agemax'
                        else 'not changed by'
                        if (
                            not changedby_none
                            and compare == 'changedby'
                            and condition_dict.get('changedbynegate') == 'True'
                        )
                        else 'changed by'
                        if not changedby_none and compare == 'changedby'
                        else 'not updated by'
                        if (
                            not updatedby_none
                            and compare == 'updatedby'
                            and condition_dict.get('updatedbynegate') == 'True'
                        )
                        else 'updated by'
                        if not updatedby_none and compare == 'updatedby'
                        else 'not triggered by'
                        if (
                            not triggeredby_none
                            and compare == 'triggeredby'
                            and condition_dict.get('triggeredbynegate') == 'True'
                        )
                        else 'triggered by'
                        if not triggeredby_none and compare == 'triggeredby'
                        else '!='
                        if (not value_none and compare == 'value' and condition_dict.get('negate') == 'True')
                        else '=='
                    )
                    ...
                    if match and len(match) > 0:
                        match_info = (
                            match.get('value')
                            if compare in ['min', 'max', 'value']
                            else match.get('age')
                            if compare in ['agemin', 'agemax', 'age']
                            else match.get(compare)
                        )
                    conditionlist += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{}&nbsp;&nbsp;&nbsp;'.format(comparison)
                    conditionlist += (
                        '"{}"'.format(info)
                        if not item_none and not status_none and not eval_none and not status_eval_none
                        else ''
                    )
                    ...
                    conditionlist += (
                        '{}'.format(info)
                        if not condition_dict.get(compare) == 'None' and (cond1 or cond2 or cond3 or cond4)
                        else ''
                    )
                    conditionlist += (
                        ' (negate)'
                        if condition_dict.get('negate') == 'True' and 'age' not in compare and not compare == 'value'
                        else ''
                    )
                    conditionlist += (
                        ' (negate)' if condition_dict.get('agenegate') == 'True' and 'age' in compare else ''
                    )
                    ignore_info = str(condition_dict.get('ignore') or 'n/a')
                    match_info = (
                        '<img src="sign_ignore.png" />'
                        if ignore_info == 'True'
                        else '<img src="sign_true.png" />'
                        if match_info == 'yes'
                        else '<img src="sign_false.png" />'
                        if match_info == 'no'
                        else '<img src="sign_warn.png" />'
    ...
    def _add_actioncondition(self, state, conditionset, action_type, new_y, cond1, cond2):
        ...
        cond_enter = (
            action_type in ['actions_enter', 'actions_enter_or_stay'] and self.__states[state].get('enter') is False
        )
        cond_stay = (
            action_type in ['actions_stay', 'actions_enter_or_stay'] and self.__states[state].get('stay') is False
        )
        color_enter = (
            'gray'
            if (cond1 and cond2 and cond5) or (cond_enter and cond4 and cond5)
            else 'olivedrab'
            if cond4
            else 'indianred2'
        )
        color_stay = (
            'gray'
            if (cond1 and cond2 and cond5) or (cond_stay and cond4 and cond5)
            else 'olivedrab'
            if cond4
            else 'indianred2'
        )
        ...
        self.__nodes['{}_{}_state_{}'.format(state, conditionset, action_type)] = pydotplus.Node(
            '{}_{}_state_{}'.format(state, conditionset, action_type),
            style='filled',
            fillcolor=color,
            shape='diamond',
            label=label,
        ...
        if self.__nodes.get('{}_{}_state_actions_enter_edge'.format(state, conditionset)) is None:
            self.__nodes['{}_{}_state_{}_edge'.format(state, conditionset, action_type)] = pydotplus.Edge(
            ...
            self.__graph.add_edge(
                pydotplus.Edge(
                    self.__nodes['{}_{}_state_actions_enter'.format(state, conditionset)],
        ...
        try:
            if action_type == 'actions_enter':
                self.__nodes['{}_{}_actions_enter'.format(state, conditionset)].obj_dict['attributes']['fillcolor'] = (
                    color
                )
        except Exception:
            pass
        try:
            if action_type == 'actions_stay':
                self.__nodes['{}_{}_actions_stay'.format(state, conditionset)].obj_dict['attributes']['fillcolor'] = (
                    color
                )
        except Exception:
            pass
    ...
    def drawgraph(self, filename):
        ...
        for i, state in enumerate(self.__states):
            ...
            try:
                ...
                list_index = 0
            color = 'olivedrab' if state == self.__active_state else 'gray' if i > list_index else 'indianred2'

            if not i == 0:
                new_y -= (
                    max(float(above_nodeheights[-1] + 0.3), 0.46, float(last_action_nodeheight)) * self.__heightfactor
                )  # half elipse added at end
                position = '{},{}!'.format(0, new_y)

                condition_node = (
                    'pass'
                    if self.__nodes.get('{}_pass'.format(previous_state))
                    else 'leave'
                    if self.__nodes.get('{}_leave'.format(previous_state))
                    else list(self.__states[previous_state]['conditionsets'].keys())[-1]
                )
                lastnode = self.__nodes['{}_{}'.format(previous_state, condition_node)]
                self.__nodes['{}_above'.format(state)] = pydotplus.Node(
                    '{}_above'.format(state), pos=position, shape='square', width='0', label=''
                )
                self.__graph.add_node(self.__nodes['{}_above'.format(state)])
                position = '{},{}!'.format(10 * self.__widthfactor, new_y)
                self.__nodes['{}_above_right'.format(state)] = pydotplus.Node(
                    '{}_above_right'.format(state), pos=position, shape='square', width='0', label=''
                )
                self.__graph.add_node(self.__nodes['{}_above_right'.format(state)])
                self.__graph.add_edge(
                    pydotplus.Edge(
                        self.__nodes['{}_above'.format(state)],
                        self.__nodes['{}_above_right'.format(state)],
                        style='bold',
                        color='black',
                        label='',
                        dir='none',
                ...
                self.__graph.add_edge(
                    pydotplus.Edge(
                        state, self.__nodes['{}_above'.format(state)], style='bold', color='black', label='', dir='back'
                    )
                )
            ...
            self.__nodes[state] = pydotplus.Node(
                state,
                pos=position,
                pin=True,
                notranslate=True,
                ...
                fillcolor=color,
                shape='ellipse',
                label='<<table border="0"><tr><td>{}</td></tr><hr/><tr><td>{}</td></tr></table>>'.format(
                    state, self.__states[state]['name']
                ),
            )
            position = '{},{}!'.format(10 * self.__widthfactor, new_y)
            self.__nodes['{}_right'.format(state)] = pydotplus.Node(
                '{}_right'.format(state), pos=position, shape='square', width='0', label=''
            )
            ...
            for j, conditionset in enumerate(self.__states[state]['conditionsets']):
                ...
                try:
                    cond2 = j > list(self.__states[state]['conditionsets'].keys()).index(
                        self.__active_conditionset
                ...
                color = (
                    ...
                    if (conditionset == self.__active_conditionset or cond3) and state == self.__active_state
                    else 'indianred2'
                )
                try:
                ...
                if len(actions_enter) > 0 or len(actions_enter_or_stay) > 0:
                    actionlist_enter, action_tooltip_enter, action_tooltip_count_enter = self._actionlabel(
                        state, 'actions_enter', conditionset, active
                    )
                active = True if cond_stay and cond6 else False
                if len(actions_stay) > 0 or len(actions_enter_or_stay) > 0:
                    actionlist_stay, action_tooltip_stay, action_tooltip_count_stay = self._actionlabel(
                        state, 'actions_stay', conditionset, active
                    )
                ...
                if len(actions_leave) > 0:
                    actionlist_leave, action_tooltip_leave, action_tooltip_count_leave = self._actionlabel(
                        state, 'actions_leave', conditionset, active
                    )
                ...
                if len(actions_pass) > 0:
                    actionlist_pass, action_tooltip_pass, action_tooltip_count_pass = self._actionlabel(
                        state, 'actions_pass', conditionset, active
                    )
                ...
                if j > 0:
                    new_y -= (
                        max(float(actions_nodeheights.get(j - 1, 0.0) + 0.2), conditionset_nodeheights[j - 1] + 0.2)
                        * self.__heightfactor
                    )
                else:
                ...
                if not conditionlist == '':
                    self.__nodes['{}_{}_conditions'.format(state, conditionset)] = pydotplus.Node(
                        '{}_{}_conditions'.format(state, conditionset),
                        style='filled',
                        fillcolor=color,
                        shape='rect',
                        label=conditionlist,
                    ...
                    parenthesis_edge = pydotplus.Edge(
                        self.__nodes['{}_{}_conditions'.format(state, conditionset)],
                        self.__nodes['{}_{}'.format(state, conditionset)],
                        arrowhead='none',
                        color='black',
                        style='dotted',
                ...
                if not actionlist_enter == '':
                    ...
                    xlabel = (
                        '1 tooltip'
                        if action_tooltip_count_enter == 1
                    ...
                    self.__nodes['{}_{}_actions_enter'.format(state, conditionset)] = pydotplus.Node(
                        '{}_{}_actions_enter'.format(state, conditionset),
                        style='filled',
                        fillcolor=color,
                        shape='rectangle',
                        label=actionlist_enter,
                        pos=position,
                        tooltip=action_tooltip_enter,
                        xlabel=xlabel,
                    )
                ...
                if not actionlist_stay == '':
                    ...
                    xlabel = (
                        '1 tooltip'
                        if action_tooltip_count_stay == 1
                    ...
                    self.__nodes['{}_{}_actions_stay'.format(state, conditionset)] = pydotplus.Node(
                        '{}_{}_actions_stay'.format(state, conditionset),
                        style='filled',
                        fillcolor=color,
                        shape='rectangle',
                        label=actionlist_stay,
                        pos=position,
                        tooltip=action_tooltip_stay,
                        xlabel=xlabel,
                    )
                ...
                if cond1 and cond2 and cond3 and cond4:
                    ...
                    nodeheight = 0.5
                    self.__nodes['{}_{}_right'.format(state, conditionset)] = pydotplus.Node(
                        '{}_{}_right'.format(state, conditionset),
                        shape='circle',
                        width='0.7',
                        pos=position,
                        label='',
                ...
                if j == 0:
                    self.__graph.add_edge(
                        pydotplus.Edge(
                            self.__nodes[state],
                            self.__nodes['{}_right'.format(state)],
                            style='bold',
                            color='black',
                            dir='none',
                            xlabel=xlabel,
                    ...
                    self.__graph.add_edge(
                        pydotplus.Edge(
                            ...
                            style='bold',
                            color='black',
                            tooltip='check first conditionset',
                        )
                    )
                    ...
                    self.__graph.add_edge(
                        pydotplus.Edge(
                            ...
                            style='bold',
                            color='black',
                            tooltip='check next conditionset',
                        )
                    )
            ...
            if len(actions_leave) > 0:
                ...
                self.__nodes['{}_leave'.format(state)] = pydotplus.Node(
                    '{}_leave'.format(state),
                    style='filled',
                    fillcolor=color,
                    shape='diamond',
                ...
                self.__graph.add_edge(
                    pydotplus.Edge(
                        previous_conditionset,
                        self.__nodes['{}_leave'.format(state)],
                        style='bold',
                        color='black',
                        tooltip='check leave',
                    )
                ...
                xlabel = (
                    '1 tooltip'
                    if action_tooltip_count_leave == 1
                    else '{} tooltips'.format(action_tooltip_count_leave)
                    if action_tooltip_count_leave > 1
                    else ''
                )
                # self._log_debug('action leave: {}', position)
                self.__nodes['{}_actions_leave'.format(state)] = pydotplus.Node(
                    '{}_actions_leave'.format(state),
                    style='filled',
                    fillcolor=color,
                    shape='rectangle',
                    label=actionlist_leave,
                    pos=position,
                    align='center',
                ...
                self.__graph.add_edge(
                    pydotplus.Edge(
                        self.__nodes['{}_leave'.format(state)],
                        self.__nodes['{}_actions_leave'.format(state)],
                        style='bold',
                        taillabel='True',
                        tooltip='run leave actions',
                    )
                )
            ...
            if len(actions_pass) > 0:
                ...
                color = 'gray' if cond2 and not cond3 else 'olivedrab' if cond3 else 'indianred2'
                self.__nodes['{}_pass'.format(state)] = pydotplus.Node(
                    '{}_pass'.format(state),
                    style='filled',
                    fillcolor=color,
                    shape='diamond',
                ...
                self.__graph.add_edge(
                    pydotplus.Edge(
                        previous_conditionset,
                        self.__nodes['{}_pass'.format(state)],
                        style='bold',
                        color='black',
                        tooltip='check pass',
                    )
                ...
                xlabel = (
                    '1 tooltip'
                    if action_tooltip_count_pass == 1
                    else '{} tooltips'.format(action_tooltip_count_pass)
                    if action_tooltip_count_pass > 1
                    else ''
                )
                # self._log_debug('action pass: {}', position)
                self.__nodes['{}_actions_pass'.format(state)] = pydotplus.Node(
                    '{}_actions_pass'.format(state),
                    style='filled',
                    fillcolor=color,
                    shape='rectangle',
                    label=actionlist_pass,
                    pos=position,
                    align='center',
                ...
                self.__graph.add_edge(
                    pydotplus.Edge(
                        self.__nodes['{}_pass'.format(state)],
                        self.__nodes['{}_actions_pass'.format(state)],
                        style='bold',
                        taillabel='    True',
                        tooltip='run pass actions',
                    )
                )
