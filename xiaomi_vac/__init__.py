...
class Robvac(SmartPlugin):
    ...
    def _read(self):
        ...
        try:
            ...
            self._data['clean_total_duration'] = clean_history.total_duration.total_seconds() / 60
            self._data['clean_ids'] = clean_history.ids
            self.logger.debug(
                'Xiaomi_Robvac: Statistics count {0}, area {1}², duration {2}, clean ids {3}'.format(
            ...
            if self._data.get('clean_ids') is not None:
                ...
                try:
                    self._data['clean_details_last0'] = self.vakuum.clean_details(
                        self._data['clean_ids'][0], return_list=False
                    )
                except Exception:
                    try:
                        self._data['clean_details_last0'] = self.vakuum.clean_details(self._data['clean_ids'][0])
                    except Exception:
                        self._data['clean_details_last0'] = []
                        self.logger.info('No details for last0 clean found')
                try:
                    self._data['last0_area'] = round(self._data['clean_details_last0'].area, 2)
                    self._data['last0_complete'] = self._data['clean_details_last0'].complete
                    self._data['last0_duration'] = round(
                        self._data['clean_details_last0'].duration.total_seconds() / 60, 2
                    )
                    self._data['last0_start_date'] = self._data['clean_details_last0'].start.strftime('%d.%m.%Y')
                    self._data['last0_start_time'] = self._data['clean_details_last0'].start.strftime('%H:%M')
                    self._data['last0_end_date'] = self._data['clean_details_last0'].start.strftime('%d.%m.%Y')
                    self._data['last0_end_time'] = (
                        self._data['clean_details_last0'].start + self._data['clean_details_last0'].duration
                    ).strftime('%H:%M')
                except Exception:
                ...
                try:
                    self._data['clean_details_last1'] = self.vakuum.clean_details(
                        self._data['clean_ids'][1], return_list=False
                    )
                except Exception:
                    try:
                        self._data['clean_details_last1'] = self.vakuum.clean_details(self._data['clean_ids'][1])
                    except Exception:
                        self._data['clean_details_last1'] = []
                        self.logger.info('No details for last1 clean found')

                try:
                    self._data['last1_area'] = round(self._data['clean_details_last1'].area, 2)
                    self._data['last1_complete'] = self._data['clean_details_last1'].complete
                    self._data['last1_duration'] = round(
                        self._data['clean_details_last1'].duration.total_seconds() / 60, 2
                    )
                    self._data['last1_start_date'] = self._data['clean_details_last1'].start.strftime('%d.%m.%Y')
                    self._data['last1_start_time'] = self._data['clean_details_last1'].start.strftime('%H:%M')
                    self._data['last1_end_date'] = self._data['clean_details_last1'].start.strftime('%d.%m.%Y')
                    self._data['last1_end_time'] = (
                        self._data['clean_details_last1'].start + self._data['clean_details_last1'].duration
                    ).strftime('%H:%M')
                except Exception:
                ...
                try:
                    self._data['clean_details_last2'] = self.vakuum.clean_details(
                        self._data['clean_ids'][2], return_list=False
                    )
                except Exception:
                    try:
                        self._data['clean_details_last2'] = self.vakuum.clean_details(self._data['clean_ids'][2])
                    except Exception:
                        self._data['clean_details_last2'] = []
                        self.logger.info('No details for last2 clean found')

                try:
                    self._data['last2_area'] = round(self._data['clean_details_last2'].area, 2)
                    self._data['last2_complete'] = self._data['clean_details_last2'].complete
                    self._data['last2_duration'] = round(
                        self._data['clean_details_last2'].duration.total_seconds() / 60, 2
                    )
                    self._data['last2_start_date'] = self._data['clean_details_last2'].start.strftime('%d.%m.%Y')
                    self._data['last2_start_time'] = self._data['clean_details_last2'].start.strftime('%H:%M')
                    self._data['last2_end_date'] = self._data['clean_details_last2'].start.strftime('%d.%m.%Y')
                    self._data['last2_end_time'] = (
                        self._data['clean_details_last2'].start + self._data['clean_details_last2'].duration
                    ).strftime('%H:%M')
                except Exception:
                ...
                try:
                    self._data['clean_details_last3'] = self.vakuum.clean_details(
                        self._data['clean_ids'][3], return_list=False
                    )
                except Exception:
                    try:
                        self._data['clean_details_last3'] = self.vakuum.clean_details(self._data['clean_ids'][3])
                    except Exception:
                        self._data['clean_details_last3'] = []
                        self.logger.info('No details for last3 clean found')

                try:
                    self._data['last3_area'] = round(self._data['clean_details_last3'].area, 2)
                    self._data['last3_complete'] = self._data['clean_details_last3'].complete
                    self._data['last3_duration'] = round(
                        self._data['clean_details_last3'].duration.total_seconds() / 60, 2
                    )
                    self._data['last3_start_date'] = self._data['clean_details_last3'].start.strftime('%d.%m.%Y')
                    self._data['last3_start_time'] = self._data['clean_details_last3'].start.strftime('%H:%M')
                    self._data['last3_end_date'] = self._data['clean_details_last3'].start.strftime('%d.%m.%Y')
                    self._data['last3_end_time'] = (
                        self._data['clean_details_last3'].start + self._data['clean_details_last3'].duration
                    ).strftime('%H:%M')
                except Exception:
                ...
                self.logger.debug(
                    'Xiaomi_Robvac: historic id1 {}, id2 {}, id3 {}'.format(
                        self._data['clean_details_last0'],
                        self._data['clean_details_last1'],
                        self._data['clean_details_last2'],
                    )
                ...
                self.logger.debug(
                    'Xiaomi_Robvac: Clean Run complete id1 {}, id2 {}, id3 {}'.format(
                        self._data['last1_complete'], self._data['last2_complete'], self._data['last3_complete']
                    )
                )
            ...
            self.logger.debug(
                'Xiaomi_Robvac: Carpet Mode high: {}, integral: {}, low: {}, enabled: {}, stall_time: {}'.format(
            ...
            self.logger.debug(
                'Xiaomi_Robvac: Serial{}, vol {}, dnd status {}, dnd start {},dnd end {},'.format(
                    self._data['serial'],
                    self._data['volume'],
            ...
            self.logger.debug(
                'Xiaomi_Robvac: segment_status {}, fanspeed {}, '
            ...
            self.logger.debug(
                'Xiaomi_Robvac: error {}, pause {}, status {} , timer {}, timezone {}'.format(
                    self._data['error'],
                    self._data['pause'],
                    self._data['state'],
                    self._data['timer'],
            ...
            self.logger.debug(
                'Xiaomi_Robvac: Brush Side {0}/{1}, Brush Main {2}/{3}, Filter{4}/{5}, Sensor{6}/{7}'.format(
        ...
        for x in self._data:
            if x in self.messages:
                self.logger.debug(
                    'Xiaomi_Robvac: Update item {1} with key {0} = value {2}'.format(x, self.messages[x], self._data[x])
                )
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        if caller != 'Robvac' and self.alive:
            ...
            if not self._connected:
                ...
                self.logger.error(
                    'Xiaomi_Robvac: Keyword {0}, item {1} not changed - no connection! Resetting item to {2}'.format(
                ...
                message = self.get_iattr_value(item.conf, 'robvac')
                self.logger.debug(
                    'Xiaomi_Robvac: Keyword {0}, item {1} changed to {2}'.format(message, item, item.property.value)
                )
                if message == 'fanspeed':
                    ...
                    if item().lower() in ['sensor_dirty', 'sensor_reinigen']:
                        ...
                        self.logger.warning(
                            'Consumable {} does not exit. Please use only sensor_dirty/sensor_reinigen, main_brush/buerste_haupt, side_brush/buerste_seite, filter.'.format(
                                item.property.value
                            )
                        )
