...
class RCS1000N(SmartPlugin):
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, 'rcs_SystemCode'):
            ...
            systemcode_ok = cRcSocketSwitch.RCS1000N.sanity_check_Systemcode(
                self.get_iattr_value(item.conf, 'rcs_SystemCode')
            )
            if self.has_iattr(item.conf, 'rcs_ButtonCode'):
                # do a sanity check for rcs_ButtonCode
                buttoncode_ok = cRcSocketSwitch.RCS1000N.sanity_check_Buttoncode(
                    self.get_iattr_value(item.conf, 'rcs_ButtonCode')
                )
                if systemcode_ok and buttoncode_ok:
                    return self.update_item
                else:
                    self.logger.warning(
                        'Warning: Item {} is NOT correctly configured!. Item will be ignored by rcSwitch_python plugin'.format(
                            item.property.path
                        )
                    )
            else:
                self.logger.warning(
                    'Warning: attribute <rcs_ButtonCode> for {} missing. Item will be ignored by rcSwitch_python plugin'.format(
                        item.property.path
                    )
                )
                return None
            ...
            self.logger.warning(
                'Warning: attribute <rcs_SystemCode> for {} missing. Item will be ignored by RCswitch plugin'.format(
                    item.property.path
                )
            )
            return None
        ...
        if self.alive and caller != self.get_shortname():
            ...
            with self._lock:
                try:
                    ...
                    obj.send(*values)
                    self.logger.info(
                        'Info: setting Device {} with SystemCode {} to {}'.format(ButtonCode, SystemCode, value)
                    )
                finally:
