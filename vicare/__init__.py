...
class Vicare(SmartPlugin):
    ...
    def __init__(self, sh):
        ...
        if not temp == '':
            ...
            self.logger.error(
                'ClientID not configured for pluging. Please register device via Viessmann developper portal.'
            )
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        if caller != self.get_shortname():
            if self.has_iattr(item.conf, 'vicare_tx_key'):
                ...
                uri, tag, type, min, max, stepping, enumList = self.decodeCommandFeature(
                    self.featureListJson, vicare_tx_key=tx_key, vicare_tx_path=tx_path, log_features=False
                )
                ...
                if uri is not None:
                    ...
                    self.logger.debug(
                        f'Item {item} cannot be controlled because no valid command url was found. Aborting.'
                    )
            else:
    ...
    def pollInstallationId(self):
        ...
        if response.status_code == 200:
            ...
            self.logger.warning(
                f'pollInstallationId request was unsuccessfull. Status code: {response.status_code}, Text: {response.text}'
            )
            return
        ...
        if response.json() is not None:
            ...
            if 'data' in responseJson:
                ...
                if nr_installations > 1:
                    self.logger.warning(
                        f'pollInstallationId: {nr_installations} installations found but only the first is decoded.'
                    )
    ...
    def pollSerial(self):
        ...
        if response.status_code == 200:
            ...
            self.logger.warning(
                f'pollSerial request was unsuccessfull. Status code: {response.status_code}, Text: {response.text}'
            )
            return
    ...
    def pollDevices(self):
        ...
        if response.status_code == 200:
            ...
            self.logger.warning(
                f'pollDevicesrequest was unsuccessfull. Status code: {response.status_code}, Text: {response.text}'
            )
            return
                ...
                if self.nr_devices > 1:
                    ...
                    for i in range(0, self.nr_devices):
                        if 'boilerSerial' in dataJson[i] and (dataJson[i]['boilerSerial'] is not None):
                            ...
                            index_of_device = i
                            self.logger.info(
                                f'pollDevices: Decoding device with index {index_of_device} and boiler serial number:{dataJson[i]["boilerSerial"]}'
                            )

                    if not found_valid_serial:
                        self.logger.error(
                            'pollDevices: No device with valid boiler serial number found. Perhaps plugin has to be extended for additional device types. Aborting decoding.'
                        )
                        return
                ...
                for item in self._rx_items:
                    if item.conf['vicare_rx_key'] == feature:
                        ...
                        if isEnabled and isReady and self.has_iattr(item.conf, 'vicare_path') and properties:
                            ...
                            self.logger.warning(
                                f'Vicare Item found for feature: {feature} but not marked as usable (enabled:{isEnabled}, ready:{isReady}, properties:{properties}, vicare_path:{self.has_iattr(item.conf, "vicare_path")})'
                            )
        else:
        ...
        for i in range(0, nr_features):
            ...
            if feature == vicare_tx_key:
                    ...
                    if not commandsItem == {}:
                        ...
                        for k in range(0, length_path):
                            ...
                            try:
                                ...
                                self.logger.debug(
                                    f'Additional debug info for exception: k:{k}, vicare_tx_path:{vicare_tx_path}, tag:{tag}'
                                )
                                self.logger.debug(
                                    f'Additional debug info for exception2: commandsItem:{commandsItem}, controlItem :{controlItem}'
                                )
                ...
                if not uri == '' and isExecutable:
                    self.logger.debug(
                        f'Debug: Execute command with uri,tag,type, min, max, stepping, enumList: {uri},{tag},{type},{min},{max},{stepping},{enumList}'
                    )
    ...
    def controlItem(self, url, tag, type, min, max, stepping, enumList, value):
        ...
        if enumList and len(enumList) > 0 and isinstance(value, str):
            if value not in enumList:
                self.logger.warning(
                    f'controlItem: String value ({value}) is not in the list of allowed values ({enumList}). Aborting.'
                )
                return
            else:
        ...
        if type is not None:
            ...
            if not value:
                self.logger.debug(
                    'controlItem: Supressing sending for command without additional data because item is false'
                )
                return
        ...
        if response is not None and response.status_code == 200:
            ...
            self.logger.warning(
                f'controlItem request was unsuccessfull. Status code: {response.status_code}, text: {response.text}'
            )
            return
    ...
    def generateAPIToken(self, codeVerifier=''):
        ...
        oauth_session = OAuth2Session(
            self.clientID, redirect_uri=self.redirectUrl, scope=VIESSMANN_SCOPE, code_challenge_method='S256'
        )
        ...
        response = requests.post(
            authorization_url, headers=header, auth=(self.user, self.password), allow_redirects=True
        )
        ...
        token = oauth_session.fetch_token(
            TOKEN_URL, authorization_response=response.headers['Location'], code_verifier=codeVerifier
        )
