    ...
    def __init__(self, smarthome):
        ...
        try:
            ...
            self._squeezebox_tcp_connection.set_callbacks(
                connected=self._on_connect, data_received=self._on_received_data, disconnected=self._on_disconnect
            )
            self._val = {}
            self._obj = {}
            self._init_cmds = []
    ...
    def _resolv_full_cmd(self, item, attr):
        ...
        if self.has_iattr(item.conf[attr], '<playerid>'):
            ...
            if (
                ...
                and self.has_iattr(parent_item.conf, 'squeezebox_playerid')
                and self._check_mac(self.get_iattr_value(parent_item.conf, 'squeezebox_playerid'))
            ):
                item.conf[attr] = item.conf[attr].replace(
                    '<playerid>', self.get_iattr_value(parent_item.conf, 'squeezebox_playerid')
                )
            else:
                grandparent_item = parent_item.return_parent()
                if (
                    (grandparent_item is not None)
                    and self.has_iattr(grandparent_item.conf, 'squeezebox_playerid')
                    and self._check_mac(self.get_iattr_value(grandparent_item.conf, 'squeezebox_playerid'))
                ):
                    item.conf[attr] = item.conf[attr].replace(
                        '<playerid>', self.get_iattr_value(grandparent_item.conf, 'squeezebox_playerid')
                    )
                else:
                    grandgrandparent_item = grandparent_item.return_parent()
                    if (
                        (grandgrandparent_item is not None)
                        and self.has_iattr(grandgrandparent_item.conf, 'squeezebox_playerid')
                        and self._check_mac(self.get_iattr_value(grandgrandparent_item.conf, 'squeezebox_playerid'))
                    ):
                        item.conf[attr] = item.conf[attr].replace(
                            '<playerid>', self.get_iattr_value(grandgrandparent_item.conf, 'squeezebox_playerid')
                        )
                    else:
                        self.logger.warning(
                            'could not resolve playerid for {0} from parent item {1}, neither from grandparent {2} or grandgrandparent {3}'.format(
                                item, parent_item, grandparent_item, grandgrandparent_item
                            )
                        )
                        return None
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        if caller != 'LMS':
            ...
            condition2 = (
                ...
                and cmd[1] == 'playlist'
                and cmd[2] == 'shuffle'
                and caller == 'on_change'
                and newsource == 'shuffle'
            )
            condition3 = (
                condition0
                and cmd[1] == 'playlist'
    ...
    def _send(self, cmd):
        if self._squeezebox_tcp_connection.connected():
            ...
            while not cmd == 'listen 1' and self._listen is False:
                self.logger.debug(
                    'Waiting to send command {} as connection is not yet established. Count: {}/10'.format(cmd, i)
                )
                i += 1
                time.sleep(1)
                if i >= 10:
        ...
        try:
            ...
            if self._check_mac(data[0]):
                if data[1] == 'time' and (data[2].startswith('+') or data[2].startswith('-')):
                    ...
                    (((data[1] == 'prefset') and (data[2] == 'server')) or (data[1] == 'mixer'))
                    and (data[-2] == 'volume')
                    ...
                    if data[2] == 'play' and data[3] == '1':
                        ...
                        self._update_items_with_data([data[0], 'playlist name', ''])
