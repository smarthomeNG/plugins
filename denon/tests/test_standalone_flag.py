...
"""
Tests that plugins.denon's module-level bootstrap does not clobber an
...
locate('plugins.denon.commands'), which needs the plugins.denon package
importable under its real dotted name - re-running this module's top-level
code with __name__ == 'plugins.denon', hitting the `else` branch. Simulated
...
class TestDenonStandaloneFlagNotClobbered(unittest.TestCase):
    def test_reload_does_not_reset_true_to_false(self):
        import plugins.denon  # noqa: F401  (else branch runs, sets False - expected for normal import)
        ...
        importlib.reload(plugins.denon)
