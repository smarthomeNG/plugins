...
class Smlx(SmartPlugin):
    ...
    def __init__(self, sh, *args, **kwargs):
        ...
        if device == 'hex':
            ...
            self._prepare = self._prepareRaw
        self.logger.debug(
            'Using CRC params poly={}, reflect_in={}, xor_in={}, reflect_out={}, xor_out={}, swap_crc_bytes={}'.format(
                self.poly, self.reflect_in, self.xor_in, self.reflect_out, self.xor_out, self.swap_crc_bytes
            )
        )
                ...
                if end_sequence in data:
                    ...
                    if len(rest) > 3:
                        ...
                        if not self.swap_crc_bytes:
                            self.logger.debug(
                                'Calculated checksum is {}, given CRC is {}'.format(
                                    to_Hex(crc_calculated), to_Hex(checksum)
                                )
                            )
                            data_is_valid = crc_calculated == checksum
                        else:
                            self.logger.debug(
                                'Calculated and swapped checksum is {}, given CRC is {}'.format(
                                    to_Hex(swap16(crc_calculated)), to_Hex(checksum)
                                )
                            )
    ...
    def _parse(self, data):
        ...
        while self._dataoffset < len(data) - packetsize:
            ...
            if (
                data[self._dataoffset] == 0x77
                and data[self._dataoffset + 1] == 0x07
                and data[self._dataoffset + 2] != 0xFF
            ):
                ...
                try:
                    ...
                    entry['valueReal'] = (
                        entry['value'] * 10 ** entry['scaler'] if entry['scaler'] is not None else entry['value']
                    )
                    entry['unitName'] = (
                        self._units[entry['unit']]
                        if entry['unit'] is not None and entry['unit'] in self._units
                        else None
                    )
                    entry['actualTime'] = (
