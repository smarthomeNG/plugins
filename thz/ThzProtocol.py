    ...
    if self._portHandler.isPortOpen():
      ...
      if not msg == None:
        ...
        if version in VersionSpecificMsgData:
          ...
          logger.info('thz: Message templates adjusted to version {}'.format(version))
        else:
          # no version-specific data availble
          logger.info('thz: Using generic message templates with version {}'.format(version))
      else:
        logger.warning('thz: The messages could not be adjusted to the heat pump version due to missing heat pump response.')

    else:
      logger.warning('thz: The messages could not be adjusted to the heat pump version due to com port failure.')
  ...
  def _decodeMsg(self, data):
    ...
    if data[0] == 0x15:
      ...
      if crc != self._computeChecksum(data):
        ...
        logger.warning('Received message with bad CRC')
        return None
      if data[3] == 0x0a or data[3] == 0x0b or data[3] == 0x0c:
      ...
      if param in self._msgNameFromCode:
        ...
        logger.debug('param = {} ({})'.format(name, param))
      else:
        logger.warning('Rx: Unknown command - {}'.format(param))
        return None
      msg = {name: {}}
      for key in MsgTemplate[name]['params'].keys():
        ...
        try:
          ...
          logger.warning('Message {} too short (length = {}, index = {})'.format(name, len(payload), item[0]))
      return msg
    return None
...
###############################################################################
  def _sendGetRequest(self, cmd):
    ...
    try:
      ...
      if not data == None:
        self._rxCount += 1
        logger.debug('Rx: ' + ' '.join(format(x, '02x') for x in data))
        msg = self._decodeMsg(data)
      else:
        self._rxTimeoutCount += 1
        msg = None
    except:
      logger.error('Get request failed - {:06x}\n{}'.format(cmd, sys.exc_info()))
    finally:
      self._lock.release()
    return msg
...
###############################################################################
#
# getMsgFromParameter() returns the message name for the given parameter name
#
###############################################################################
...
###############################################################################
#
# isParameterWritable() returns True if the specified parameter is writable
#
###############################################################################
...
###############################################################################
#
# sendSetRequest() encodes and sends a SET request and processes the response
#
###############################################################################
    ...
    if param in self._msgNameFromParam:
      ...
      try:
        ...
        if (value < tmpl['params'][param][4] or
          ...
          logger.warning('Parameter value out of range - {}={}, [{}:{}]'.format(param, value, tmpl['params'][param][4], tmpl['params'][param][5]))
        else:
          # value OK
          ...
          data = self._portHandler.readData()
          if data == None:
            logger.warning('Set request timed out')
            self._rxTimeoutCount += 1
          else:
            logger.debug('Rx: ' + ' '.join(format(x, '02x') for x in data))
            self._rxCount += 1
          if not (data[0] == 0x01 and data[1] == 0x80):
            logger.warning('Set request failed - {}: {}'.format(param, ' '.join(format(x, '02x') for x in data)))
            self._rxNackCount += 1
      except:
      ...
      logger.warning('Invalid parameter name - {}'.format(param))

###############################################################################
#
# sendRawSetRequest() encodes and sends a RAW SET request and processes the
#  response. Param and value are hex strings.
#
###############################################################################
  def sendRawSetRequest(self, param, value):
    ...
    try:
        ...
        if data == None:
          ...
          logger.info('Rx: ' + ' '.join(format(x, '02x') for x in data))
          self._rxCount += 1
        if not (data[0] == 0x01 and data[1] == 0x80):
          logger.warning('Set request failed - {}: {}'.format(param, ' '.join(format(x, '02x') for x in data)))
          self._rxNackCount += 1
...
###############################################################################
#
# requestData() returns a dict with message and parameter values for the
#               specified request
#
###############################################################################
...
###############################################################################
#
# stop() passes the stop request to the port handler
#
###############################################################################
  def stop(self):
    self._portHandler.stop()
...
###############################################################################
#
# getStats() returns the counter values
#
###############################################################################
  def getStats(self):
    val = {}
...
###############################################################################
#
# logRegister() - logs the value of the specified register
#
###############################################################################
...
###############################################################################
#
# logFullScan() - performs a full register scan and logs the results
#
###############################################################################
