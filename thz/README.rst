...
-  Serial port connected to the LWZ/THZ maintenance port. 
   The wiring description is provided at: http://robert.penz.name/heat-pump-lwz
...
According to the FHEM forum (http://forum.fhem.de/index.php/topic,13132.0.html)
the original Perl code also works with other heatpump models (303, 304, 403).
...
-  **serial\_port**: serial port connected to the heatpump
-  **baudrate**: port speed, recent models use 115200 bps, older heatpumps may use 57600 bps or even 9600 bps
-  **poll\_period**: interval in seconds how often the data is read from the heatpump
-  **min\_update\_period**: interval in seconds for updating the values changing frequently (e.g. temperature), it should be higher than the poll period
-  **max\_update\_period**: interval in seconds for updating the values with infrequent changes (e.g. counters updated once per day)
