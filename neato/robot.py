...
class Robot:
    ...
    def robot_command(self, command, arg1=None, arg2=None):
        ...
        try:
            start_cleaning_response = self._session.post(
                self.__urlNucleo + '/vendors/' + self.__vendor + '/robots/' + self.serial + '/messages',
                data=n,
                headers={
                    'X-Date': self.__get_current_date(),
        ...
        if 'result' in responseJson:
            if str(responseJson['result']) == 'ok':
                ...
                self.logger.warning(
                    f'Command returned {str(responseJson["result"])}: Retry starting with non-persistent-map'
                )
    ...
    def update_robot(self):
        ...
        h = hmac.new(self.__secretKey.encode('utf-8'), message.encode('utf8'), hashlib.sha256)
        try:
            robot_cloud_state_response = self._session.post(
                self.__urlNucleo + '/vendors/' + self.__vendor + '/robots/' + self.serial + '/messages',
                data=m,
                headers={
                    'X-Date': self.__get_current_date(),
        ...
        if statusCode == 200:
            ...
            self.logger.warning(
                f'Internal Backend Server Error 500, Optional message: {robot_cloud_state_response.text}'
            )
            return 'error'
        else:
            self.logger.error(
                f'Sending cloud state request error: {statusCode}, msg: {robot_cloud_state_response.text}'
            )
            return 'error'
    ...
    def __get_access_token(self):
        ...
        try:
            access_token_response = self._session.post(
                self.__urlBeehive + '/sessions',
                json=json_data,
    ...
    def __get_secret_key(self):
        ...
        try:
            secret_key_response = self._session.get(
                self.__urlBeehive + '/users/me/robots',
                data=auth_data,
    ...
    def __cleaning_start_string(self, boundary_id=None, map_id=None, fallback_category=None):
        ...
        if (local_category is None) or (local_category == '') or (local_category != 4 and local_category != 2):
            ...
            self.logger.info(
                'Robot: category changed for send command from {0} (received) to {1}'.format(
                    self.category, local_category
                )
            )
        ...
        if fallback_category is not None:
            self.logger.info(
                f'Robot: category changed via external category for send command from {local_category} (received) to {fallback_category}'
            )
            local_category = fallback_category

        if self.houseCleaning == 'basic-1':
            return (
                '{"reqId": "77","cmd": "startCleaning","params": {"category": '
                + str(local_category)
                + ',"mode": '
                + str(self.mode)
                + ', "modifier": '
    ...
    def request_oauth2_code(self, hash=''):
        ...
        self.logger.info(
            'Requesting authentication code for {0} with challenge {1}'.format(self.__email, self._clientIDHash)
        )
        ...
        try:
            request_code_response = self._session.post(
                'https://mykobold.eu.auth0.com/passwordless/start',
                json=usedata,
    ...
    def request_oauth2_token(self, code, hash=''):
        ...
        if not hash == '':
            self._clientIDHash = str(hash)
        self.logger.info(
            'Requesting authentication token for {0} with code {1} and challenge {2}'.format(
                self.__email, code, self._clientIDHash
            )
        )
        ...
        try:
            request_token_response = self._session.post(
                'https://mykobold.eu.auth0.com/oauth/token',
                json=usedata,
                headers={'Content-Type': 'application/json'},
                timeout=self._timeout,
            )
    ...
    def get_secretKey_viaOauth(self):
        ...
        try:
            request_robots_response = self._session.get(
                self.__urlBeehive + '/dashboard',
                headers={'Authorization': 'Auth0Bearer ' + str(self._token)},
                timeout=self._timeout,
                verify=False,
            )
