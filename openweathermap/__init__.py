...
class OpenWeatherMap(SmartPlugin):
    ...
    def __init__(self, sh, *args, **kwargs):
        ...
        if (
            self.get_parameter_value('latitude') != ''
            and self.get_parameter_value('longitude') != ''
            and self.get_parameter_value('altitude') != ''
        ):
        ...
        self._data_sources = {
            self._data_source_key_weather: {'url': '', 'fetched': '', 'data': 'Not downloaded!'},
        ...
        self._origins_onecall = ['lat', 'lon', 'timezone', 'timezone_offset', 'alerts']

        self._origins_weather = [
            'base',
            'clouds',
            'cod',
            'coord',
        ...
        self._origins_layer = ['clouds_new', 'precipitation_new', 'pressure_new', 'wind_new', 'temp_new']
    ...
    def _download_data(self):
        ...
        Downloads data according to items' demands
        """
        self.__query_api_if(self._data_source_key_weather, only_if=self._request_weather)
        self.__query_api_if(self._data_source_key_forecast, only_if=self._request_forecast)
        self.__query_api_if(self._data_source_key_uvi, only_if=self._request_uvi)
        self.__query_api_if(self._data_source_key_airpollution_current, only_if=self._request_airpollution_current)
        self.__query_api_if(self._data_source_key_airpollution_forecast, only_if=self._request_airpollution_forecast)

        self.__query_api_if(
            self._data_source_key_airpollution_back1day, only_if=self._request_airpollution_back1day, delta_t=-1
        )
        self.__query_api_if(
            self._data_source_key_airpollution_back2day, only_if=self._request_airpollution_back2day, delta_t=-2
        )
        self.__query_api_if(
            self._data_source_key_airpollution_back3day, only_if=self._request_airpollution_back3day, delta_t=-3
        )
        self.__query_api_if(
            self._data_source_key_airpollution_back4day, only_if=self._request_airpollution_back4day, delta_t=-4
        )

        get_onecall = (
            self._request_current
            or self._request_minutely
            or self._request_hourly
            or self._request_daily
            or self._request_alerts
        )
        self.__query_api_if(self._data_source_key_onecall, only_if=get_onecall)

        self.__query_api_if(self._data_source_key_back0day, only_if=self._request_back0day, delta_t=0)
        self.__query_api_if(self._data_source_key_back1day, only_if=self._request_back1day, delta_t=-1)
        self.__query_api_if(self._data_source_key_back2day, only_if=self._request_back2day, delta_t=-2)
        self.__query_api_if(self._data_source_key_back3day, only_if=self._request_back3day, delta_t=-3)
        self.__query_api_if(self._data_source_key_back4day, only_if=self._request_back4day, delta_t=-4)
    ...
    def get_value_with_meta(self, owm_matchstring, correlation_hint=''):
        ...
        try:
            if (s.startswith('current/') or s.startswith('daily/') or s.startswith('day/')) and s.endswith('/eto'):
                ...
                s.startswith('current/') or s.startswith('daily/') or s.startswith('day/') or s.startswith('hour/')
            ) and (s.endswith('/wind_speed/beaufort') or s.endswith('/wind_speed/description')):
                wrk_typ = 'onecall [bft-calculation]'
                mps_string = s.replace('/wind_speed/beaufort', '/wind_speed')
                mps_string = mps_string.replace('/wind_speed/description', '/wind_speed')
                wind_mps, updated_s = self.__get_val_from_dict(mps_string, wrk, correlation_hint, owm_matchstring)
                ...
                s.startswith('current/') or s.startswith('daily/') or s.startswith('day/') or s.startswith('hour/')
            ) and (s.endswith('/wind_gust/beaufort') or s.endswith('/wind_gust/description')):
                wrk_typ = 'onecall [bft-calculation]'
                mps_string = s.replace('/wind_gust/beaufort', '/wind_gust')
                mps_string = mps_string.replace('/wind_gust/description', '/wind_gust')
                wind_mps, updated_s = self.__get_val_from_dict(mps_string, wrk, correlation_hint, owm_matchstring)
                bft_val = self.get_beaufort_number(wind_mps)
                if s.endswith('/beaufort'):
                    ret_val = bft_val
    ...
    def get_value(self, owm_matchstring, correlation_hint=''):
        ret_val, _, _, _ = self.get_value_with_meta(owm_matchstring, correlation_hint)
        return ret_val
            ...
            if owm_matchstring in self._origins_layer:
                ...
                try:
                    ...
                    if ret_val is None:
                        ...
                        self.logger.info(
                            '%s INFO: owm-string: %s --> %s from wrk=%s, Info: %s'
                            % (item, owm_matchstring, changed_match_string, wrk_typ, ret_val)
                        )
                    elif isinstance(ret_val, Exception):
                        if str(ret_val) == 'cannot unpack non-iterable NoneType object':
                            self.logger.debug(
                                '%s ERROR: owm-string: %s --> %s from wrk=%s, Error: %s'
                                % (item, owm_matchstring, changed_match_string, wrk_typ, ret_val)
                            )
                        else:
                            self.logger.info(
                        ...
                        item(ret_val, self.get_shortname(), f'{wrk_typ} // {changed_match_string}')
                        if was_ok:
                            self.logger.debug(
                                '%s OK: owm-string: %s --> %s from wrk=%s'
                                % (item, owm_matchstring, changed_match_string, wrk_typ)
                            )
                        else:
                            self.logger.warning(
                                '%s OK, FIXED: owm-string: %s --> %s from wrk=%s'
                                % (item, owm_matchstring, changed_match_string, wrk_typ)
                            )
                except Exception as e:
    ...
    def __calculate_eto(self, s, correlation_hint):
        ...
        """
        self.logger.debug('%s _calculate_eto: for %s' % ((correlation_hint, s)))
        ...
        slopeSvpc = 4098 * (0.6108 * math.exp((17.27 * tMean) / (tMean + 237.3))) / math.pow((tMean + 237.3), 2)
        ...
        Ra = (
            ...
            * 0.082
            * dR
            * ((sunsetHA * math.sin(lRad) * math.sin(sD)) + (math.cos(lRad) * math.cos(sD) * math.sin(sunsetHA)))
        )
        Rso = Ra * (0.75 + (2 * alt / 100000))
        Rns = rS * (1 - 0.23)
        Rnl = 4.903 * math.pow(10, -9) * (math.pow((273.16 + climate_max), 4) + math.pow((273.16 + climate_min), 4)) / 2
        ...
        self.logger.debug('%s _calculate_eto: %s, eTo: %s' % (correlation_hint, s, eTo))
        return eTo
    ...
    def __get_virtual_value(self, virtual_ms, correlation_hint):
        ...
        self.logger.debug(f'{correlation_hint} {virtual_ms}  Pool: {pformat(pool, width=4000)}')
    ...
    def __handle_fail(self, last_popped, current_leaf, successful_path, original_match_string, correlation_hint):
        ...
        if fail_match_string in self._soft_fails:
            ...
            if soft_fail_mode == 'log_info':
                ...
                self.logger.debug(
                    '%s DEBUG: owm-string: %s --> %s, Missing Child, Soft-Fail to no_update'
                    % (correlation_hint, original_match_string, changed_match_string)
                )
                return None
    ...
    def __get_val_from_dict(self, s, wrk, correlation_hint, original_match_string):
        ...
        if wrk == 'Not downloaded!':
            self.logger.debug(
                f'{correlation_hint} __get_val_from_dict function aborted because dictionary data are invalid.'
            )
            return
        ...
        while True:
            ...
            if type(wrk) is list:
                ...
                if type(wrk) is not dict:
                    self.logger.error(f'{correlation_hint}s={s} wrk={wrk} type(wrk)={type(wrk)}')
                wrk = wrk.get(sp[0])
    ...
    def get_daily_forecast(self, s):
        ...
        if forecast is None:
            self.logger.error('get_daily_forecast: forecast is None')
            return None
                ...
                for entry in forecast_list:
                    ...
                    if dt >= int(date_requested.timestamp()):
                        ...
                        if isinstance(val, float) or isinstance(val, int):
                            ...
                            self.logger.error(
                                "get_daily_forecast: found None value while calculating daily forecast for matchstring '{}'.".format(
                                    s
                                )
                            )
                            return 0
    ...
    def __query_api_if(self, data_source_key, only_if=False, delta_t=0, force=False):
        if only_if or force:
            ...
            if self._data_sources[data_source_key]['data'] == 'Not downloaded!':
                self._data_sources[data_source_key]['data'] = (
    ...
    def __query_api(self, data_source_key, delta_t=0, force=False):
        ...
        try:
            ...
            self.logger.error(f'Request failed for DataSource {data_source_key}: {str(e)}')
            return False
    ...
    def __get_timestamp_for_delta_days(self, delta_t):
        ...
        date_requested = datetime(datetime_now.year, datetime_now.month, datetime_now.day, tzinfo=timezone.utc)
    ...
    def __build_url(self, url_type=None, item=None, delta_t=0, force=False):
        ...
        if url_type is None or url_type == self._data_source_key_weather:
            ...
            parameters = '?lat=%s&lon=%s&appid=%s&lang=%s&units=%s' % (
                self._lat,
                self._lon,
                self._key,
                self._lang,
            ...
            parameters = '?lat=%s&lon=%s&appid=%s' % (self._lat, self._lon, self._key)
            url = '%s%s' % (url, parameters)
            ...
            parameters = '?lat=%s&lon=%s&start=%i&end=%i&appid=%s' % (
                self._lat,
                self._lon,
                self.shtime.utcnow().timestamp(),
                self.__get_timestamp_for_delta_days(5),
                self._key,
            )
            url = '%s%s' % (url, parameters)
            ...
            parameters = '?lat=%s&lon=%s&start=%i&end=%i&appid=%s' % (
                self._lat,
                self._lon,
                self.__get_timestamp_for_delta_days(delta_t),
                self.__get_timestamp_for_delta_days(delta_t + 1),
                self._key,
            )
            url = '%s%s' % (url, parameters)
            ...
            parameters = '?lat=%s&lon=%s&exclude=%s&appid=%s&lang=%s&units=%s' % (
                self._lat,
                self._lon,
                exclude,
                self._key,
            ...
            parameters = '?lat=%s&lon=%s&dt=%i&appid=%s&lang=%s&units=%s' % (
                self._lat,
                self._lon,
                self.__get_timestamp_for_delta_days(delta_t),
                self._key,
            ...
            if self.has_iattr(item.conf, 'owm_coord_z'):
                ...
                self.logger.warning(f'{item.property.path} owm_coord_z attribute not set for item, setting default 7')
                z = 7

            x_fallback, y_fallback = self.__get_map_tile_from_geo_coord(float(self._lat), float(self._lon), z)
            ...
            self.logger.error('%s __build_url: Wrong url type specified: %s' % (item.property.path, url_type))
        return url
