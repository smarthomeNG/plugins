...
class MusicServiceItem:
    ...
    def didl_metadata(self):
        ...
        item = XML.SubElement(xml, '{urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/}item', item_attrib)

        # Add title and class
        XML.SubElement(item, '{http://purl.org/dc/elements/1.1/}title').text = self.title
        XML.SubElement(item, '{urn:schemas-upnp-org:metadata-1-0/upnp/}class').text = self.item_class
        # Add the desc element
        desc_attrib = {'id': 'cdudn', 'nameSpace': 'urn:schemas-rinconnetworks-com:metadata-1-0/'}
        desc = XML.SubElement(item, '{urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/}desc', desc_attrib)
...
class MSTrack(MusicServiceItem):
    ...
    required_fields = ['title', 'item_id', 'extended_id', 'uri', 'description', 'service_id']
...
class MSArtist(MusicServiceItem):
    ...
    def __init__(self, title, item_id, extended_id, service_id, **kwargs):
        content = {'title': title, 'item_id': item_id, 'extended_id': extended_id, 'service_id': service_id}
        content.update(kwargs)
        super().__init__(**content)
...
class MSFavorites(MusicServiceItem):
    ...
    valid_fields = ['id', 'item_type', 'title', 'can_play', 'can_cache', 'album_art_uri']
