...
class EnOcean(SmartPlugin):
            ...
            if readin:
                ...
                while len(msg) >= 6:
                    ...
                    if msg[0] == PACKET.SYNC_BYTE and msg[5] == self.crc(msg[1:5]):
                        ...
                        if self.log_for_debug:
                            self.logger.debug(
                                f'Received header with data_length = {data_length} / opt_length = 0x{opt_length:02x} / type = {packet_type}'
                            )
                        ...
                        if self.crc(msg[6 : msg_length - 1]) == msg[msg_length - 1]:
                            if self.log_for_debug:
                                self.logger.debug(
                                    'Accepted package with type = 0x{:02x} / len = {} / data = [{}]!'.format(
                                        packet_type, msg_length, ', '.join(['0x%02x' % b for b in msg])
                                    )
                                )
                            ...
                            if packet_type == PACKET_TYPE.RADIO:
                                ...
                                self.logger.error(
                                    'Received packet with unknown type = 0x{:02x} - len = {} / data = [{}]'.format(
                                        packet_type, msg_length, ', '.join(['0x%02x' % b for b in msg])
                                    )
                                )
                        else:
                            self.logger.error(
                                'Crc error - dumping packet with type = 0x{:02x} / len = {} / data = [{}]!'.format(
                                    packet_type, msg_length, ', '.join(['0x%02x' % b for b in msg])
                                )
                            )
                        msg = msg[msg_length:]
            ...
            if found_rx_id and found_eep:
                ...
                if self.eep_parser.CanParse(rx_eep):
                    if rx_key in ['A0', 'A1', 'B0', 'B1']:
                        self.logger.warning(
                            f'Key "{rx_key}" does not match EEP - "0" (Zero, number) should be "O" (letter) (same for "1" and "I") - will be accepted for now'
                        )
    ...
    def _send_radio_packet(self, id_offset, code, data=[], optional=[]):
        ...
        self._send_packet(
            PACKET_TYPE.RADIO,
            [code] + data + list((self.tx_id + id_offset).to_bytes(4, byteorder='big')) + [0x00],
            optional,
        )
        self._response_lock.acquire()
    ...
    def _send_UTE_response(self, data, optional):
        ...
        self._send_radio_packet(
            self.learn_id,
            choice,
            [0x91, payload[1], payload[2], payload[3], payload[4], payload[5], payload[6]],
            [PACKET_TYPE.RADIO_SUB_TEL, data[-5], data[-4], data[-3], data[-2], db, Secu],
        )
        self.UTE_listen = False
    ...
    def _send_packet(self, packet_type, data=[], optional=[]):
        ...
        self.logger.info(
            'Sending packet with len = {} / data = [{}]!'.format(len(packet), ', '.join(['0x%02x' % b for b in packet]))
        )
    ...
    def _process_packet_type_radio(self, data, optional):
        ...
        repeater_cnt = status & 0x0F
        self.logger.info(
            'Radio message: choice = {:02x} / payload = [{}] / sender_id = {:08X} / status = {} / repeat = {}'.format(
                choice, ', '.join(['0x%02x' % b for b in payload]), sender_id, status, repeater_cnt
            )
        )
        ...
        if len(optional) == 7:
            ...
            if self.log_for_debug:
                self.logger.debug(
                    f'Radio message with additional info: subtelnum = {subtelnum} / dest_id = {dest_id:08X} / signal = {dBm} dBm / SecurityLevel = {SecurityLevel}'
                )
        ...
        if sender_id in self._rx_items:
            ...
            for eep, items in self._rx_items[sender_id].items():
                ...
                if eep.startswith('{:02X}'.format(choice)):
                    ...
                    if 'DEBUG' in results:
                        self.logger.warning(
                            'DEBUG Info: processing radio message with data = [{}] / optional = [{}]'.format(
                                ', '.join(['0x%02x' % b for b in data]), ', '.join(['0x%02x' % b for b in optional])
                            )
                        )
                            ...
                            if 'enocean_rocker_sequence' in item.conf:
                                try:
                                    if hasattr(item, '_enocean_rs_thread') and item._enocean_rs_thread.is_alive():
                                        ...
                                        item._enocean_rs_events = {
                                            'PRESSED': threading.Event(),
                                            'RELEASED': threading.Event(),
                                        }
                                        item._enocean_rs_thread = threading.Thread(
                                            target=self._rocker_sequence,
                                            name='enocean-rs',
                                            args=(item, sender_id, item.conf['enocean_rocker_sequence'].split(',')),
                                        )
            ...
            if self._log_unknown_msg:
                ...
                self.logger.warning(
                    'Unknown device sent radio message: choice = {:02x} / payload = [{}] / sender_id = {:08X} / status = {} / repeat = {}'.format(
                        choice, ', '.join(['0x%02x' % b for b in payload]), sender_id, status, repeater_cnt
                    )
                )
        ...
        # handle common commands
            ...
            if self._last_cmd_code == COMMON_COMMAND.WR_RESET and len(data) == 1:
                ...
                if data[0] == 0 and len(data) == 33:
                    self.logger.info(
                        'Chip ID = 0x{} / Chip Version = 0x{}'.format(
                            ''.join(['%02x' % b for b in data[9:13]]), ''.join(['%02x' % b for b in data[13:17]])
                        )
                    )
                    self.logger.info(
                        'APP version = {} / API version = {} / App description = {}'.format(
                            '.'.join(['%d' % b for b in data[1:5]]),
                            '.'.join(['%d' % b for b in data[5:9]]),
                            ''.join(['%c' % b for b in data[17:33]]),
                        )
                    )
                ...
                if data[0] == 0 and len(data) == 2:
                    self.logger.info(
                        'Number of taught in devices = 0x{}'.format(''.join(['%02x' % b for b in data[1]]))
                    )
                elif data[0] == 0 and len(data) == 0:
            ...
            self.logger.error(
                'Processing unexpected response with return code = {} / data = [{}] / optional = [{}]'.format(
                    RETURN_CODE(data[0]),
                    ', '.join(['0x%02x' % b for b in data]),
                    ', '.join(['0x%02x' % b for b in optional]),
                )
            )
    ...
    def send_learn_protocol(self, id_offset=0, device=10):
        ...
        if device == 10:
            ...
            self.logger.info(
                'Sending learn telegram for switch command with [Device], [ID-Offset], [RORG], [payload] / [{}], [{:#04x}], [{:#04x}], [{}]'.format(
                    device, id_offset, rorg, ', '.join('{:#04x}'.format(x) for x in payload)
                )
            )
        ...
        # device range 20 - 29 --> Learn protocol for dim actuators
            ...
            self.logger.info(
                'Sending learn telegram for dim command with [Device], [ID-Offset], [RORG], [payload] / [{}], [{:#04x}], [{:#04x}], [{}]'.format(
                    device, id_offset, rorg, ', '.join('{:#04x}'.format(x) for x in payload)
                )
            )
        elif device == 21:
            ...
            self.logger.info(
                'Sending learn telegram for rgbw dim command with [Device], [ID-Offset], [RORG], [payload] / [{}], [{:#04x}], [{:#04x}], [{}]'.format(
                    device, id_offset, rorg, ', '.join('{:#04x}'.format(x) for x in payload)
                )
            )
        ...
        # device range 30 - 39 --> Learn protocol for radiator valves
            ...
            self.logger.info(
                'Sending learn telegram for radiator valve with [Device], [ID-Offset], [RORG], [payload] / [{}], [{:#04x}], [{:#04x}], [{}]'.format(
                    device, id_offset, rorg, ', '.join('{:#04x}'.format(x) for x in payload)
                )
            )
        ...
        # device range 40 - 49 --> Learn protocol for other actuators
            ...
            self.logger.info(
                'Sending learn telegram for actuator with [Device], [ID-Offset], [RORG], [payload] / [{}], [{:#04x}], [{:#04x}], [{}]'.format(
                    device, id_offset, rorg, ', '.join('{:#04x}'.format(x) for x in payload)
                )
            )
        else:
