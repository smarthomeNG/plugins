...
import sys
sys.path.append('/usr/local/lib/python3.5/dist-packages/pysrc')
import pydevd
...
class AVDevice(SmartPlugin):
    ...
    def __init__(self,
        ...
        self._is_connected = []
        self._parsinginput = False
        self._verboselevel = verboselevel

        self.init = Init(self._sh, self._name, self._model, self._items, self._verboselevel)
        self._rs232, self._baud, self._timeout, self._writetimeout = self.init._process_variables(rs232, 'rs232')
        self._tcp, self._port, self._tcp_timeout = self.init._process_variables(tcp, 'tcp')
        self._dependson, self._dependson_value, self._depend0_power0, self._depend0_volume0 = self.init._process_variables([dependson, depend0_power0, depend0_volume0], 'dependson')
        ...
        self._ignore_response, self._error_response, self._force_buffer, self._ignoredisplay = self.init._process_variables([ignore_response, error_response, forcebuffer, inputignoredisplay], 'responses')

        self.logger.debug("Initializing {}: variables set. Model: {}, Manufacturer: {}".format(self._name, self._model, self._manufacturer))
        ...
        try:
            ...
            try:
                ...
                if self._verboselevel >= 2:
                    self.logger.warning("Resetting {}: Cannot find Sendingcommand. Using first Command in queue: {}. Message: {}".format(self._name, response, e))
                        ...
                        try:
                            if resp.startswith(key, 0, entry[1]):
                                ...
                                try:
                                    ...
                                    testvalue = ''
                                if self._verboselevel >= 2:
                                    self.logger.debug("Resetting {}: Comparing value {} with type {} and expected type {}.".format(
                                        self._name, entry, testvalue, entry[7]))
                                if testvalue == entry[7]:
                                    keyfound = entry
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Resetting {}: Found correct entry: {}".format(self._name, keyfound))
                                    break
                                else:
                                    keyfound = ''
                            else:
                            ...
                            keyfound = ''
                            if self._verboselevel >= 1:
                                self.logger.warning("Resetting {}: Problem comparing response: {}. Error: {}".format(self._name, resp, err))
                if not keyfound == '':
    ...
    def _convertvalue(self, receivedvalue, expectedtype, invert):
        if expectedtype == 'bool' and not receivedvalue == '':
            if self._manufacturer.lower() == 'epson':
                try:
                    receivedvalue = max(min(int(receivedvalue), 1), 0)
                    if self._verboselevel >= 2:
                        self.logger.debug("Storing Values {}: Limiting bool value for {}.".format(
            ...
            try:
                if invert is True:
                    if (int(receivedvalue) == 1 and len(receivedvalue) <= 1):
                        ...
                        receivedvalue = True
                    if self._verboselevel >= 2:
                        self.logger.debug("Parsing Input {}: Receivedvalue {} reversed".format(self._name, receivedvalue))
                else:
                    if (int(receivedvalue) == 1 and len(receivedvalue) <= 1):
                        receivedvalue = True
                        if self._verboselevel >= 2:
                            self.logger.debug("Parsing Input {}: Receivedvalue {} converted to bool.".format(self._name, receivedvalue))
                    elif (int(receivedvalue) == 0 and len(receivedvalue) <= 1):
                        receivedvalue = False
                        if self._verboselevel >= 2:
                            self.logger.debug("Parsing Input {}: Receivedvalue {} converted to bool.".format(self._name, receivedvalue))
    ...
    def _write_itemsdict(self, data):
        try:
            ...
            for command in sorted_response_commands:
                if self._verboselevel >= 2:
                    self.logger.debug("Storing Values {}: Comparing command {}.".format(self._name, command))
                for entry in self._response_commands[command]:
                    if self._verboselevel >= 2:
                        self.logger.debug("Storing Values {}: Comparing entry {}.".format(self._name, entry))
                    if entry[1] == entry[2]:
                        commandstart = 0
                        commandend = entry[2]
                    else:
                    ...
                    if data[commandstart:commandend] == command:
                        ...
                        if updated == 1:
                            if self._verboselevel >= 2:
                                self.logger.debug("Storing Values {}: Stored all relevant items from function {}. step 1".format(self._name, function))
                            break
                    if updated == 1:
                        if self._verboselevel >= 2:
                            self.logger.debug("Storing Values {}: Stored all relevant items from function {}. step 2".format(self._name, function))
                        break
                if updated == 1:
                    if self._verboselevel >= 2:
                        self.logger.debug("Storing Values {}: Stored all relevant items from function {}. step 3".format(self._name, function))
                    break
        except Exception as err:
            self.logger.error(
            ...
            if self._verboselevel >= 2:
                self.logger.debug("Storing Values {}: Finished.".format(self._name))
        ...
        try:
            ...
            while buffering:
                if '\r\n' in buffer:
                    ...
                    line = re.sub('[\\n\\r]', '', line).strip()
                    if not line.startswith(tuple(self._response_commands)) and not line.startswith(tuple(self._error_response)) and '' not in self._error_response:
                        if self._verboselevel >= 2:
                        ...
                        if self._verboselevel >= 2:
                            self.logger.debug("Processing Response {}: Detected Display info {}. buffer: \r\n{}".format(self._name, line, buffer))
                        buffering = False
                        buffer += '\r\n{}\r\n'.format(line)
                        buffer = tidy(buffer)
                        if self._verboselevel >= 2:
                            self.logger.debug("Processing Response {}: Append Display info {} to buffer: \r\n{}".format(self._name, line, buffer))
                    else:
                        if self._response_buffer is False and not buffer.startswith(tuple(self._force_buffer)) and '' not in self._force_buffer:
                            buffering = False
                            if self._verboselevel >= 2:
                                self.logger.debug("Processing Response {}: Clearing buffer: {}".format(self._name, buffer))
                            buffer = '\r\n'
                        if self._verboselevel >= 2:
                            self.logger.debug("Processing Response {}: Sending back line: {}. Display Command: {}".format(
            ...
            if not buffer == '\r\n' and self._response_buffer is True or type(self._response_buffer) is int:
                ...
                if len(bufferlist) > 1:
                    if self._verboselevel >= 2:
                        self.logger.debug("Bufferlist after: {}".format(bufferlist))
                buffering = False
                for buf in bufferlist:
                    if not re.sub('[ ]', '', buf) == '' and not buf.startswith(tuple(self._ignore_response)) and '' not in self._ignore_response:
                        if self._verboselevel >= 1:
                            self.logger.debug("Processing Response {}: Sending back {} from buffer because Responsebuffer is activated.".format(self._name, buf))
                        self._wait(0.2)
                        yield buf
    ...
    def run(self):
        if self._tcp is None and self._rs232 is None:
            ...
            if self._verboselevel >= 1:
                self.logger.debug("Initializing {}: Functions: {}, Number of Zones: {}".format(self._name, self._functions, self._number_of_zones))
                self.logger.debug("Initializing {}: Responsecommands: {}.".format(self._name, self._response_commands))
                self.logger.debug("Initializing {}: Special Commands: {}".format(self._name, self._special_commands))
                self.logger.debug("Initializing {}: Powercommands: {}".format(self._name, self._power_commands))
                self.logger.debug("Initializing {}: Querycommands: {}, Query Zone: {}".format(self._name, self._query_commands, self._query_zonecommands))
                ...
                self.logger.debug("Initializing {}: done".format(self._name))
    ...
    def connect(self, trigger):
        self._trigger_reconnect = True
        if self._verboselevel >= 2:
            self.logger.debug("Connecting {}: Starting to connect. Triggered by {}. Current Connections: {}".format(self._name, trigger, self._is_connected))
        try:
            ...
            if depending is False:
                if self._tcp is not None and 'TCP' not in self._is_connected:
                    if self._verboselevel >= 2:
                        self.logger.debug("Connecting {}: Starting TCP scheduler".format(self._name))
                ...
                if self._rs232 is not None and 'Serial' not in self._is_connected:
                    if self._verboselevel >= 2:
                        self.logger.debug("Connecting {}: Starting RS232 scheduler".format(self._name))
            ...
            if ('TCP' not in self._is_connected and self._tcp is not None) and str(self._auto_reconnect).lower() in ['1', 'yes', 'true', 'on']:
                ...
                for zeit in self._keep_commands:
                    ...
                    if time.time() - zeit <= self._secondstokeep and self._keep_commands[zeit] not in keeptemp:
                        ...
                        keeping = True
                    self.logger.debug("Connecting TCP {}: Age {}s of command {}. Secondstokeep: {}. Keeping command: {}".format(
        ...
        try:
            if self._rs232 is not None and 'Serial' not in self._is_connected:
                ...
                while ser.in_waiting == 0:
                    ...
                    if self._verboselevel >= 2:
                        self.logger.warning("Connecting Serial {}:  Buffer: {}. Reconnecting Retry: {}.".format(self._name, buffer, i))
                    if i >= 4:
                        ser.close()
                        if self._verboselevel >= 2:
                            self.logger.warning("Connecting Serial {}:  Ran through several retries.".format(self._name))
                        break
                if ser.isOpen():
            ...
            if 'Serial' in self._is_connected:
                ...
                if self._parsinginput is False:
                    ...
                    self.parse_input('serialconnect')
    ...
    def parse_input(self, trigger):
        self._parsinginput = True
        if self._verboselevel >= 1:
            self.logger.debug("Parsing Input {}: Received Parseinput Trigger from {}.".format(self._name, trigger))
        if trigger == 'tcpconnect' or trigger == 'serialconnect':
            try:
                ...
                try:
                    ...
                    if self._verboselevel >= 2:
                        self.logger.debug("Parsing Input {}: No statusupdate defined, not querying status after {}. Message: {}".format(self._name, trigger, err))
        ...
        while self.alive and not self._is_connected == []:
            if not self._sendingcommand == '' and not self._sendingcommand == 'done' and not self._sendingcommand == 'gaveup':
                if self._verboselevel >= 2:
                    self.logger.debug("Parsing Input {}: Starting to parse input. Alive: {}. Connected: {}. Sendcommand:{}".format(
            ...
            try:
                ...
                for data in databuffer:
                    ...
                    if data == '' and not self._sendingcommand == '' and not self._sendingcommand == 'done' and not self._sendingcommand == 'gaveup':
                        if self._verboselevel >= 2:
                            self.logger.warning("Parsing Input {}: Problem with empty response.".format(self._name))
                    ...
                    if not self._send_commands == []:
                        expectedresponse = []
                        if self._verboselevel >= 2:
                            self.logger.debug("Parsing Input {}: Parsing input while waiting for response.".format(self._name))
                        try:
                        ...
                        try:
                            ...
                            for expected in expectedresponse:
                                ...
                                try:
                                    for expectedpart in expectedlist:
                                        ...
                                        if data[:datalength].startswith(expectedpart) and len(data[:datalength]) == len(expectedpart):
                                            ...
                                            if self._verboselevel >= 2:
                                                self.logger.debug("Parsing Input {}: Expected response edited: {}.".format(self._name, found))
                                except Exception:
                                    found = expectedlist
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Parsing Input {}: Expected response after splitting: {}.".format(self._name, found))
                                if data.startswith(tuple(found)):
                                    ...
                                    self._resend_counter = 0
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Parsing Input {}: No response expected".format(self._name))
                                elif expectedlist[0].lower() == 'string':
                                    value = data
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Parsing Input {}: String found and testing... ".format(self._name))
                            ...
                            if not self._send_commands == [] and not self._sendingcommand == 'done':
                                ...
                                if self._requery_counter >= self._resend_retries:
                                    ...
                                    if self._send_commands[0] not in self._query_commands and not self._send_commands[0] in self._special_commands['Display']['Command']:
                                        ...
                                        self.logger.debug("Parsing Input {}: Giving up {}, because no answer received.".format(self._name, temp_sendcommand))
                                    self._send_commands.pop(0)
                                    if self._verboselevel >= 2:
                    ...
                    updated = 0
                    if self._verboselevel >= 2:
                        self.logger.debug("Parsing Input {}: Starting comparing values for data {}.".format(self._name, data, updated))
                    for key in sorted_response_commands:
                        if self._verboselevel >= 2:
                            self.logger.debug("Parsing Input {}: Starting comparing values for data {} with key: {}.".format(self._name, data, key, updated))
                        for entry in self._response_commands[key]:
                            ...
                            index = data.find(key)
                            if self._verboselevel >= 2:
                                self.logger.debug("Parsing Input {}: Entry: {}, Valuelength: {}, Expected Type: {}. ".format(
                            ...
                            if not index == -1:
                                ...
                                if data.startswith(self._special_commands['Display']['Command']) and not self._special_commands['Display']['Command'] == '':
                                    ...
                                    try:
                                        content = data[2:][:28]
                                        if self._verboselevel >= 2:
                                            self.logger.debug("AVDevice {}: Display Data {}. Item: {}".format(self._name, content, item))
                                        tempvalue = "".join(
                                    ...
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Parsing Input {}: Neither Display nor Now Playing in response. receivedvalue: {}.".format(
                                            self._name, receivedvalue))
                                ...
                                if sametype is False:
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Parsing Input {}: Receivedvalue {} does not match type {} - ignoring it.".format(self._name, receivedvalue, expectedtype))
                                else:
                                    if self._verboselevel >= 1:
                                        self.logger.debug("Parsing Input {}: Receivedvalue {} does match type {} - going on.".format(self._name, receivedvalue, expectedtype))
                                    if data.startswith(tuple(inputcommands)) and receivedvalue in self._ignoredisplay and '' not in self. _ignoredisplay:
                                        ...
                                        if self._verboselevel >= 3:
                                            self.logger.debug("Parsing Input {}: Data {} has NO value in ignoredisplay {}. Ignorecommands are now: {}. Display Ignore is {}. Input Ignore is {}".format(self._name, data, self._ignoredisplay, self._ignore_response, self._special_commands['Display']['Ignore'], self._special_commands['Input']['Ignore']))
                                        if self._special_commands['Display']['Ignore'] == 0 and 1 not in self._special_commands['Input']['Ignore']:
                                            ...
                                            if self._verboselevel >= 3:
                                                self.logger.warning("Parsing Input {}: Removing {} from ignore.".format(self._name, self._special_commands['Display']['Command']))
                                    value = receivedvalue
                                    if self._verboselevel >= 1:
                                    ...
                                    if function in self._items[zone].keys():
                                        ...
                                        if self._verboselevel >= 1:
                                            self.logger.debug("Parsing Input {}: Updated Item dict {}".format(self._name, self._items[zone][function]))
                                    ...
                                    if updated == 1:
                                        if self._verboselevel >= 2:
                                            self.logger.debug("Parsing Input {}: Updated all relevant items from item {}. step 1".format(self._name, item))
                                        break
                                if updated == 1:
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Parsing Input {}: Updated all relevant items from {}. step 2".format(self._name, item))
                                    break
                            ...
                            if updated == 1:
                                if self._verboselevel >= 2:
                                    self.logger.debug("Parsing Input {}: Updated all relevant items from {}. step 3".format(self._name, item))
                                break
                        if updated == 1:
                            if self._verboselevel >= 2:
                                self.logger.debug("Parsing Input {}: Updated all relevant items from {}. step 4".format(self._name, item))
                            break
                    if self._verboselevel >= 2:
                        self.logger.debug("Parsing Input {}: Finished. ".format(self._name))
            except Exception as err:
                self.logger.error(
                ...
                if not self._send_commands == [] and data == 'waiting':
                    if self._verboselevel >= 1:
                        self.logger.debug("Parsing Input {}: Waiting for response..".format(self._name))
                if not self._send_commands == [] and not data == 'waiting':
                    ...
                    if self._is_connected == []:
                        ...
                        if self._verboselevel >= 2:
                            self.logger.debug("Parsing Input {}: Sending again because list is not empty yet. Sending return is {}.".format(
                                self._name, sending))
        ...
        if self.alive:
            if caller != 'AVDevice':
                try:
                    emptycommand = False
                    if self._verboselevel >= 2:
                        self.logger.debug("Updating Item for avdevice_{}: Starting to update item {}. Caller: {}, Source: {}. Destination: {}. Reconnectrigger is {}".format(
                    ...
                    if item == self._dependson:
                        ...
                        if depending is False:
                            try:
                                ...
                                if self._verboselevel >= 2:
                                    self.logger.debug("Updating Item for avdevice_{}: Depend value is same as set up, statusupdate starting {}.".format(
                                ...
                                try:
                                    ...
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Updating Item for avdevice_{}: No statusupdate defined, not querying status after {}. Message: {}".format(self._name, caller, err))
                        ...
                        if letsgo is True:
                            ...
                            try:
                                ...
                                if self._functions['zone{}'.format(zone)][command][2] == '':
                                    emptycommand = True
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Updating Item for avdevice_{}: Function is empty. Sending nothing. Command: {} Item: {}".format(
                                    ...
                                    if command == 'statusupdate':
                                        ...
                                        if (checkvalue is True or caller == 'Init') and not self._special_commands['Display']['Ignore'] >= 5:
                                            ...
                                            self._trigger_reconnect = True
                                            if self._verboselevel >= 2:
                                                self.logger.debug("Updating Item for avdevice_{}: Updating status. Querycommands: {}. Reconnecttrigger: {}. Display Ignore: {}".format(self._name.lower(), self._send_commands, self._trigger_reconnect, self._special_commands['Display']['Ignore']))
                                    ...
                                    self._send_commands.append('{},{},{},{}'.format(
                                        commandinfo[2], commandinfo[3], commandinfo[4], commandinfo[8]))
                            ...
                            if updating is True:
                                ...
                                if command in self._functions['zone{}'.format(zone)]:
                                    ...
                                    try:
                                        ...
                                        replacedreverse = reversecommand.replace('*', reversevalue)
                                        appendcommand = '{},{},{},{}'.format(commandinfo[2], commandinfo[3], replacedcommand, commandinfo[8])
                                        reversecommand = '{},{},{},{}'.format(reverseinfo[2], reverseinfo[3], replacedreverse, commandinfo[8])
                                    except Exception as err:
                                    ...
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Updating Item for avdevice_{}: Appendcommand on: {}, Send Commands: {}".format(
                                    ...
                                    if appendcommand in self._send_commands:
                                        ...
                                        if self._verboselevel >= 2:
                                            self.logger.debug("Updating Item for avdevice_{}: New Commandlist {}.".format(
                                    ...
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Updating Item for avdevice_{}: Commandinfo: {}, {}, {}".format(
                                            self._name.lower(), commandinfo[2], commandinfo[3], commandinfo[4]))
                                    try:
                                        ...
                                        self.logger.debug("Updating Item for avdevice_{}: Problems replacing *: {}".format(
                                            self._name.lower(), err))
                                    if self._verboselevel >= 2:
                                        self.logger.debug("Updating Item for avdevice_{}: Appendcommand off: {}, Send Commands: {}".format(
                                            self._name.lower(), appendcommand, self._send_commands))
                                    if appendcommand in self._send_commands:
                                        if self._verboselevel >= 1:
                                    ...
                                    if isinstance(value, int) and (commandinfo[8] == 'int,float' or commandinfo[8] == 'int' or commandinfo[8] == 'float'):
                                        ...
                                        if commandinfo[2].count('*') > 1:
                                            ...
                                            if command.startswith('speakers'):
                                                ...
                                                if self._verboselevel >= 2:
                                                    self.logger.debug("Updating Item for avdevice_{}: Speaker {} current value is {}. Item: {} with value {}. Multiply: {}. Value: {}".format(self._name.lower(), self._items['zone{}'.format(zone)]['speakers']['Item'][0], currentvalue, item, item(), multiply, value))
                                                ...
                                                if value > 0:
                                                    ...
                                                    self._send_commands.insert(0, '{},{},{},{}'.format(
                                                        powercommands[2], powercommands[3], powercommands[4].replace('*', replacedvalue), commandinfo[8]))
                                            ...
                                            if self._verboselevel >= 2:
                                                self.logger.debug("Updating Item for avdevice_{}: Value has to be 1 digit. Value is {}".format(self._name.lower(), value))
                                ...
                                if not self._send_commands == [] and setting is True:
                                    ...
                                    for sendcommand in self._send_commands:
                                        if self._verboselevel >= 2:
                                            self.logger.debug("Updating Item for avdevice_{}: Testing send command: {}".format(self._name.lower(), sendcommand))
                                        if commandinfo[3] in sendcommand:
                                            ...
                                            if testvalue == commandinfo[8]:
                                                ...
                                                if self._verboselevel >= 2:
                                                    self.logger.debug("Updating Item for avdevice_{}: Command Set {} already in Commandlist {}. Value type: {}, expected type: {}. Replaced. Sendingcommand: {}".format(self._name.lower(), command, self._send_commands, type(value), commandinfo[8], self._sendingcommand))
                                                ...
                                                pass
                                                if self._verboselevel >= 3:
                                                    self.logger.debug("Updating Item for avdevice_{}: Command Set {} already in Commandlist {} but value is not same type. Continue...".format(
                    ...
                    if not self._send_commands == []:
                        ...
                        self._sendingcommand = self._send_commands[0]
                    if self._verboselevel >= 2:
                        self.logger.debug("Updating Item for avdevice_{}: Updating item. Command list is {}. Sendingcommand: {}. ".format(
                    ...
                    try:
                        if not self._is_connected == [] and not self._send_commands == []:
                            ...
                            if self._verboselevel >= 2:
                                self.logger.debug("Updating Item for avdevice_{}: Updating item. Command list is {}. Return from send is {}".format(
                                    self._name.lower(), self._send_commands, sending))

                        if self._reset_onerror is True and emptycommand is False and not self._send_commands == [] and not self._sendingcommand == 'done' and self._is_connected == []:  # and self._trigger_reconnect is True and updating is True and self._is_connected == []:
    ...
    def _send(self, command, caller):
        if self._verboselevel >= 2:
            self.logger.debug("Sending {}: Sending function called by: {}.".format(self._name, caller))
        try:
            if not self._send_commands == []:
                if command == 'command':
                ...
                if self._verboselevel >= 2:
                    self.logger.debug("Sending {}: Starting to send {} {} from list {}. Caller: {}.".format(self._name, command, to_send, self._send_commands, caller))
                try:
                    self._sendingcommand = self._send_commands[0]
                except Exception:
                ...
                if self._parsinginput is False:
                    if self._verboselevel >= 2:
                        self.logger.debug("Sending {}: Starting Parse Input. Expected response: {}".format(self._name, response))
                    self.parse_input('sending')
                ...
                if self._send_commands[0] in self._query_commands and len(self._send_commands) > 1 and keyfound is not True and self._special_commands['Display']['Ignore'] < 5:
                    ...
                    if self._special_commands['Display']['Command'] not in self._ignore_response and '' not in self._ignore_response and not self._special_commands['Display']['Command'] == '':
                        self._ignore_response.append(self._special_commands['Display']['Command'])
                    if self._verboselevel >= 3:
                        self.logger.debug("Sending {}: Querycommand. Command: {}. Querycommand: {}, Display Ignore: {}, Input Ignore: {}".format(
                    ...
                    if self._special_commands['Display']['Ignore'] >= 5:
                        self._special_commands['Display']['Ignore'] = self._special_commands['Display']['Ignore'] - 5
                    if self._verboselevel >= 3:
                        self.logger.debug("Sending {}: No Querycommand. Command: {}. Querycommand: {}. Display Ignore: {}. Input Ignore:{}".format(
                            self._name, self._send_commands[0], self._query_commands, self._special_commands['Display']['Ignore'], self._special_commands['Input']['Ignore']))
                    ...
                    if self._special_commands['Display']['Ignore'] == 0 and 1 not in self._special_commands['Input']['Ignore']:
                        while self._special_commands['Display']['Command'] in self._ignore_response:
                            self._ignore_response.remove(self._special_commands['Display']['Command'])
                        if self._verboselevel >= 2:
                            self.logger.debug("Sending {}: Removing {} from ignore. Ignored responses are now: {}".format(self._name, self._special_commands['Display']['Command'], self._ignore_response))
            ...
            if err.errno == 32:
                ...
                try:
                    ...
                    if self._verboselevel >= 1:
                        self.logger.debug("Sending {}: No TCP socket to close.".format(self._name))
                try:
                    self._is_connected.remove('TCP')
                    if self._verboselevel >= 2:
                ...
                try:
                    ...
                    if self._verboselevel >= 2:
                        self.logger.debug("Sending {}: No Serial socket to close.".format(self._name))
                try:
                    self._is_connected.remove('Serial')
                    if self._verboselevel >= 2:
    ...
    def stop(self):
        ...
        try:
            ...
            if self._verboselevel >= 2:
                self.logger.debug("Stopping {}: No TCP socket to close.".format(self._name))
        try:
            self._serialwrapper.close()
        except Exception:
            if self._verboselevel >= 2:
                self.logger.debug("Stopping {}: No Serial socket to close.".format(self._name))
