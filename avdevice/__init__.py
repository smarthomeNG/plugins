...
class AVDevice(SmartPlugin):
            ...
            if not self._parsinginput == [] and not self._is_connected == []:
                ...
                try:
                    ...
                    for data in databuffer:
                        ...
                        updated = 0
                        if not data == 'ERROR':
                            self.logger.log(VERBOSE1, "Parsing Input {}: Starting comparing values for data {}.".format(self._name, data))
                            for key in sorted_response_commands:
                    ...
                    if not self._send_commands == [] and not data == 'waiting':
                        ...
                        for command in self._send_commands:
                            if command in self._query_commands:
                                ...
                                self.logger.log(VERBOSE1, "Parsing Input {}: Ordering power command {} to first position.".format(
                                    self._name, command))
                ...
                try:
                    ...
                    if item == self._dependson:
                        ...
                        if depending is False:
                            ...
                            self.logger.log(VERBOSE1, "Updating Item for avdevice_{}: Depend value is false.".format(self._name.lower()))
    ...
    def _send(self, command, caller):
        self.logger.log(VERBOSE1, "Sending {}: Sending function called by: {}.".format(self._name, caller))
        try:
            if not self._send_commands == []:
                if command == 'command':
