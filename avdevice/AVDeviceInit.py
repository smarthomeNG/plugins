							...
							for command in self._functions[dependzone]:
								...
								if self._functions[dependzone][command][1] == dependingfunction:
									for entrylist in self._items[dependzone][dependingfunction]['Master']:
										...
										dependvalue = entrylist['Dependvalue']
										compare = entrylist['Compare']
										commandlist = '{},{},{}'.format(querycommand, querycommand, responsecommand)
										toadd = {'Item': entrylist['Item'], 'Dependvalue': dependvalue, 'Compare': compare, 'Zone': entrylist['Zone'], 'Function': entrylist['Function']}
			...
			for entry in dependson_list[zone]:
				...
				for entrylist in dependson_list[zone][entry]:
					...
					try:
						...
						problems[zone].append(entry)
						self.logger.error("Initializing {}: Item with function {} for dependency does not exist. Error: {}".format(self._name, sub, err))
					count += 1
					...
					try:
						...
						for response in response_to_split:
							...
							if response.find('?{str}') >= 0:
								...
								# commandlength = len(re.sub('[*]', '', response))
