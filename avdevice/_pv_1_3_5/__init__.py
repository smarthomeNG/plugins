...
class AVDevice(SmartPlugin):
    ...
    def __init__(self, smarthome,
        ...
        self._rs232, self._baud, self._timeout = ProcessVariables([rs232_port, rs232_baudrate, rs232_timeout],
                                                                  self._name).rs232()
        self._tcp, self._port, self._tcp_timeout = ProcessVariables([tcp_ip, tcp_port, tcp_timeout], self._name).tcp()
        self._dependson, self._dependson_value, self._depend0_power0, self._depend0_volume0 = ProcessVariables(
            [dependson_item, dependson_value, depend0_power0, depend0_volume0], self._name).dependson()

        self._response_buffer = ProcessVariables(responsebuffer, self._name).responsebuffer()
        self._reset_onerror = ProcessVariables(resetonerror, self._name).resetonerror()
        self._ignore_response, self._error_response, self._force_buffer, self._ignoredisplay = ProcessVariables(
            [ignoreresponse, errorresponse, forcebuffer, inputignoredisplay], self._name).responses()
        self.logger.debug(
        ...
        self._update_exclude = ProcessVariables(update_exclude, self._name).update_exclude()
    ...
    def _translate(self, code, dictentry, caller):
        ...
        try:
            ...
            result = None
        self.logger.log(VERBOSE2,
                        "Translating {}: Called by: {}. Dictentry: {}, Code: {}, Result: {}.".format(self._name,
                                                                                                     origcaller,
                                                                                                     dictentry, code,
                                                                                                     result))
        return result
            ...
            try:
                for zone in self._items:
                    if 'power' in self._items[zone].keys() and self._depend0_power0 is True:
                        ...
                        self.logger.log(VERBOSE1, 'Resetting {}: Power to 0 for item {}'.format(self._name,
                                                                                                self._items[zone][
                                                                                                    'power']['Item']))
                    if 'speakers' in self._items[zone].keys() and self._depend0_power0 is True:
                        ...
                        for itemlist in self._items_speakers[zone].keys():
                            ...
                            speaker = self._items_speakers[zone][itemlist]['Item']
                            speaker(0, 'AVDevice', self._tcp)
                            self.logger.log(VERBOSE1, 'Resetting {}: Speakers to 0 for item {}'.format(self._name,
                                                                                                       speaker))
                        speaker = self._items[zone]['speakers']['Item']
                        speaker(0, 'AVDevice', self._tcp)
                        self.logger.log(VERBOSE1,
                                        'Resetting {}: Speakers to 0 for item {}'.format(self._name, speaker))
                    if 'volume' in self._items[zone].keys() and self._depend0_volume0 is True:
                        ...
                        self.logger.log(VERBOSE1, 'Resetting {}: Volume to 0 for item {}'.format(self._name,
                                                                                                 self._items[zone][
                                                                                                     'volume']['Item']))
    ...
    def _convertvalue(self, receivedvalue, expectedtype, invert, valuelength, command):
        ...
        if command in self._special_commands['Display']['Command']:
            try:
                ...
                try:
                    ...
                    if line.isdigit():
                        line = int(line)
                        receivedvalue = receivedvalue[4:] if line == 0 else receivedvalue[
                                                                            5:] if line == 1 else receivedvalue[6:]
                        if receivedvalue:
                            # Now playing
                            if line == 1:
                                self.logger.info(
                                    "Converting Values {}: Now playing {}".format(self._name, receivedvalue))
                            ...
                            # Internet radio Station name
                                ...
                                for singleitem in self._special_commands['Input']['Item']:
                                    if singleitem() == 'IRADIO':
                                        self.logger.info(
                                            "Converting Values {}: Internet radio station {}".format(self._name,
                                                                                                     receivedvalue))
                                        self._items['zone0']['station'](receivedvalue, 'AVDevice', self._tcp)
                except Exception as err:
                    self.logger.debug(
                        "Converting Values {}: Unknown display info for Denon received. Message: {}".format(self._name,
                                                                                                            err))
            ...
            try:
                if invert is True:
                    if (int(receivedvalue) == 1 and len(str(receivedvalue)) <= 1) and valuelength == 1:
                        ...
                        receivedvalue = True
                    self.logger.log(VERBOSE1,
                                    "Converting Values {}: Receivedvalue {} reversed".format(self._name, receivedvalue))
                else:
                    if (int(receivedvalue) == 1 and len(str(receivedvalue)) <= 1) and valuelength == 1:
                        receivedvalue = True
                        self.logger.log(VERBOSE1,
                                        "Converting Values {}: Receivedvalue {} converted to bool.".format(self._name,
                                                                                                           receivedvalue))
                    elif (int(receivedvalue) == 0 and len(str(receivedvalue)) <= 1) and valuelength == 1:
                        receivedvalue = False
                        self.logger.log(VERBOSE1,
                ...
                self.logger.log(VERBOSE2,
                                "Converting Values {}: Receivedvalue {} not converted further. Message: {}.".format(
                                    self._name, receivedvalue, err))
                pass
            try:
                if int(receivedvalue) == 1 and len(str(receivedvalue)) <= 1 and valuelength == 1:
                    receivedvalue = True
        ...
        if 'int' in expectedtype:
            try:
                if str(receivedvalue).lower() == 'on' and ('bool' in expectedtype or valuelength == 1):
                    receivedvalue = 1
                elif (str(receivedvalue).lower() == 'off' or str(receivedvalue).lower() == 'standby') and (
                        'bool' in expectedtype or valuelength == 1):
                    receivedvalue = 0
            except Exception:
                pass
            try:
                ...
                pass
        elif not (('bool' in expectedtype and 'int' in expectedtype) or (
                'str' in expectedtype and 'bool' in expectedtype)):
            try:
                if (str(receivedvalue).lower() == 'on' and (valuelength == 100 or valuelength == 2)) or (
                        str(receivedvalue).lower() == 'open' and (valuelength == 100 or valuelength == 4)):
                    receivedvalue = True
                elif (str(receivedvalue).lower() == 'off' and (valuelength == 100 or valuelength == 3)) or (
                        str(receivedvalue).lower() == 'standby' and (valuelength == 100 or valuelength == 7)) or (
                        str(receivedvalue).lower() == 'close' and (valuelength == 100 or valuelength == 5)) or (
                        str(receivedvalue).lower() == 'clos' and (valuelength == 100 or valuelength == 4)):
                    receivedvalue = False
            except Exception:
                pass
        try:
        ...
        if not expectedtype == 'str':
            try:
                ...
                pass
        self.logger.debug(
            "Converting Values {}: Received Value is now: {} with type {}.".format(self._name, receivedvalue,
                                                                                   type(receivedvalue)))
        return receivedvalue
    ...
    def _write_itemsdict(self, data, found):
        ...
        try:
            ...
            for command in sorted_response_commands:
                ...
                for entry in self._response_commands[self._response_wildcards['wildcard'][command]]:
                    ...
                    if data[commandstart:commandend] == command:
                        ...
                        if sametype is True:
                            self._items[zone][av_function]['Value'] = self._translate(value, entry[9], 'writedict')
                            self.logger.debug(
                                "Storing Values {}: Found writeable dict key: {}. Zone: {}. Value {} with type {}. Function: {}.".format(
                                    self._name, command, zone, receivedvalue, expectedtype, av_function))
                            updated = 1
                            break
                        else:
                            self.logger.debug(
                                "Storing Values {}: Found writeable dict key: {} with type {}, but received value {} is type {}. Not writing value!".format(
            ...
            self.logger.log(VERBOSE1,
                            "Storing Values {}: Finished. Send Commands: {}. Returning: {}, {}, {}".format(self._name,
                                                                                                           self._send_commands,
                                                                                                           self._items[
                                                                                                               zone][
                                                                                                               av_function],
    ...
    def _parse_depend_item(self, item, info, zone):
        for dependzone in range(0, 5):
            ...
            if self.has_iattr(item.conf, 'avdevice_{}_depend'.format(dependzone)) or (
                    self.has_iattr(item.conf, 'avdevice_depend') and dependzone == 'zone0'):
        ...
        if command.find('?') >= 1:
            ...
            self.logger.log(VERBOSE2,
                            "Processing Wildcard {}: Command: {} (original: {}), Splitcommand: {}. Splitreal: {}. Data: {}".format(
                                self._name, command, unprocessed, splitcommand, splitreal, data))
            ...
            if wildcard_replace:
                self.logger.log(VERBOSE2,
                                "Processing Wildcard {}: Wildcard replace: {}, Wildcard: {}.".format(self._name,
                                                                                                     wildcard_replace,
                                                                                                     wildcard))
                newstring = ''
                for i in range(0, len(splitcommand)):
                    try:
                        ...
                        replace = True if (
                                    (len(wildcard_replace[i]) == len(wildcard[i]) or '{str}' in splitreal[i]) and not
                                    wildcard[i] == '') else False
                    except Exception as err:
                        replace = False
                    try:
                        if replace is True:
        ...
        try:
            for resp in self._send_commands:
                ...
                for i in range(0, len(splitresponse)):
                    ...
                    self.logger.log(VERBOSE2,
                                    "Processing Response {}: Splitresponse after wildcard {}: {}.".format(self._name, i,
                                                                                                          splitresponse[
                                                                                                              i]))
                wildcardresponse = []
        ...
        try:
            ...
            try:
                ...
                if not buffer == '' and (self._response_buffer is not False or self._response_buffer is not 0):
                    ...
                    self.logger.log(VERBOSE1,
                                    "Processing Response {}: Received empty response while sending command: {}. Return from send is {}. Retry: {}".format(
                ...
                try:
                    if not self._sendingcommand == 'done' and not self._sendingcommand == 'gaveup' and not (
                            self._sendingcommand.split(',')[2] == '' or self._sendingcommand.split(',')[2] == ' ' or
                            self._sendingcommand.split(',')[2] == 'none'):
                        buffering = True
                        self.logger.log(VERBOSE1,
                                        "Processing Response {}: Error reading.. Error: {}. Sending Command: {}. RS232: {}, Host: {}, Socket: {}".format(
                                            self._name, err, self._sendingcommand, self._rs232, self._tcp, socket))
                        if self._rs232 and (socket == self._serialwrapper or socket == self._serial):
                            self.logger.log(VERBOSE1,
                                            "Processing Response {}: Problems buffering RS232 response. Error: {}. Increasing timeout temporarily.".format(
                                                self._name, err))
                            self._wait(1)
                            socket.timeout = 2
                        ...
                        if self._tcp and socket == self._tcpsocket:
                            self.logger.log(VERBOSE1,
                                            "Processing Response {}: Problems buffering TCP response. Error: {}. Increasing timeout temporarily.".format(
                                                self._name, err))
                            self._wait(1)
            ...
            while buffering:
                if '\r\n' in buffer:
                    self.logger.log(VERBOSE2,
                                    "Processing Response {}: Buffer before removing duplicates: {}".format(self._name,
                                                                                                           re.sub(
                                                                                                               '[\r\n]',
                                                                                                               ' --- ',
                                                                                                               buffer)))
                    ...
                    if not self._send_commands == []:
                                ...
                                if entry == e:
                                    ...
                                    try:
                                        for resp in self._response_commands[realresponse]:
                                            ...
                                            if (len(entry) == resp[1] or resp[1] == 100 or resp[0] == 100) and entry not in sortedbuffer:
                                                self.logger.log(VERBOSE2,
                        ...
                        buffer = "{}\r\n".format(buffer)
                        self.logger.log(VERBOSE2, "Processing Response {}: Buffer after sorting: {}.".format(self._name,
                                                                                                             re.sub(
                                                                                                                 '[\r\n]',
                                                                                                                 ' --- ',
                                                                                                                 buffer)))
                    ...
                    if not ('' in self._force_buffer and len(self._force_buffer) == 1) and (
                            self._response_buffer is False or self._response_buffer == 0):
                        if not re.sub('[ ]', '', buffer) == '' and not re.sub('[ ]', '', line) == '':
                            ...
                            if bufferlist:
                                ...
                                self.logger.log(VERBOSE2,
                                                "Processing Response {}: No forced buffer found.".format(self._name))
                    ...
                    try:
                        for resp in ','.join(self._sendingcommand.split(';')[0].split(',')[2:]).split('|'):
                            ...
                            if len(line) == len(resp):
                                ...
                                self.logger.log(VERBOSE2,
                                                "Processing Response {}: Comparing line {} to expected response from send {}".format(
                                                    self._name, line, resp))
                                if line == resp:
                                    responseforsending = True
                                    self.logger.log(VERBOSE2,
                                                    "Processing Response {}: Line and expected response from send are the same".format(
                                                        self._name))
                    except Exception as err:
                        self.logger.log(VERBOSE2,
                    ...
                    if not line.startswith(tuple(
                            responsecommands)) and line not in self._error_response and responseforsending is False:
                        self.logger.log(VERBOSE1,
                        ...
                        try:
                            ...
                            if keyfound is True:
                                self.logger.log(VERBOSE1,
                                                "Processing Response {}: Sending Command: {} Keep command {}".format(
                                ...
                                for entry in self._keep_commands:
                                    if self._send_commands[0] in self._keep_commands.get(entry):
                                        ...
                                        self.logger.log(VERBOSE1,
                                                        "Processing Response {}: Removed Keep command {} from {} because command sent successfully".format(
                                                            self._name, entry, self._keep_commands))
                                        break
                                self._send_commands.pop(0)
                                self._sendingcommand = 'done'
                                self.logger.debug(
                                    "Processing Response {}: Response {} is same as expected {} and defined as response to be ignored. Removing command from send list. It is now: {}. Ignore responses are: {}".format(
                                        self._name, line, compare, self._send_commands, self._ignore_response))
                                sending = self._send('command', 'commandremoval')
                            else:
                                self.logger.log(VERBOSE1,
                                                "Processing Response {}: Response {} is in ignore command {} but not same as expected response {}".format(
                                                    self._name, line, self._ignore_response, compare))
                        except Exception as err:
                            self.logger.log(VERBOSE2,
                                            "Processing Response {}: Response {} is ignored because ignore responses is {}. Command list is now: {}. Message: {}".format(
                            ...
                            self._response_buffer is not False and \
                            not self._special_commands['Display']['Command'] == '':
                        self.logger.log(VERBOSE1, "Processing Response {}: Detected Display info {}. buffer: {}".format(
                        ...
                        if self._response_buffer is False and not buffer.startswith(
                            ...
                            buffering = False
                            self.logger.log(VERBOSE1, "Processing Response {}: Clearing buffer: {}".format(self._name,
                                                                                                           re.sub(
                                                                                                               '[\r\n]',
                                                                                                               ' --- ',
                                                                                                               buffer)))
                            buffer = '\r\n'
                        self.logger.log(VERBOSE1,
                                        "Processing Response {}: Sending back line: {}. Display Command: {}".format(
            ...
            if not buffer == '\r\n' and (self._response_buffer is True or type(self._response_buffer) is int):
                ...
                for buf in bufferlist:
                    if not buf.startswith(
                            tuple(self._ignore_response)) and '' not in self._ignore_response and buf.startswith(
                            tuple(self._response_commands)):
                ...
                for buf in bufferlist:
                    if not re.sub('[ ]', '', buf) == '' and not buf.startswith(
                            tuple(self._ignore_response)) and '' not in self._ignore_response:
                        self.logger.log(VERBOSE1,
                                        "Processing Response {}: Sending back {} from buffer because Responsebuffer is activated.".format(
                                            self._name, buf))
                        self._wait(0.2)
                        yield buf
                ...
                for buf in bufferlist:
                    if not re.sub('[ ]', '', buf) == '' and not buf.startswith(
                            tuple(self._ignore_response)) and '' not in self._ignore_response:
                        self.logger.debug(
                            "Processing Response {}: Sending back {} from filtered buffer: {}.".format(self._name, buf,
                                                                                                       re.sub('[\r\n]',
                                                                                                              ' --- ',
                                                                                                              buffer)))
                        self._wait(0.2)
            ...
            self.logger.error("Processing Response {}: Problems: {} in line {}.".format(self._name, err,
    ...
    def connect_tcp(self):
        try:
            ...
            if ('TCP' not in self._is_connected and self._tcp is not None) and \
                    str(self._auto_reconnect).lower() in ['1', 'yes', 'true', 'on']:
                self._trigger_reconnect = False
                self.logger.warning("Connecting TCP {}: Reconnecting. Command list while connecting: {}.".format(
                    self._name, self._send_commands))
            elif (
                    'TCP' in self._is_connected and self._tcp is not None) or self._reconnect_counter >= self._reconnect_retries:
                self._sh.scheduler.change('avdevice-tcp-reconnect', active=False)
                self._reconnect_counter = 0
                self._addfrom_keepcommands('connect_tcp', 'all')
                self._trigger_reconnect = True
                self.logger.debug(
                    "Connecting TCP {}: Deactivating reconnect scheduler. Command list while connecting: {}. Keep Commands: {}".format(
                        self._name, self._send_commands, self._keep_commands))
        ...
        try:
            if self._rs232 is not None and 'Serial' not in self._is_connected:
                ...
                try:
                    ...
                    self.logger.debug(
                        "Connecting Serial {}: Starting to connect to {} with init command {}.".format(self._name,
                                                                                                       self._rs232,
                                                                                                       command))
                except Exception:
                    self.logger.warning(
                        "Connecting Serial {}: No power commands found. Please check your config files!".format(
                            self._name))
                    command = '?P'
                while ser.in_waiting == 0:
                    ...
                    self.logger.log(VERBOSE1,
                                    "Connecting Serial {}:  Buffer: {}. Reconnecting Retry: {}.".format(self._name,
                                                                                                        re.sub('[\r\n]',
                                                                                                               ' --- ',
                                                                                                               buffer),
                                                                                                        i))
            ...
            if ('Serial' not in self._is_connected and self._rs232 is not None) and str(
                    self._auto_reconnect).lower() in ['1', 'yes', 'true', 'on']:
                self._trigger_reconnect = False
                self.logger.log(VERBOSE1,
                                "Connecting Serial {}: Activating reconnect scheduler. Command list while connecting: {}.".format(
                                    self._name, self._send_commands))
            elif (
                    'Serial' in self._is_connected and self._rs232 is not None) or self._reconnect_counter >= self._reconnect_retries:
                self._sh.scheduler.change('avdevice-serial-reconnect', active=False)
                self._reconnect_counter = 0
                self._addfrom_keepcommands('connect_serial', 'all')
                self._trigger_reconnect = True
                self.logger.debug(
                    "Connecting Serial {}: Deactivating reconnect scheduler. Command list while connecting: {}. Keep commands: {}".format(
                        self._name, self._send_commands, self._keep_commands))
            self._reconnect_counter += 1
            if 'Serial' in self._is_connected:
    ...
    def _checkdependency(self, dep_function, dep_type):
        ...
        if dep_type == 'statusupdate' or dep_type == 'initupdate' or dep_type == 'checkquery' or (
                dep_type == 'update' and not dep_function == ''):
            ...
            if dep_type == 'statusupdate' or dep_type == 'initupdate':
                ...
                totest = self._dependencies['Master_dep_function']
                queryzone = orig_function.split(', ')[0]
                dep_function = orig_function.split(', ')[1]

            for zone in totest:
                if (dep_function in totest[zone] and not dep_type == 'checkquery') or \
                        (dep_type == 'checkquery' and zone == queryzone and dep_function in totest[zone]):
                    donedependitems = []
                    ...
                    for entry in totest[zone][dep_function]:
                        ...
                        try:
                            ...
                            depending = False
                            self.logger.warning(
                                "Checking Dependency {}: Adding dep_function {} caused problem: {}.".format(self._name,
                                                                                                            dep_function,
                                                                                                            err))
                        ...
                        if dep_type == 'checkquery' and dependitem not in donedependitems:
                            ...
                            try:
                                ...
                                depending = False
                                self.logger.warning(
                                    "Checking Dependency {}: Adding dep_function {} in zone {} caused problem: {}.".format(
                            ...
                            try:
                                ...
                                self.logger.log(VERBOSE2,
                                                "Checking Dependency {}: Dependent dep_functions found for {}. But no Query command. Message: {}".format(
                            ...
                            if dependtotal == comparetotal:
                                if primarycount > 0 and queryentry is not None:
                                    if queryentry not in self._send_commands:
                                        ...
                                        self.logger.debug(
                                            "Checking Dependency {}: Dependent Query command {} added to Send Commands. It is now: {}. Dependencies: {}".format(
                                    ...
                                    self.logger.log(VERBOSE2,
                                                    "Checking Dependency {}: Primary dependency not fullfilled, not adding or removing query  {}".format(
                                                        self._name, queryentry))
                            elif primarycount == 0 and queryentry is not None:
                                try:
                                    self._send_commands.remove(queryentry)
                                    self._clearbuffer = True
                                    self.logger.debug(
                                        "Checking Dependency {}: Dependent Query command {} removed from Send Commands. It is now: {}. Dependencies: {}".format(
                                            self._name, queryentry, self._send_commands, dependitems))
                                except Exception:
                                    self.logger.log(VERBOSE2,
                                                    "Checking Dependency {}: Dependent Query command {} not in Send Commands, not removing it. Dependencies: {}".format(
                    ...
                    if not dep_type == 'checkquery':
                        self.logger.log(VERBOSE2,
                                        "Checking Dependency {}: Groupcount: {}, Grouptotal: {}.".format(self._name,
                                                                                                         groupcount,
                                                                                                         grouptotal))
                        comparetotal = 0
                        dependtotal = 0
                    ...
                    if dependtotal == comparetotal and not dep_type == 'checkquery':
                        depending = False
                        self.logger.log(VERBOSE1,
                                        "Checking Dependency {}: Adding dep_function {} because dependency fullfilled: {}.".format(
                                            self._name, dep_function, dependitems))
                    elif not dep_type == 'checkquery':
                        depending = True
                        self.logger.debug(
                            "Checking Dependency {}: Not adding dep_function {} because dependency not fullfilled: {}".format(
                                self._name, dep_function, dependitems))
            ...
            try:
                ...
                if dependsvalue == self._dependson_value:
                    ...
                    if dep_type == 'dependitem':
                        try:
                            eval(self._items['zone0']['statusupdate']['Item'])(1, 'Depending',
                                                                               self._rs232 if self._rs232 is not None else self._tcp)
                        except Exception:
                            try:
                    ...
                    if dep_type == 'connect':
                        self._is_connected = []
                        self._parsinginput = []
                    if (dep_type == 'parseinput' or dep_type == 'dependitem') and (
                            self._depend0_volume0 is True or self._depend0_power0 is True):
    ...
    def _addfrom_keepcommands(self, trigger, zone):
        self.logger.log(VERBOSE1,
                        "Keep Commands {}: Trigger from {} for zone {}. Send Commands: {}".format(self._name, trigger,
                                                                                                  zone,
                                                                                                  self._send_commands))
        keeptemp = []
    ...
    def _parse_input_init(self, trigger):
        if not self._is_connected == [] and not self._is_connected == ['Connecting']:
            ...
            self._parsinginput = []
        self.logger.log(VERBOSE1, "Parsing Input {}: Init Triggerd by these functions so far: {}".format(self._name,
                                                                                                         self._parsinginput))
    ...
    def _parse_input(self, trigger):
        ...
        while self.alive and not self._parsinginput == [] and not \
                self._is_connected == [] and not self._is_connected == ['Connecting']:
            ...
            if not self._sendingcommand == '' and not self._sendingcommand == 'done' and not self._sendingcommand == 'gaveup':
                self.logger.log(VERBOSE1,
                                "Parsing Input {}: Starting to parse input. Alive: {}. Connected: {}. Parsinginput: {}. Sendcommand: {}".format(
                                    self._name, self.alive, self._is_connected, self._parsinginput,
            ...
            try:
                ...
                for data in databuffer:
                    ...
                    if data == 'ERROR' and not self._sendingcommand == 'gaveup' and not self._sendingcommand == 'done':
                        if self._resend_counter >= self._resend_retries:
                            self.logger.warning(
                                "Parsing Input {}: Giving up Sending {} and removing from list. Received Data: {}. Original Commandlist: {}".format(
                                    self._name, self._sendingcommand, data, self._send_commands))
                            self._resend_counter = 0
                            self.logger.log(VERBOSE1,
                                            "Parsing Input {}: Resetting Resend Counter because maximum retries exceeded.".format(
                                                self._name))
                            # Resetting Item if Send not successful
                            if self._reset_onerror is True:
                                self._resetitem('')
                            try:
                                if self._send_commands[0] not in self._query_commands and not self._send_commands[0] in \
                                                                                              self._special_commands[
                                                                                                  'Display']['Command']:
                                    self._keep_commands[time.time()] = self._send_commands[0]
                                    self.logger.debug(
                                        "Parsing Input {}: Removing item from send command, storing in keep commands: {}.".format(
                                            self._name, self._keep_commands))
                                elif self._send_commands[0] in self._query_commands:
                                    self.logger.warning(
                                        "Parsing Input {}: Giving up {}, because no answer received 1.".format(
                                            self._name, self._sendingcommand))
                                self._send_commands.pop(0)
                                if not self._send_commands == []:
                                ...
                                self.logger.debug(
                                    "Parsing Input {}: Nothing to remove from Send Command List. Error: {}".format(
                                        self._name, err))
                            self._sendingcommand = 'gaveup'
                            if self._trigger_reconnect is True:
                                self.logger.log(VERBOSE1,
                                                "Parsing Input {}: Trying to connect while parsing item".format(
                                                    self._name))
                    ...
                    for i in range(0, len(sorted_response_commands)):
                        try:
                            sorted_response_commands[i] = self._response_wildcards['original'][
                                sorted_response_commands[i]]
                        except Exception as err:
                            sorted_response_commands[i] = None
                            self.logger.log(VERBOSE2,
                    ...
                    self.logger.log(VERBOSE2,
                                    "Parsing Input {}: New Response Command list after sorting: {}".format(self._name,
                                                                                                           sorted_response_commands))

                    self.logger.debug("Parsing Input {}: Response: {}. Send Commands: {}".format(self._name, data,
                                                                                                 self._send_commands))
                    updated = 0
                    if (data == 'ERROR' and self._send_commands == []) or data in self._error_response:
                        ...
                        if self._resend_counter >= self._resend_retries:
                            self._resend_counter = 0
                            self.logger.debug(
                                "Parsing Input {}: Gave up sending {} because no response received. Is the command WRITEONLY?".format(
                                    self._name, self._sendingcommand))
                            self._sendingcommand = 'gaveup'
                    elif not self._send_commands == []:
                        expectedresponse = self._expected_response
                        self.logger.debug("Parsing Input {}: Expected response while parsing: {}.".format(self._name,
                                                                                                          expectedresponse))
                        ...
                        try:
                                        ...
                                        try:
                                            for expectedpart in expectedlist:
                                                try:
                                                    ...
                                                    self.logger.log(VERBOSE2,
                                                                    "Parsing Input {}: Comparing Data {} (cut: {}) to: {}, expectedlength: {}, datalength: {}, string: {}.".format(
                                                                        self._name, data, data[:datalength],
                                                                        expectedpart, expectedlength, len(data),
                                                                        stringvalue))
                                                    if data[:datalength].startswith(expectedpart) and (
                                                            len(data) in expectedlength or True in stringvalue):
                                            ...
                                            self.logger.debug(
                                                "Parsing Input {}: Expected response after exception: {}. Problem: {}".format(
                                                    self._name, found, err))
                                        self.logger.log(VERBOSE1,
                                                        "Parsing Input {}: Found: {}.".format(self._name, found))
                                        try:
                                            if data.startswith(tuple(found)):
                                                self.logger.log(VERBOSE1,
                                                                "Parsing Input {}: Expected response edited {}. Data starts with one of the entries.".format(
                                                                    self._name, found))
                                                entry, value, valuetype = self._write_itemsdict(data, found)
                                                self._sendingcommand = 'done'
                                                self._resend_counter = 0
                                                self.logger.log(VERBOSE1,
                                                                "Parsing Input {}: Resetting Resend Counter because data is same as expected.".format(
                                                                    self._name))
                                                self.logger.log(VERBOSE1,
                                                                "Parsing Input {}: Data {} found in {}. Entry: {}, Value: {}. Expected Type: {}".format(
                                                                    self._name, data, found, entry, value, valuetype))
                                                ...
                                                value = data
                                                self.logger.log(VERBOSE1,
                                                                "Parsing Input {}: String found and testing... ".format(
                                                                    self._name))
                                                if value.startswith(tuple(self._response_commands.keys())):
                                                    self.logger.log(
                                                        VERBOSE1,
                                                        "Parsing Input {}: Found string but ignored because it is a legit response for another command.".format(
                                                            self._name))
                                                else:
                                                    entry, value, valuetype = self._write_itemsdict(data, found)
                                                    self.logger.debug(
                                                        "Parsing Input {}: String FOUND. Written to dict: {}. Resend Counter reset.".format(
                                                            self._name, entry))
                                                    self._sendingcommand = 'done'
                                                    self._resend_counter = 0
                                            ...
                                            for expectedindex in expectedindices:
                                                ...
                                                if self._send_commands[expectedindex] not in deletecommands:
                                                    ...
                                                    if not valuetype == testvalue or not found or data == 'ERROR':
                                                        self.logger.log(VERBOSE2,
                                                                        "Parsing Input {}: Test Value {} of {} is not same as Valuetype: {} or nothing found {}. Keeping in Sendcommands.".format(
                                            ...
                                            self.logger.log(VERBOSE1,
                                                            "Parsing Input {}: Write to dict problems: {}".format(
                                                                self._name, err))

                                self.logger.log(VERBOSE2,
                                                "Parsing Input {}: Deleting {} from sendcommands.".format(self._name,
                                                                                                          deletecommands))
                            ...
                            self.logger.log(VERBOSE1,
                                            "Parsing Input {}: Sendcommands: {}. Sendingcommand: {}".format(self._name,
                                                                                                            self._send_commands,
                                                                                                            self._sendingcommand))
                                ...
                                if self._resend_counter >= self._resend_retries:
                                    ...
                                    self.logger.log(VERBOSE1,
                                                    "Parsing Input {}: Resetting Resend Counter because maximum retries exceeded.".format(
                                                        self._name))
                                    if not self._send_commands[
                                               0] in self._query_commands and not self._send_commands == []:
                                        self._sendingcommand = self._send_commands[0]
                                        self.logger.warning(
                                            "Parsing Input {}: Going to reset item {}.".format(self._name,
                                                                                               self._sendingcommand))
                                        self._resetitem('')
                                    ...
                                    if self._send_commands[0] not in self._query_commands and not self._send_commands[
                                                                                                      0] in \
                                                                                                  self._special_commands[
                                                                                                      'Display'][
                                                                                                      'Command']:
                                        # self._keep_commands[time.time()] = self._send_commands[0]
                                        self.logger.debug(
                                            "Parsing Input {}: Removing item from send command, not storing in keep commands.".format(
                                                self._name))
                                    elif self._send_commands[0] in self._query_commands:
                                        self.logger.warning(
                                            "Parsing Input {}: Giving up {}, because no answer received.".format(
                                                self._name, temp_sendcommand))
                                    self._send_commands.pop(0)
                                    self.logger.log(VERBOSE1,
                                                    "Parsing Input {}: Send commands are now: {}".format(self._name,
                                                                                                         self._send_commands))
                                elif depending is True:
                                    self._resend_counter = 0
                                    self.logger.log(VERBOSE1,
                                                    "Parsing Input {}: Resetting Resend Counter because dependency not fulfilled.".format(
                                                        self._name))
                                    try:
                                        if not self._send_commands[
                                                   0] in self._query_commands and not self._send_commands == []:
                                            self._sendingcommand = self._send_commands[0]
                                            self.logger.warning(
                                                "Parsing Input {}: Reset item {} because dependency not fulfilled.".format(
                                                    self._name, self._sendingcommand))
                                            self._resetitem('')
                                        self._sendingcommand = 'gaveup'
                                        if self._send_commands[0] not in self._query_commands and not \
                                                self._send_commands[0] in self._special_commands['Display']['Command'] and not \
                                                self._sendingcommand == 'gaveup':
                                            self._keep_commands[time.time()] = self._send_commands[0]
                                            self.logger.debug(
                                                "Parsing Input {}: Removing item from send command, storing in keep commands: {}.".format(
                                        ...
                                        self._send_commands.pop(0)
                                        self.logger.log(VERBOSE1,
                                                        "Parsing Input {}: Keepcommands: {}. Sendcommands: {}".format(
                                                            self._name, self._keep_commands, self._send_commands))
                                    except Exception as err:
                                        self.logger.log(VERBOSE1,
                                                        "Parsing Input {}: Nothing to reset as send commands is empty: {}. Message: {}".format(
                                                            self._name, self._send_commands, err))
                                if not self._sendingcommand == 'gaveup':
                                    to_send = 'query' if (
                                                self._resend_counter % 2 == 1 and not self._send_commands[0].split(',')[
                                                                                          1] == '') else 'command'
                                    self.logger.debug(
                            ...
                            self.logger.warning(
                                "Parsing Input {}: Problems with checking for expected response. Error: {}".format(
                                    self._name, err))
                    ...
                    if not data == 'ERROR' and data not in self._error_response:
                        ...
                        for dictkey in sorted_response_commands:
                            ...
                            for entry in self._response_commands[comparekey]:
                                ...
                                self.logger.log(VERBOSE2,
                                                "Parsing Input {}: Entry: {}, Valuelength: {}, Expected Type: {}. ".format(
                                ...
                                if index == 0:
                                    ...
                                    if data.startswith(self._special_commands['Display']['Command']) and not \
                                            self._special_commands['Display']['Command'] == '':
                                        receivedvalue = self._convertvalue(
                                    ...
                                    elif data.startswith(tuple(self._special_commands['Nowplaying']['Command'])) and not \
                                        ...
                                        try:
                                            ...
                                            self.logger.debug(
                                                "Parsing Input {}: Problems reading Now Playing info. Error:{}".format(
                                                    self._name, err))
                                    elif data.startswith(tuple(self._special_commands['Speakers']['Command'])) and not \
                                        ...
                                        receivedvalue = self._convertvalue(
                                            data[responseposition:responseposition + valuelength], expectedtype, False,
                                            valuelength, self._special_commands['Speakers']['Command'])
                                        try:
                                        ...
                                        origvalue = value = receivedvalue = data[
                                                                            responseposition:responseposition + valuelength]
                                        self.logger.log(VERBOSE1,
                                    ...
                                    try:
                                        ...
                                        if receivedvalue == '' and expectedtype == 'empty':
                                            ...
                                            sametype = False
                                            self.logger.log(VERBOSE2,
                                                            "Parsing Input {}: Cannot compare {} with {}. Message: {}.".format(
                                                                self._name, receivedvalue, expectedtype, err))
                                    if sametype is False:
                                        ...
                                        deletekeep = []
                                        for e in self._keep_commands:
                                            self.logger.log(VERBOSE1,
                                                            "Parsing Input {}: Testing Keep Command e {} with age of {}s".format(
                                                                self._name, e, int(time.time() - e)))
                                            if data in self._keep_commands.get(e).split(',')[2].split('|'):
                                                self.logger.debug(
                                                    "Parsing Input {}: Removing {} from Keep Commands {} because corresponding value received.".format(
                                                        self._name, e, self._keep_commands))
                                                deletekeep.append(e)
                                            elif time.time() - e >= self._secondstokeep:
                                                self.logger.debug(
                                                    "Parsing Input {}: Removing {} from Keep Commands {} because age is {}s.".format(
                                                        self._name, e, self._keep_commands, int(time.time() - e)))
                                                deletekeep.append(e)
                                        for todelete in deletekeep:
                                            self._keep_commands.pop(todelete)
                                        newvalue = self._translate(origvalue, entry[9], 'parse')
                                        if not newvalue == '' and newvalue is not None:
                                            value = newvalue
                                        ...
                                        try:
                                            ...
                                            commandstarts = []
                                            for e in self._response_commands:
                                                if e.split('?')[0] in testcommand:
                                                    commandstarts.append(e.split('?')[0])
                                    ...
                                    if updated == 1:
                                        self.logger.log(VERBOSE1,
                                                        "Parsing Input {}: Updated all relevant items from {}. step 2".format(
                                                            self._name, item))
                                        break
                                elif dictkey.lower() == 'string':
                                    value = data
                                    if value.startswith(tuple(sorted_response_commands)):
                                        self.logger.log(VERBOSE1,
                                                        "Parsing Input {}: Found string for Item {} with Value {} but ignored because it is a legit response for another command.".format(
                                        ...
                                        self._wait(0.15)
                                        self.logger.debug(
                                            "Parsing Input {}: Updating item {} with value {}".format(self._name, item,
                                                                                                      value))
                                        break
                                if updated == 1:
                                    self.logger.log(VERBOSE1,
                ...
                self.logger.error("Parsing Input {}: Problems parsing input. Error: {} on {}".format(self._name, err,
                                                                                                     sys.exc_info()[
                                                                                                         -1].tb_lineno))
            finally:
                if not self._send_commands:
                    ...
                    for command in self._send_commands:
                        ...
                        if command in self._query_commands:
                            ...
                            self.logger.log(VERBOSE1,
                                            "Parsing Input {}: Adding command {} to position {}.".format(self._name,
                                                                                                         command,
                                                                                                         index))
                            index += 1
                        ...
                        for command in self._send_commands:
                            self.logger.log(VERBOSE1,
                                            "Parsing Input {}: Going to reset in the end because connection is lost: {}.".format(
                                                self._name, command))
                            if command not in self._query_commands and \
                                    command not in self._special_commands['Display']['Command'] and not \
                                    self._sendingcommand == 'gaveup':
    ...
    def _createresponsecommand(self, func_type, commandinfo, reverseinfo, value):
        self.logger.log(VERBOSE1,
                        "Updating Item {}: Create response command with func_type {}, command {}, reverse {}, value {}".format(
                            self._name, func_type, commandinfo, reverseinfo, value))
        try:
        ...
        for count, splitre in enumerate(splitresponse):
            ...
            if valuelength > 0 or 'R' in commandinfo[5]:
                if func_type == 'off':
                    ...
                    if value == 0 and 'bool' in commandinfo[9]:
                    ...
                    elif (isinstance(value, int) and 'int' in commandinfo[9]) or (
                            isinstance(value, float) and 'float' in commandinfo[9]):
                        try:
                        ...
                        self.logger.log(VERBOSE2,
                                        "Updating Item {}: There might be something wrong with replacing the response.".format(
                                            self._name))
                    if not replacedresponse == '':
                        responselist.append('{},{},{}'.format(replacedresponse, commandinfo[9], valuelength))
        ...
        if not replacedreverse == '':
            self.logger.log(VERBOSE2,
                            "Updating Item {}: Replaced response: {}, replaced reverse: {}. Type: {}".format(self._name,
                                                                                                             replacedresponse,
                                                                                                             replacedreverse,
                                                                                                             func_type))
            ...
            self.logger.log(VERBOSE2,
                            "Updating Item {}: Replaced response: {}. Type: {}".format(self._name, replacedresponse,
                                                                                       func_type))
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        if self.alive:
            ...
            if not caller == 'AVDevice' and caller not in self._update_exclude:
                emptycommand = False
                depending = True
                commandinfo = command_re = response = reversevalue = replacedvalue = ''
                value = item()
                try:
                    self.logger.debug("Updating Item {}: {} trying to update {}".format(self._name, caller, item))
                    self.logger.log(VERBOSE1,
                                    "Updating Item {}: Starting to update item {}. Caller: {}, Source: {}. Destination: {}. Reconnectrigger is {}".format(
                    ...
                    if (self.has_iattr(item.conf, 'avdevice') and self.get_iattr_value(item.conf,
                                                                                       'avdevice') == 'reload') or (
                            self.has_iattr(item.conf, 'avdevice_zone0') and self.get_iattr_value(item.conf,
                                                                                                 'avdevice_zone0') == 'reload'):
                        self._initialize()
                        ...
                        try:
                            if self.has_iattr(item.conf, 'avdevice'):
                                ...
                                if command in self._items['zone{}'.format(zone)].keys():
                                    ...
                                    self.logger.debug(
                                        "Updating Item {}: Corresponding Item for command {} in zone {} does not exist. Skipping.".format(
                                ...
                                zone_x = True
                                self.logger.debug(
                                    "Updating Item {}: Command is {}. Zone is {}".format(self._name, command, zone))
                            else:
                                zone_x = False
                        except Exception:
                            zone_x = False
                        ...
                        if letsgo is True:
                            ...
                            try:
                                ...
                                if self._functions['zone{}'.format(zone)][command][2] == '':
                                    ...
                                    if not self._is_connected == []:
                                        self.logger.log(VERBOSE1,
                                                        "Updating Item {}: Function is empty. Command: {} Item: {}".format(
                                    ...
                                    if command == 'statusupdate':
                                        try:
                                            ...
                                            checkvalue = True
                                        self.logger.log(VERBOSE1,
                                                        "Updating Item {}: Statusupdate. Checkvalue: {}. Display Ignore: {}".format(
                                                            self._name, checkvalue,
                                                            self._special_commands['Display']['Ignore']))
                                        if (checkvalue is True or caller == 'Init') and not \
                                                self._special_commands['Display']['Ignore'] >= 5:
                                            if not self._is_connected == []:
                                            ...
                                            if not self._is_connected == []:
                                                self.logger.log(VERBOSE1,
                                                                "Updating Item {}: Updating status. Sendcommands: {}. Reconnecttrigger: {}. Display Ignore: {}".format(
                                                                    self._name, self._send_commands,
                                                                    self._trigger_reconnect,
                                                                    self._special_commands['Display']['Ignore']))
                                        elif checkvalue is False and not self._special_commands['Display'][
                                                                             'Ignore'] >= 5:
                                            depending = self._checkdependency(item, 'globaldepend')
                                            if depending is True or self._is_connected == [] or self._is_connected == [
                                                'Connecting'] and (
                                                    self._depend0_volume0 is True or self._depend0_power0 is True):
                                ...
                                if self._functions['zone{}'.format(zone)][command][5].lower() == 'r':
                                    ...
                                    if commandinfo[2] == '' and commandinfo[3] == '':
                                        ...
                                        elif appendcommand not in self._query_commands and appendcommand not in \
                                                                                               self._special_commands[
                                                                                                   'Display'][
                                                                                                   'Command'] and depending is True:
                            ...
                            if updating is True:
                                ...
                                if command in self._functions['zone{}'.format(zone)]:
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            reverseinfo = ''
                                    replacedresponse, replacedreverse = self._createresponsecommand('in_decrease',
                                                                                                    commandinfo,
                                                                                                    reverseinfo, '')
                                    try:
                                    ...
                                    if appendcommand in self._send_commands:
                                        ...
                                        self._send_commands[self._send_commands.index(
                                            reversecommand)] = self._sendingcommand = appendcommand
                                ...
                                elif command_on in self._functions['zone{}'.format(zone)] and isinstance(value,
                                                                                                         bool) and value == 1:
                                    commandinfo = self._functions['zone{}'.format(zone)][command_on]
                                    reverseinfo = self._functions['zone{}'.format(zone)][command_off]
                                    replacedresponse, replacedreverse = self._createresponsecommand('on', commandinfo,
                                                                                                    reverseinfo, '')
                                    ...
                                    if appendcommand in self._send_commands:
                                        ...
                                        self._keep_commands[time.time()] = appendcommand
                                        self.logger.debug(
                                            "Updating Item {}: Not adding on command {} because dependency is not fullfilled, storing in keep commands: {}.".format(
                                                self._name, appendcommand, self._keep_commands))
                                    else:
                                        self._send_commands.append(appendcommand)
                                ...
                                elif command_off in self._functions['zone{}'.format(zone)] and isinstance(value,
                                                                                                          bool) and value == 0:
                                    commandinfo = self._functions['zone{}'.format(zone)][command_off]
                                    reverseinfo = self._functions['zone{}'.format(zone)][command_on]
                                    replacedresponse, replacedreverse = self._createresponsecommand('off', commandinfo,
                                                                                                    reverseinfo, '')
                                    ...
                                    try:
                                        ...
                                        pass
                                    if newvalue is not None and not newvalue == '':
                                        value = newvalue
                                    try:
                                        value = eval(value.lstrip('0'))
                                    except Exception:
                                    ...
                                    if value == 0 and 'bool' in commandinfo[9]:
                                        ...
                                        try:
                                            value = max(min(value, int(commandinfo[8])), int(commandinfo[7]))
                                            self.logger.debug(
                                                "Updating Item {}: value limited to min {} and max {}.".format(
                                            ...
                                            try:
                                                ...
                                                self.logger.debug(
                                                    "Updating Item {}: Value limited to specific number of digits".format(
                                                        self._name))
                                        if commandinfo[2].count('*') > 1:
                                            anzahl = commandinfo[2].count('*')
                                            self.logger.log(
                                                VERBOSE1,
                                                "Updating Item {}: Value has to be {} digits.".format(self._name,
                                                                                                      anzahl))
                                            value = max(min(value, int(
                                                re.sub('[^0-9]', '', re.sub('\*', '9', commandinfo[2])))), 0)
                                            command_re = re.sub(r'(\*)\1+', '{0:0{1}d}'.format(value, anzahl),
                                                                commandinfo[2])
                                            ...
                                            if command.lower().startswith('speakers'):
                                                ...
                                                self.logger.log(VERBOSE1,
                                                                "Updating Item {}: Speaker {} current value is {}. Item: {} with value {}. Multiply: {}. Value: {}".format(
                                                                    self._name,
                                                                    self._items['zone{}'.format(zone)]['speakers'][
                                                                        'Item'], currentvalue, item, item(), multiply,
                                                                    value))
                                                if not currentvalue == value or multiply == -1:
                                                    value = currentvalue + (value * multiply)
                                                response = self._createresponsecommand('set', commandinfo, '', value)
                                                if value > 0:
                                                    replacedresponse = self._createresponsecommand('power', powerinfo,
                                                                                                   '', True)
                                                    try:
                                                        poweritem = self._items['zone{}'.format(zone)][
                                                            powerinfo[1]].get('Item')
                                                    except Exception:
                                            ...
                                            self.logger.log(VERBOSE1,
                                                            "Updating Item {}: Value has to be 1 digit. Value is {}".format(
                                                                self._name, value))

                                        elif commandinfo[2].count('*') == 0:
                                            self.logger.error(
                                                "Updating Item {}: Set command {} does not have any placeholder *.".format(
                                                    self._name, commandinfo))

                                    elif isinstance(value, str) and 'str' in commandinfo[9]:
                                        setting = True
                                        value = value.upper()
                                        self.logger.debug(
                                            "Updating Item {}: Value has to be string. Value is {}".format(self._name,
                                                                                                           value))
                                        try:
                                ...
                                if not self._send_commands == [] and setting is True:
                                    ...
                                    for sendcommand in self._send_commands:
                                        self.logger.log(VERBOSE1,
                                                        "Updating Item {}: Testing send command: {}".format(self._name,
                                                                                                            sendcommand))
                                        if commandinfo[3] in sendcommand:
                                            ...
                                            if commandinfo[9] in testvalues:
                                                self._sendingcommand = appendcommand
                                                self.logger.log(VERBOSE1,
                                                                "Updating Item {}: Command Set {} ({}) already in Commandlist {}. Value type: {}, expected type: {}. Replaced. Sendingcommand: {}".format(
                                                                    self._name, command, commandinfo[3],
                                                                    self._send_commands, type(value), commandinfo[9],
                                                                    self._sendingcommand))
                                                self._send_commands[
                                                    self._send_commands.index(sendcommand)] = appendcommand
                                                self._resend_counter = 0
                                                appending = False
                                                self.logger.log(VERBOSE1,
                                                ...
                                                self.logger.log(VERBOSE2,
                                                                "Updating Item {}: Command Set {} ({}) already in Commandlist {} but value {} is not same type as {}. Continue...".format(
                                                                    self._name, command, commandinfo[3], type(value),
                                                                    commandinfo[9], self._send_commands))
                    ...
                    try:
                        if not self._is_connected == [] and not self._send_commands == [] and not \
                                self._is_connected == ['Connecting']:
                            self.logger.log(VERBOSE1,
                        ...
                        if self._reset_onerror is True and emptycommand is False and not self._send_commands == [] and not self._sendingcommand == 'done' and self._is_connected == []:
                            ...
                            try:
                                ...
                                self.logger.log(VERBOSE1,
                                                "Updating Item {}: Sending command {} not in Sendcommands {} list, but found in {}".format(
                                                    self._name, self._sendingcommand, self._send_commands, befehle))
                            if self._send_commands[index] not in self._query_commands and not self._send_commands[
                                                                                                  index] in \
                                                                                              self._special_commands[
                                                                                                  'Display']['Command']:
                            ...
                            if resetting == '':
                                self.logger.debug(
                                    "Updating Item {}: Connection error while trying to query info.".format(self._name))
                            else:
                                self.logger.info(
                                    "Updating Item {}: Connection error. Resetting Item {}. Keepcommands: {}. Sendcommands: {} Sendingcommand: {}".format(
                                        self._name, resetting, self._keep_commands, self._send_commands,
                                        self._sendingcommand))
                            self._sendingcommand = 'done'
    ...
    def _displayignore(self, response, receivedvalue, caller):
        ...
        if receivedvalue is None:
            try:
                ...
                if sending in self._query_commands and len(
                        self._send_commands) > 1 and keyfound is not True and displayignore < 5:
                    ...
                    if self._special_commands['Display']['Ignore'] == 0 and \
                            1 not in inputignore and not displaycommand == '':
                ...
                if not (self._ignore_response == responseignore and
                        self._special_commands['Display']['Ignore'] == displayignore and
                        self._special_commands['Input']['Ignore'] == inputignore):
                    self.logger.debug(
            ...
            try:
                if response.startswith(tuple(inputcommands)) and str(
                        receivedvalue) in self._ignoredisplay and '' not in self._ignoredisplay:
                    ...
                    if displaycommand not in self._ignore_response and not displaycommand == '' and '' not in self._ignore_response:
                        self._ignore_response.append(displaycommand)
                    self.logger.debug(
                        "Display Ignore {}: Data {} has value in ignoredisplay {}. Ignorecommands are now: {}. Display Ignore is {}. Input Ignore is {}".format(
                            self._name, response, self._ignoredisplay, self._ignore_response, displayignore,
                            inputignore))
                elif response.startswith(tuple(inputcommands)) and str(
                        receivedvalue) not in self._ignoredisplay and '' not in self._ignoredisplay:
                    for i in range(0, len(inputcommands)):
                        if response.startswith(inputcommands[i]):
                            self._special_commands['Input']['Ignore'][i] = 0
                    self.logger.log(VERBOSE2,
                                    "Display Ignore {}: Data {} with received value {} has NO value in ignoredisplay {}. Ignored responses are now: {}. Display Ignore is {}. Input Ignore is {}".format(
    ...
    def _send(self, command, caller):
        ...
        try:
            if not self._send_commands == []:
                ...
                self.logger.log(VERBOSE1,
                                "Sending {}: Starting to send {} {}. Caller: {}.".format(self._name, command, to_send,
                                                                                         caller))
                try:
                    self._sendingcommand = self._send_commands[0]
                except Exception:
                ...
                if not self._parsinginput:
                    self.logger.log(VERBOSE1,
                                    "Sending {}: Starting Parse Input. Expected response: {}".format(self._name,
                                                                                                     response))
            ...
            if err.errno == 32:
                self.logger.warning(
                    "Sending {}: Problem sending multicommand {}, not connected. Message: {}".format(self._name,
                                                                                                     self._send_commands[
                                                                                                         0], err))
            ...
            try:
                self.logger.warning("Sending {}: Problem sending multicommand {}. Message: {}".format(self._name,
                                                                                                      self._send_commands[
                                                                                                          0], err))
            except Exception:
                self.logger.warning(
                    "Sending {}: Problem sending multicommand {}. Message: {}".format(self._name, self._send_commands,
                                                                                      err))
