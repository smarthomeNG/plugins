...
class Init(object):
                                    ...
                                    for entrylist in self._items[dependzone][dependingfunction]['Master']:
                                        ...
                                        for splitted in splitresponse:
                                            ...
                                            if valuelength > 0 or 'R' in self._functions[dependzone][command][5]:
                                                ...
                                                response_toadd = re.sub('[?]', '*', response_toadd) if splitted.count('?') == 1 \
                                                    and splitted.count('*') == 0 else response_toadd
                                                responselist.append(
                                                    '{},{},{}'.format(response_toadd, valuetype, valuelength))
                                        ...
                                        if not querycommand == '' and self._functions[dependzone][command][4].find('*') >= 0:
                                            ...
                                            try:
                                                if toadd not in dependencies['Slave_query'][dependzone][commandlist]:
                                                    dependencies['Slave_query'][dependzone][commandlist].append(
                                                        toadd)
    ...
    def _dependstage2(self, dependson_list, problems):
        for zone in dependson_list:
            for entry in dependson_list[zone]:
                for count, _entrylist in enumerate(dependson_list[zone][entry]):
    ...
    def _create_querycommands(self):
        ...
        try:
            ...
            self.logger.debug(
                "Initializing {}: Starting to create query commands. Lock is {}. Query Commands: {}, Query Zone: {}".format(
            ...
            for zone in range(0, self._number_of_zones + 1):
                for command in self._functions['zone{}'.format(zone)]:
                    try:
                        ...
                        responsecommand = "|".join(responselist)
                        commandlist = '{},{},{}'.format(querycommand, querycommand, responsecommand)
                        if commandlist not in self._query_zonecommands['zone{}'.format(zone)] \
                                and not responsecommand == '' and not responsecommand == ' ' \
                                and not responsecommand == 'none' and not querycommand == '' \
                                and not self._functions['zone{}'.format(zone)][command][4] in self._ignoreresponse:
                            if not self._functions['zone{}'.format(zone)][command][4] in \
                        ...
                        if commandlist not in self._query_commands \
                                and not responsecommand == '' and not responsecommand == ' ' \
                                and not responsecommand == 'none' \
                                and not querycommand == '' \
                                and not self._functions['zone{}'.format(zone)][command][4] in self._ignoreresponse:
                            if not self._functions['zone{}'.format(zone)][command][4] in \
                                   self._special_commands['Display']['Command']:
    ...
    def _create_responsecommands(self):
        ...
        try:
            ...
            self.logger.debug(
                "Initializing {}: Starting to create response commands. Lock is {}. Response Commands: {}".format(
                    ...
                    if not command == 'init' and not command == 'statusupdate':
                        try:
                            ...
                            for response in response_to_split:
                                ...
                                response = re.sub('\?\{str\}', '?', response) if response.find(
                                    '?{str}') >= 0 else response
                                if ((response.count('?') == 1 and response.count('*') == 0) or
                                        response.count('*') == 1) and \
                                        'str' in self._functions['zone{}'.format(zone)][command][9].split(','):
                                    valuelength = 100
                                    response = re.sub('\*\{str\}', '*', response)
                                    response = re.sub('[?]', '*', response) \
                                        if (response.count('?') == 1 and response.count('*') == 0) else response
    ...
    def _read_parsefile(self, device_function):
        ...
        try:
            self.logger.debug(
                "Initializing {}: Starting to read specialparse file {}. Lock is {}. ".format(
                    self._name, self._model, self._threadlock_standard.locked()))
            filename = '{}/{}.txt'.format(os.path.abspath(os.path.dirname(__file__)), device_function)
    ...
    def _read_commandfile(self):
        ...
        try:
                ...
                for line in commands:
                    ...
                    if comment == 0 and not cond3:
                        ...
                        try:
                            ...
                            itemkeys = []
                    if itemtest in itemkeys and not (device_function == "FUNCTION" or
                                                       device_function == '' or
                                                       device_function == "FUNCTION FUNCTIONTYPE"):
            ...
            if self._threadlock_standard.locked():
                self._threadlock_standard.release()
            self.logger.log(VERBOSE1,
                            "Initializing {}: Finishing reading file. Lock is released. Lock is now {}".format(
