    ...
    def parse_item(self, item):
        ...
        if 'alexa_actions' in item.conf:
            ...
            for action_name in action_names:
                if action_name and self.actions.by_name(action_name) is None:
                    self.logger.error(
                        "Alexa: invalid alexa action '{}' specified in item {}, ignoring item".format(
                            action_name, item.property.path
                        )
                    )
                    return None
        ...
        if name and (not device.name or name_is_explicit):
            ...
            if device.name and device.name != name:
                self.logger.warning(
                    "Alexa: item {} is changing device-name of {} from '{}' to '{}'".format(
                        item.property.path, device_id, device.name, name
                    )
                )
            device.name = name
        ...
        if 'alexa_description' in item.conf:
            ...
            if device.description and device.description != descr:
                self.logger.warning(
                    "Alexa: item {} is changing device-description of {} from '{}' to '{}'".format(
                        item.property.path, device_id, device.description, descr
                    )
                )
        ...
        if 'alexa_color_value_type' in item.conf:
            ...
            self.logger.debug(
                'Alexa4P3: {}-ColorValueType = {}'.format(item.property.path, device.alexa_color_value_type)
            )
        ...
        if 'alexa_range_delta' in item.conf:
            ...
            self.logger.debug(
                'Alexa4P3: {}-Alexa_Range_Delta = {}'.format(item.property.path, device.alexa_range_delta)
            )
        ...
        if 'alexa_color_temp_delta' in item.conf:
            ...
            self.logger.debug(
                'Alexa4P3: {}-Alexa_ColorTemp_Delta = {}'.format(item.property.path, item.alexa_color_temp_delta)
            )
        ...
        if 'alexa_proxy_credentials' in item.conf:
            ...
            self.logger.debug(
                'Alexa4P3: {}-Proxy-Credentials = {}'.format(item.property.path, device.alexa_proxy_credentials)
            )
        ...
        if 'alexa_cam_modifiers' in item.conf:
            ...
            self.logger.debug(
                'Alexa4P3: {}-Camera-Stream-Modifiers = {}'.format(item.property.path, device.alexa_cam_modifiers)
            )
        ...
        if 'alexa_proactivelyReported' in item.conf:
            ...
            self.logger.debug(
                'Alexa4P3: {}-alexa_proactivelyReported = {}'.format(item.property.path, device.proactivelyReported)
            )
        ...
        if action_names:
            ...
            self._proto.addEntry(
                'INFO   ', 'Alexa: {} supports {} as device {}'.format(item.property.path, action_names, device_id)
            )
...
class WebInterface(SmartPluginWebIf):
    ...
    def index(self, reload=None):
        ...
        payload_action = (
            'used Actions Payload V2 = '
            + str(self.plugin.action_count_v2)
            + ' / '
            + '<strong>used Actions Payload V3 = '
            + str(self.plugin.action_count_v3)
            + '</strong>'
        )
