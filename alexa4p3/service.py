    ...
    'actionMappings': [
        ...
        },
        {
            ...
            'directive': {
                ...
                'payload': {'rangeValueDelta': 999, 'rangeValueDeltaDefault': False},
            },
        },
        {
            '@type': 'ActionsToDirective',
...
class AlexaService(object):
    def __init__(
        self, Proto, logger, version, devices, actions, host, port, auth=None, https_certfile=None, https_keyfile=None
    ):
        self.logger = logger
        ...
        if https_certfile:  # https://www.piware.de/2011/01/creating-an-https-server-in-python/
            self.logger.info(
                'Alexa: enabling SSL/TLS support with cert-file {} & key-file {}'.format(https_certfile, https_keyfile)
            )
            # TODO: client-certificates can be handled here as well: https://docs.python.org/2/library/ssl.html
            self.server.socket = ssl.wrap_socket(
                self.server.socket, server_side=True, certfile=https_certfile, keyfile=https_keyfile
            )
...
class AlexaRequestHandler(BaseHTTPRequestHandler):
    ...
    def confirm_health(self, payload):
        ...
        return {
            ...
            'payload': {'description': 'The system is currently healthy', 'isHealthy': True},
        }
    ...
    def p3_discover_appliances(self):
        ...
        for device in self.devices.all():
            ...
            newcapa = {'type': 'AlexaInterface', 'interface': 'Alexa', 'version': '3'}
            ...
            if len(myNameSpace) != 0:
                for NameSpace in myNameSpace:
                    ...
                    newcapa = {
                        ...
                        'version': '3',
                        'properties': {
                            'supported': [{'name': myNameSpace[NameSpace]}],
                    ...
                    if NameSpace == 'Alexa.ThermostatController':
                        ...
                        for mode in myModeList:
                            myModes.append(mode)
                        mysupported = {'supportsScheduling': False, 'supportedModes': myModes}
                        newcapa['properties']['configuration'] = mysupported
                        mysupported = [{'name': 'thermostatMode'}, {'name': 'targetSetpoint'}]
                ...
                appliance = {
                    ...
                    'displayCategories': device.icon,
                    'cookie': {
                        'extraDetail{}'.format(idx + 1): item.property.path
                        for idx, item in enumerate(device.backed_items())
                    },
    ...
    def discover_appliances(self):
        ...
        return {
            ...
            'payload': {'discoveredAppliances': discovered},
        }
    ...
    def p3_ReportState(self, directive):
                ...
                try:
                    ...
                    if myAction.response_type not in str(alreadyReportedControllers):
                        ...
                        if myAction.namespace == 'Alexa.PowerController':
                            msg = "calculating Value for Controller'{}' - Item : {} ".format(
                                Item.property.name, myAction.namespace
                            )
                            myValue = Item()
                            if myValue == 0:
                                myValue = 'OFF'
                            ...
                            myAction.namespace == 'Alexa.ThermostatController'
                            and myAction.response_type == 'targetSetpoint'
                        ):
                            msg = "calculating Value for Controller'{}' - Item : {} ".format(
                                Item.property.name, myAction.namespace
                            )
                            item_now = Item()
                            myValue = {'value': item_now, 'scale': 'CELSIUS'}
                        elif (
                            myAction.namespace == 'Alexa.ThermostatController'
                            and myAction.response_type == 'thermostatMode'
                        ):
                            msg = "calculating Value for Controller'{}' - Item : {} ".format(
                                Item.property.name, myAction.namespace
                            )
        ...
        myResponse = {
            ...
            'event': {'header': myHeader, 'endpoint': {'scope': myScope, 'endpointId': myEndPointID}, 'payload': {}},
        }
        ...
        if header['name'] == 'ReportState':
            ...
            try:
                ...
                self._proto.addEntry(
                    'ERROR  ', "Alexa P3: execution of control-directive '{}' failed: {}".format(directive, e)
                )
                self.respond(
                    {'header': self.header('DriverInternalError', 'Alexa.ConnectedHome.Control'), 'payload': {}}
                )
                return
        self._proto.addEntry(
            'INFO   ',
            "received Directive {} for '{}' Payload : {}".format(directive, device_id, json.dumps(mydirective)),
        )
    ...
    def header(self, name, namespace):
        return {'messageId': uuid.uuid4().hex, 'name': name, 'namespace': namespace, 'payloadVersion': '2'}
