...
def set_target_temp(self, payload):
    ...
    return self.respond(
        {
            'targetTemperature': {'value': new_temp},
            'temperatureMode': {'value': 'AUTO'},
            'previousState': {'targetTemperature': {'value': previous_temp}, 'mode': {'value': 'AUTO'}},
        }
    )
...
def incr_target_temp(self, payload):
    ...
    for item in items:
        ...
        self.logger.info('Alexa: incrementTargetTemperature({}, {:.1f})'.format(item.property.path, item_new))
        item(item_new)
...
def decr_target_temp(self, payload):
    ...
    for item in items:
        ...
        item_now = item()
        item_new = clamp_temp(item_now - delta_temp, item_range)
        self.logger.info('Alexa: decrementTargetTemperature({}, {:.1f})'.format(item.property.path, item_new))
        item(item_new)
