...
class UZSU(SmartPlugin):
    ...
    def run(self):
        ...
        for item in self._items:
            ...
            if cond1 and cond2:
                ...
                self._get_sun4week(item, caller='run')
                self._series_calculate(item, caller='run')
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        self.logger.debug(f"Update Item {item} with value {self._items[item]}")
        ...
        if cond and self._items[item].get('active') is False and not source == 'update_sun':
            ...
            #self._update_item(item, 'UZSU Plugin', 'item_deactivated')
        if cond:
    ...
    def _series_calculate(self, item, caller,source=None):
        ...
        try:
            ...
            #self._update_item(item, 'UZSU Plugin', 'calculate_serie')
        except Exception as e:
    ...
    def _series_get_time(self, mydict,timescan=''):
        ...
        if (timescan == 'next'):
            ...
            #print ('next     : '+ str(mySortedList[myIndex+1]))
        
        else:
            #print ('previous : '+ str(mySortedList[myIndex-1]))
            returnValue = mySortedList[myIndex-1]
