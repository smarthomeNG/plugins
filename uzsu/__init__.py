...
class UZSU(SmartPlugin):
    ...
    def run(self):
        ...
        self.alive = True
        self.scheduler_add(
            'uzsu_sunupdate', self._update_all_suns, value={'caller': 'Scheduler:UZSU'}, cron=self._suncalculation_cron
        )
        ...
        for item in self._items:
            ...
            for i, entry in enumerate(self._items[item].get('list')):
                ...
                if next and next.time() > datetime.now(self._timezone).time() and entry.get('activeToday'):
                    ...
                    update = 'activeToday'
                    self.logger.debug(
                        f"Item '{item}' activeToday of entry {entry} set to False because it is in the future"
                    )
            ...
            if cond1 and cond2:
                ...
                self.logger.info(
                    f'Dry run of scheduler calculation for item {item} to get calculated sunset/rise entries'
                )
    ...
    def _update_sun(self, item, caller=None):
        ...
        try:
            ...
            self.logger.debug(
                f'Updated sun entries for item {item}, triggered by {caller}. '
    ...
    def _update_suncalc(self, item, entry, entryindex, entryvalue):
        ...
        if entry.get('calculated') and entryvalue is None:
            ...
            self.logger.debug(
                f'Updated calculated time for item {item} entry '
            ...
            self.logger.debug(
                f'Sun calculation {entryvalue} entry not updated for item {item} with value {entry["calculated"]}'
            )
    ...
    def _get_type(self, item):
        ...
        try:
            ...
            if _itemtype is None:
                ...
                self.logger.warning(
                    f'Item to be set by uzsu "{_itemforuzsu}" does not exist, type is None. Error: {err}'
                )
            else:
                self.logger.warning(
                    f'Item to be set by uzsu "{_itemforuzsu}" does not have a type attribute. Error: {err}'
                )
        return _itemtype
    ...
    def interpolation(self, intpl_type=None, interval=5, backintime=0, perday=False, item=None):
        ...
        if intpl_type is None:
            ...
            self.logger.warning(
                f'Item {item} interpolation can only be set to none, linear or cubic, not to {intpl_type}.'
            )
            return self._items[item].get('interpolation')
            ...
            self.logger.info(
                f'Item {item} interpolation is set via logic to: '
    ...
    def planned(self, item=None, write=False):
        ...
        if self._planned.get(item) not in [None, {}, 'notinit'] and self._items[item].get('active') is True:
            self.logger.info(
                f"Item '{item}' is going to be set to {self._planned[item]['value']} at {self._planned[item]['next']}"
            )
    ...
    def _remove_dupes(self, item):
        self._items[item]['list'] = [
            i for n, i in enumerate(self._items[item]['list']) if i not in self._items[item]['list'][:n]
        ]
                ...
                for new in newentries:
                    ...
                    if not entry.get('value') == new.get('value') and new.get('active') is True:
                        ...
                        if found is True:
                            ...
                            self.logger.warning(
                                f'Set old entry for item "{item}" at {time} with value {oldvalue} '
    ...
    def update_item(self, item, caller=None, source='', dest=None):
        ...
        self._add_dicts(item)
        if (
            not self._items[item]['interpolation'].get('itemtype')
            or self._items[item]['interpolation']['itemtype'] == 'none'
        ):
            self._items[item]['interpolation']['itemtype'] = itemtype
    ...
    def _update_item(self, item, comment=''):
        ...
        if success is True:
            ...
            self.logger.debug(
                f'Issues with updating seriesCalculated for item {item} comment: {comment}, issue: {success}'
            )
        ...
        if success is True:
            ...
            self.logger.debug(
                f'Issues with updating sunset/rise calculations for item {item} comment: {comment}, issue: {success}'
            )
    ...
    def _schedule(self, item, caller=None):
        ...
        if self._items[item].get('interpolation') is None:
            self.logger.error(
                f'Something is wrong with your UZSU item {item}. You most likely use a wrong smartVISU widget '
            ...
            self.logger.error(
                f'item "{self.get_iattr_value(item.conf, ITEM_TAG[0])}" to be set by uzsu does not exist.'
            )
            ...
            for i, entry in enumerate(self._items[item]['list']):
                ...
                self.logger.debug(
                    f'{item}, i {i} series_finished {series_finished} series_started {series_started} '
                ...
                if next is not None:
                    self.logger.debug(
                        f'uzsu active entry for item {item} with datetime {next}, '
                ...
                if _next is None:
                    ...
                    self.logger.debug(
                        f'uzsu active entry for item {item} using now {next}, value {value} and tzinfo {next.tzinfo}'
                    )
                    _next = next
                    _value = value
                    _entryindex = i
                    ...
                    self.logger.debug(
                        f'uzsu active entry for item {item} keep {_next}, value {_value} and tzinfo {_next.tzinfo}'
                    )
                cond_once = self._items[item]['list'][i].get('once') and series_status == 'finished'
                if self._items[item]['list'][i].get('series') and cond_once:
                    self.logger.debug(
                        f'Deactivating list entry {i} for item {item} because series is finished and set to once'
                    )
        ...
        if (
            ...
            and (cond_activetoday or self._items[item].get('active') is True or _caller == 'dry_run')
        ):
            ...
            if cond1 and not cond2 and cond3 and cond6:
                ...
                self.logger.info(
                    f'Updated item {item} on startup with value {_initvalue} from time {datetime.fromtimestamp(_inittime / 1000.0)}'
                )
            _itemtype = self._items[item]['interpolation'].get('itemtype')
            if cond2 and _interval < 1:
                self.logger.warning(
                    f'Interpolation is set to {_interpolation} but interval is {_interval}. Ignoring interpolation'
                )
            elif cond2 and _itemtype not in ['num']:
                self.logger.warning(
                    f'Interpolation is set to {item} but type of item {_interpolation} is {_itemtype}. Ignoring interpolation and setting UZSU interpolation to none.'
                )
                ...
                try:
                    ...
                    if _caller != 'dry_run' and _interpolated and _value:
                        ...
                        self.logger.info(
                            f'Updated: {item}, {_interpolation.lower()} interpolation value: {_value_now}, '
                    ...
                    self.logger.error(
                        f'Error {_interpolation.lower()} interpolation for item {item} with interpolation list {self._itpl[item]}: {e}'
                    )
            if cond5 and _value < 0:
                self.logger.info(
                    f'value {_value} for item "{item}" is negative. This might be due to not enough values set in the UZSU.'
                )
            ...
            if _caller != 'dry_run':
                self.logger.debug(
                    f'will add scheduler named uzsu_{item.property.path} with datetime {_next} and '
                ...
                self._webdata['items'][item.property.path].update(
                    {'planned': {'value': _value, 'next': _next.strftime('%d.%m.%Y %H:%M:%S')}}
                )
                self._webdata['items'][item.property.path].update(
                    {'seriesrunning': 'True' if self._series[item].get(_entryindex) == 'running' else 'False'}
                )
                ...
                self.scheduler_add(
                    ...
                    value={
                        ...
                        'seriesstatus': self._series[item].get(_entryindex),
                    },
                    next=_next,
                )
    ...
    def _set(self, item=None, value=None, caller=None, entryindex=None, interpolated=False, seriesstatus='-'):
        ...
        update = None
        self.logger.debug(
            f'Setting {item} entryindex: {entryindex}, seriesstatus {seriesstatus}, interpolation {interpolated} caller {caller}'
        )
        self._webdata['items'][item.property.path].update(
            {'depend': {'item': _uzsuitem.property.path, 'value': str(_itemvalue)}}
        )
        if entryindex is not None and not self._items[item]['list'][entryindex].get('activeToday'):
            ...
            update = 'activeToday'
        if (
            entryindex is not None
            and self._items[item]['list'][entryindex].get('once')
        ...
        if entryindex is not None and item:
            ...
            self.logger.debug(
                f'Updated series index {entryindex} for item {item} to {seriesstatus}. Series is now {self._series[item]}'
            )
        ...
        try:
            ...
            if 'rrule' in entry and 'series' not in time:
                ...
                if 'dtstart' in entry:
                    ...
                    try:
                        rrule = rrulestr(
                            entry['rrule'], dtstart=datetime.combine(weekbefore, parser.parse(time.strip()).time())
                        )
                        rstr = str(rrule).replace('\n', ';')
                        self.logger.debug(f"{item}: Created rrule: '{rstr}' for time: '{time}'")
                    except ValueError:
                        self.logger.debug(
                            f"{item}: Could not create rrule from rrule: '{entry['rrule']}' and time: '{time}'"
                        )
                        if 'sun' in time:
                            sun = self._sun(
                                datetime.combine(weekbefore.date(), datetime.min.time()).replace(tzinfo=self._timezone),
                                time,
                                timescan,
                            )
                ...
                while self.alive:
                    ...
                    if 'sun' in time:
                        sleep(0.01)
                        next = self._sun(
                            datetime.combine(dt.date(), datetime.min.time()).replace(tzinfo=self._timezone),
                            time,
                            timescan,
                        )
                        ...
                        next = datetime.combine(dt.date(), parser.parse(time.strip()).time()).replace(
                    ...
                    if next and compare_date == dt.date():
                        ...
                        if next - timedelta(seconds=1) > datetime.now(self._timezone):
                            ...
                            self.logger.debug(
                                f"{item}: Not returning {timescan} rrule {next} because it's in the past."
                            )
            if 'sun' in time and 'series' not in time:
                ...
                if cond_future:
                    ...
                    if caller != 'dry_run' and (not self._items[item]['interpolation'].get('perday') or cond_istoday):
                        self._itpl[item][next.timestamp() * 1000.0] = value
                        self.logger.debug(
                            f'{item}: Include previous today (sun): {next}, value {value} for interpolation.'
                        )
                    if entryindex is not None:
                        self._update_suncalc(item, entry, entryindex, next.strftime('%H:%M'))
                    next = self._sun(
                        datetime.combine(tomorrow, datetime.min.time()).replace(tzinfo=self._timezone), time, timescan
                    )
            ...
            if 'series' in time:
                ...
                self.logger.debug(
                    f'{item}: Looking for {timescan} series-related time. Found rrule: {rstr} '
    ...
    def _series_calculate(self, item, caller=None, source=None):
        ...
        try:
                ...
                try:
                    ...
                    if daycount is not None and daycount != '':
                        if int(daycount) * interval >= 1440:
                            ...
                            self.logger.warning(
                                f'Cut your SerieCount to {daycount} - because interval {interval} '
                    ...
                    if daycount is None:
                        ...
                        if int(daycount) > new_daycount:
                            self.logger.warning(
                                f'Cut your SerieCount to {new_daycount} - because interval {interval} '
                    ...
                    for day in list(rrule):
                        ...
                        try:
                            ...
                            if exceptions == 0:
                                self.logger.info(
                                    f'Item {item}: Between starttime {datetime.strftime(starttime, "%H:%M")} '
                            ...
                            actday = (
                                mydays[day.weekday()] if list(dayrule) is None else mydays[list(dayrule)[0].weekday()]
                            )
                        seriestarttime = None
                            ...
                            if mydays[time.weekday()] != actday:
                                if seriestarttime is not None:
                                    ...
                                    if original_daycount is not None:
                                        mytpl['seriesMax'] = str(
                                            (seriestarttime + timedelta(minutes=interval * count)).time()
                                        )[:5]
                                    else:
                                    ...
                                    self.logger.debug(
                                        f'Mytpl: {mytpl}, count {count}, daycount {daycount}, interval {interval}'
                                    )
                        ...
                        if seriestarttime is not None:
                            mytpl = {'seriesMin': str(seriestarttime.time())[:5]}
                            if original_daycount is not None:
                                mytpl['seriesMax'] = str((seriestarttime + timedelta(minutes=interval * count)).time())[
                                    :5
                                ]
                            else:
                                mytpl['seriesMax'] = f'{endtime.hour:02d}:{endtime.minute:02d}'
                            ...
                            mytpl['seriesDay'] = actday
                            self.logger.debug(
                                f'Mytpl for last time of day: {mytpl}, count {count} daycount '
                    ...
                    if mynewlist:
                        ...
                        self.logger.debug(
                            f'Series for item {item} calculated: {self._items[item]["list"][i]["seriesCalculated"]}'
                        )
                except Exception as e:
                    self.logger.warning(
                        f'Error: {e}. Series entry {mydict} for item {item} could not be calculated. '
            ...
            self.logger.warning(
                f'Series for item {item} could not be calculated for list {self._items[item]["list"]}. Error: {e}'
            )
    ...
    def _series_get_time(self, mydict, timescan=''):
        ...
        if daycount is not None and daycount != '':
            if seriesend is None and (int(daycount) * interval >= 1440):
                ...
                self.logger.warning(
                    f'Cut your SerieCount to {count} - because interval {interval} x SerieCount {org_count} is more than 24h'
                )
            else:
                new_daycount = int((timediff.total_seconds() // 60) // interval + 1)
                if int(daycount) > new_daycount:
    ...
    def _sun(self, dt, tstr, timescan):
        ...
        if len(tabs) == 1:
            ...
            self.logger.error(
                f'Wrong syntax: {tstr} - wrong amount of tabs. Should be [H:M<](sunrise|sunset)[+|-][offset][<H:M]'
            )
            return
        ...
        if cron.startswith('sunrise'):
            ...
            self.logger.error(
                f'Wrong syntax: {tstr} not starting with sunrise/set. Should be [H:M<](sunrise|sunset)[+|-][offset][<H:M]'
            )
            return
        if smin is not None:
            ...
            try:
                ...
                self.logger.error(
                    f'Problems assigning dmin for dt {dt}: {err}. Wrong syntax: {tstr}. Should be [H:M<](sunrise|sunset)[+|-][offset][<H:M]'
                )
                return
        elif smax is None:
            dmin = next_time
        if smax is not None:
            ...
            try:
                ...
                self.logger.error(
                    f'Problems assigning dmax for dt {dt}: {err}. Wrong syntax: {tstr}. Should be [H:M<](sunrise|sunset)[+|-][offset][<H:M]'
                )
                return
        elif smin is None:
            dmax = next_time
        ...
        try:
            ...
            pass
        self.logger.develop(
            f'dt: {dt}, dmax: {dmax}, dmin: {dmin}, tstr: {tstr}, next_time: {next_time} - was: {old_next_time}'
        )
        return next_time
    ...
    def _get_dependant(self, item):
        ...
        try:
            ...
            self.logger.warning(
                f'Item to be queried "{self.get_iattr_value(item.conf, ITEM_TAG[0])}" does not exist. Error: {err}'
            )
            return
        ...
        try:
            ...
            _itemvalue = None
            self.logger.warning(
                f'Item to be queried "{self.get_iattr_value(item.conf, ITEM_TAG[0])}" does not have a type attribute. Error: {err}'
            )
