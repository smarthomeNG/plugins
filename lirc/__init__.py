...
class LIRC(SmartPlugin):
    ...
    def parse_item(self, item):
        if self.has_iattr(item.conf, REMOTE_ATTRS[0]) and self.has_iattr(item.conf, REMOTE_ATTRS[1]):
    ...
    def _send(self, command, item=None, reply=True):
        ...
        while not self._lirc_server_alive:
            self.logger.debug(
                'Waiting to send command {} as connection is not yet established. Count: {}/10'.format(command, i)
            )
            i += 1
            time.sleep(1)
            if i >= 10:
