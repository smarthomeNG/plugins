...
class WithingsHealth(SmartPlugin):
    ...
    def _store_tokens(self, credentials2):
        self.logger.debug(
            'Updating tokens to items: access_token: {} token_expires_in: {} token_expiry: {} token_type: {} refresh_token: {}'.format(
                credentials2.access_token,
                credentials2.expires_in,
                int((self.shtime.utcnow() - datetime.datetime(1970, 1, 1)).total_seconds())
                + int(credentials2.expires_in),
                credentials2.token_type,
                credentials2.refresh_token,
            )
        )
        self.get_item('access_token')(credentials2.access_token)
        self.get_item('token_expiry')(
            int((self.shtime.utcnow() - datetime.datetime(1970, 1, 1)).total_seconds()) + int(credentials2.expires_in)
        )
    ...
    def _update(self):
        ...
        if (
            'access_token' not in self.get_items()
        ...
        if self._client is None:
            ...
            if (
                ...
                and self.get_item('token_expiry')() > 0
                and self.get_item('token_type')()
                and self.get_item('refresh_token')()
            ):
                self.logger.debug(
                    'Token Time: %s, SHNG Time: %s'
                    % (
                        datetime.datetime.fromtimestamp(self.get_item('token_expiry')(), tz=self.shtime.tzinfo()),
                        self.shtime.now(),
                    )
                )
                if self.shtime.now() < datetime.datetime.fromtimestamp(
                    self.get_item('token_expiry')(), tz=self.shtime.tzinfo()
                ):
                    self.logger.debug(
                    ...
                    self.logger.debug(
                        'Initializing NokiaCredentials: access_token - {} token_expiry - {} token_type - {} refresh_token - {} user_id - {} client_id - {} consumer_secret - {}'.format(
                            self.get_item('access_token')(),
                            self.get_item('token_expiry')(),
        ...
        try:
            ...
            self.logger.error(
                'An exception occurred when running measure_get_meas(): {}. Aborting update.'.format(str(e))
            )
            return
        self._client.refresh_token()
        if (
            get_measure_value(measures, with_measure_type=MeasureType.HEART_RATE) is not None
            and 'heart_rate' in self._items
        ):
            self._items['heart_rate'](
                get_measure_value(measures, with_measure_type=MeasureType.HEART_RATE), self.get_shortname()
            )
            self.logger.debug(
        ...
        if get_measure_value(measures, with_measure_type=MeasureType.WEIGHT) is not None and 'weight' in self._items:
            self._items['weight'](
                get_measure_value(measures, with_measure_type=MeasureType.WEIGHT), self.get_shortname()
            )
        ...
        if get_measure_value(measures, with_measure_type=MeasureType.HEIGHT) is not None and 'height' in self._items:
            self._items['height'](
                get_measure_value(measures, with_measure_type=MeasureType.HEIGHT), self.get_shortname()
            )
            self.logger.debug('height - {}'.format(get_measure_value(measures, with_measure_type=MeasureType.HEIGHT)))

        if (
            get_measure_value(measures, with_measure_type=MeasureType.FAT_FREE_MASS) is not None
            and 'fat_free_mass' in self._items
        ):
            self._items['fat_free_mass'](
                get_measure_value(measures, with_measure_type=MeasureType.FAT_FREE_MASS), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.FAT_RATIO) is not None
            and 'fat_ratio' in self._items
        ):
            self._items['fat_ratio'](
                get_measure_value(measures, with_measure_type=MeasureType.FAT_RATIO), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.FAT_MASS_WEIGHT) is not None
            and 'fat_mass_weight' in self._items
        ):
            self._items['fat_mass_weight'](
                get_measure_value(measures, with_measure_type=MeasureType.FAT_MASS_WEIGHT), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.DIASTOLIC_BLOOD_PRESSURE) is not None
            and 'diastolic_blood_pressure' in self._items
        ):
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.SYSTOLIC_BLOOD_PRESSURE) is not None
            and 'systolic_blood_pressure' in self._items
        ):
            self._items['systolic_blood_pressure'](
                get_measure_value(measures, with_measure_type=MeasureType.SYSTOLIC_BLOOD_PRESSURE), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.TEMPERATURE) is not None
            and 'temperature' in self._items
        ):
            self._items['temperature'](
                get_measure_value(measures, with_measure_type=MeasureType.TEMPERATURE), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.BODY_TEMPERATURE) is not None
            and 'body_temperature' in self._items
        ):
            self._items['body_temperature'](
                get_measure_value(measures, with_measure_type=MeasureType.BODY_TEMPERATURE), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.SKIN_TEMPERATURE) is not None
            and 'skin_temperature' in self._items
        ):
            self._items['skin_temperature'](
                get_measure_value(measures, with_measure_type=MeasureType.SKIN_TEMPERATURE), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.MUSCLE_MASS) is not None
            and 'muscle_mass' in self._items
        ):
            self._items['muscle_mass'](
                get_measure_value(measures, with_measure_type=MeasureType.MUSCLE_MASS), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.HYDRATION) is not None
            and 'hydration' in self._items
        ):
            self._items['hydration'](
                get_measure_value(measures, with_measure_type=MeasureType.HYDRATION), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.BONE_MASS) is not None
            and 'bone_mass' in self._items
        ):
            self._items['bone_mass'](
                get_measure_value(measures, with_measure_type=MeasureType.BONE_MASS), self.get_shortname()
            )
            self.logger.debug(
        ...
        if (
            get_measure_value(measures, with_measure_type=MeasureType.PULSE_WAVE_VELOCITY) is not None
            and 'pulse_wave_velocity' in self._items
        ):
        ...
        if (
            'height' in self._items
            and ('bmi' in self._items or 'bmi_text' in self._items)
            and get_measure_value(measures, with_measure_type=MeasureType.WEIGHT) is not None
        ):
            if self._items['height']() > 0:
                bmi = round(
                    get_measure_value(measures, with_measure_type=MeasureType.WEIGHT)
                    / ((self._items['height']()) * (self._items['height']())),
                    2,
                )
    ...
    def get_callback_url(self):
        ...
        for web_if in web_ifs:
            if web_if['Instance'] == self.get_instance_name():
                ...
                self.logger.debug('WebIf found for plugin {}, callback is {}'.format(self.get_fullname(), callback_url))
            return callback_url
