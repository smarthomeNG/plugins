...
class Pluggit(SmartPlugin):
    ...
    _RamIdxBypassActualStateDic = {0: 'Closed', 1: 'In process', 32: 'Closing', 64: 'Opening', 255: 'Opened'}
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, 'pluggit_read'):
            ...
            if pluggit_key in self._modbusRegisterDictionary:
                ...
                self.logger.debug(
                    "Pluggit: Inhalt des dicts _itemReadDictionary nach Zuweisung zu item: '{0}'".format(
                        self._itemReadDictionary
                    )
                )
            else:
        ...
        if self.has_iattr(item.conf, 'pluggit_write'):
            ...
            if pluggit_sendKey in self._modbusRegisterDictionary:
                ...
                self.logger.debug(
                    "Pluggit: Inhalt des dicts _itemWriteDictionary nach Zuweisung zu write item: '{0}'".format(
                        self._itemWriteDictionary
                    )
                )
                ...
                if pluggit_paramList[self.DICT_WRITE_ADDRESS] != -1:
                    ...
                    if self.has_iattr(item.conf, 'pluggit_convert'):
                        ...
                        if convType in pluggit_paramList[self.DICT_ALLOWED_CONV_LIST]:
                            ...
                            self.logger.warning(
                                'Umwandlung von {} zu {} bei Item {} nicht zulässig.'.format(
                                    pluggit_paramList[self.DICT_VALUE_TYPE], convType, item
                                )
                            )
                    if value is not None:
                        if (
                            pluggit_paramList[self.DICT_VALUE_TYPE] == 'uint'
                            or pluggit_paramList[self.DICT_VALUE_TYPE] == 'bool'
                            or pluggit_paramList[self.DICT_VALUE_TYPE] == 'timestamp'
                        ):
                    ...
                    self.logger.warning(
                        'Parameter {} bei Item {} kann nur gelesen werden.'.format(pluggit_sendkey, item)
                    )
            ...
            if modekey in self._RamIdxUnitModeDic:
                ...
                if unitstate != -1:
                    ...
                    registerValue = self._Pluggit.read_holding_registers(
                        pluggit_paramList[self.DICT_READ_ADDRESS], pluggit_paramList[self.DICT_ADDRESS_QUANTITY]
                    )
                    vdecoder = BinaryPayloadDecoder.fromRegisters(
                        registerValue.registers, byteorder=Endian.BIG, wordorder=Endian.LITTLE
                    )
    ...
    def _refresh(self):
        ...
        for item in self._itemReadDictionary:
            ...
            if pluggit_paramList[self.DICT_READ_ADDRESS] != -1:
                ...
                if pluggit_paramList[self.DICT_VALUE_TYPE] == 'uint':
                    if (
                        pluggit_paramList[self.DICT_ADDRESS_QUANTITY] == 1
                        or pluggit_paramList[self.DICT_ADDRESS_QUANTITY] == 2
                    ):
                        ...
                        if readItemValue is not None:
                            ...
                            self.logger.warning(
                                'Unbekannter Wert-Typ: {} bei Item {}.'.format(
                                    pluggit_paramList[self.DICT_VALUE_TYPE], item
                                )
                            )
