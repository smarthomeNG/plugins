...
def main():
    ...
    parser = argparse.ArgumentParser(description='Read or change status of pcomfortcloud Climate devices')

    parser.add_argument('username', help='Username for pcomfortcloud Comfort Cloud')

    parser.add_argument('password', help='Password for pcomfortcloud Comfort Cloud')

    parser.add_argument('-t', '--token', help='File to store token in', default='token.json')

    parser.add_argument('-r', '--raw', help='Raw dump of response', type=str2bool, nargs='?', const=True, default=False)
    ...
    set_parser.add_argument(
        '-p',
        '--power',
        choices=[pcomfortcloud.constants.Power.On.name, pcomfortcloud.constants.Power.Off.name],
        help='Power mode',
    )

    set_parser.add_argument('-t', '--temperature', type=float, help='Temperature')
    ...
    dump_parser = commandparser.add_parser('dump', help='Dump data of a device')

    dump_parser.add_argument(dest='device', type=int, help='Device number 1-x')

    history_parser = commandparser.add_parser('history', help='Dump history of a device')

    history_parser.add_argument(dest='device', type=int, help='Device number 1-x')

    history_parser.add_argument(dest='mode', type=str, help='mode (Day, Week, Month, Year)')

    history_parser.add_argument(dest='date', type=str, help='date of day like 20190807')
    ...
    auth = pcomfortcloud.Authentication(args.username, args.password, json_token, args.raw)
    auth.login()

    json_token = auth.get_token()

    session = pcomfortcloud.ApiClient(auth, args.raw)
    ...
    try:
        ...
        if args.command == 'get':
            if int(args.device) <= 0 or int(args.device) > len(session.get_devices()):
                raise Exception(
                    'device not found, acceptable device id is from {} to {}'.format(1, len(session.get_devices()))
                )
