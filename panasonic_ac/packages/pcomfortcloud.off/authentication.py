...
class Authentication:
    ...
    def _get_new_token(self):
        ...
        if not location.startswith(constants.REDIRECT_URI):
            response = requests_session.get(f'{constants.BASE_PATH_AUTH}/{location}', allow_redirects=False)
            ...
            response = requests_session.post(
                ...
                headers={'Auth0-Client': constants.AUTH_0_CLIENT, 'user-agent': 'okhttp/4.10.0'},
                json={
                    'client_id': constants.APP_CLIENT_ID,
                    'redirect_uri': constants.REDIRECT_URI,
    ...
    def logout(self):
        response = requests.post(f'{constants.BASE_PATH_ACC}/auth/v2/logout', headers=self._get_header_for_api_calls())
    ...
    def _get_user_info(self):
        response = requests.get(
            ...
            headers={'Auth0-Client': self.AUTH_0_CLIENT, 'Authorization': 'Bearer ' + self._token['access_token']},
        )
    ...
    def execute_post(self, url, json_data, function_description, expected_status_code):
        ...
        try:
            response = requests.post(url, json=json_data, headers=self._get_header_for_api_calls())
    ...
    def execute_get(self, url, function_description, expected_status_code):
        ...
        try:
            response = requests.get(url, headers=self._get_header_for_api_calls())
        except requests.exceptions.RequestException as ex:
            raise exceptions.RequestError(ex)
