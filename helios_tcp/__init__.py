...
class HeliosTCP(SmartPlugin):
    ...
    def _read_value(self, item):
        ...
        try:
            ...
            self.logger.warning(
                'Helios TCP: Could not assign {0} to item {1}'.format(varprop['type'](result), item.property.path)
            )
            return
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        if self.alive and caller != self.get_shortname():
            ...
            if newval < varprop['min'] or newval > varprop['max']:
                self.logger.error(
                    "Helios TCP: Variable '{0}' out of bounds. The allowed range is [{1}, {2}]".format(
                        var, varprop['min'], varprop['max']
                    )
                )
                return
