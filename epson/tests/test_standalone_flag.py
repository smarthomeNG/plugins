...
"""
Tests that plugins.epson's module-level bootstrap does not clobber an
...
locate('plugins.epson.commands'), which needs the plugins.epson package
importable under its real dotted name - re-running this module's top-level
code with __name__ == 'plugins.epson', hitting the `else` branch. Simulated
...
class TestEpsonStandaloneFlagNotClobbered(unittest.TestCase):
