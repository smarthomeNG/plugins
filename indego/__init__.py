...
class Indego(SmartPlugin):
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, 'indego_smart'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_smart', self.get_iattr_value(item.conf, 'indego_smart')
                )
            )
        ...
        if self.has_iattr(item.conf, 'indego_frequency'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_frequency', self.get_iattr_value(item.conf, 'indego_frequency')
                )
            )
        ...
        if self.has_iattr(item.conf, 'indego_add_key'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_add_key', self.get_iattr_value(item.conf, 'indego_add_key')
                )
            )
    ...
    def set_smart_frequency(self, item, frequency=0, caller=None, source=None, dest=None):
        ...
        message = self.put_url(
            self.indego_url + 'alms/' + self.alm_sn + '/predictive/useradjustment', self.context_id, command, 10
        )
        self.logger.debug('Command ' + command + ' gesendet! ' + str(message))

    def auth(self):
        auth_response = self.fetch_url(
            self.indego_url + 'authenticate',
            self.user,
            self.password,
            25,
            ...
            for x in i:
                ...
                if wertpunkt == 'wwsymbol_mg2008':
                    ...
                    if wert == '110000' or wert == '111000' or wert == '211000' or wert == '210000':
                        ...
                        wert == '220000' or wert == '121000' or wert == '120000' or wert == '330000' or wert == '320000'
                    ):
    ...
    def alert(self):
        ...
        if not alert_response:
            ...
            if len(alert_response) == 0:
                ...
                alert_date = (
                    datetime.datetime.strptime(alert_latest['date'], '%Y-%m-%dT%H:%M:%S.%fZ')
                    .replace(tzinfo=tz.gettz('UTC'))
                    .astimezone(self.get_sh().tzinfo())
                )  # alert_date = datetime.datetime.strptime(alert_latest['date'],'%Y-%m-%dT%H:%M:%S.%fZ')
    ...
    def get_smart_frequency(self):
        ...
        smart_frequency_response = self.get_url(
            self.indego_url + 'alms/' + self.alm_sn + '/predictive/useradjustment', self.context_id
        )
        ...
        if not device_data_response:
            ...
            if 'alm_firmware_version' in device_data_response:
                ...
                if alm_firmware_version != self.get_sh().indego.alm_firmware_version():
                    ...
                    self.logger.info(
                        'indego updated firmware from '
                        + self.get_sh().indego.alm_firmware_version()
                        + ' to '
                        + str(alm_firmware_version)
                    )
