...
class Indego4shNG(SmartPlugin):
    ...
    def run(self):
        ...
        if self.user != '' and self.password != '':
            ...
            self.logged_in, self._bearer, self._refresh_token, self.token_expires, self.alm_sn = (
                self._login_single_key_id(self.user, self.password)
            )
            self.login_pending = False
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, 'indego_command'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_command', self.get_iattr_value(item.conf, 'indego_command')
                )
            )
        ...
        if self.has_iattr(item.conf, 'indego_config'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_plugin_handled', self.get_iattr_value(item.conf, 'indego_config')
                )
            )
            return self.update_item
        ...
        if self.has_iattr(item.conf, 'indego_plugin_handled'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_plugin_handled', self.get_iattr_value(item.conf, 'indego_plugin_handled')
                )
            )
            return self.update_item
        ...
        if self.has_iattr(item.conf, 'indego_function_4_all'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_function_4_all', self.get_iattr_value(item.conf, 'indego_function_4_all')
                )
            )
            return self.update_item
        ...
        if self.has_iattr(item.conf, 'indego_function_4_visu'):
            self.logger.debug(
                "Item '{}' has attribute '{}' found with {}".format(
                    item, 'indego_function_4_visu', self.get_iattr_value(item.conf, 'indego_function_4_visu')
                )
            )
            return self.update_item
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        if caller != self.get_shortname() and caller != 'Autotimer' and caller != 'Logic':
            ...
            if self.has_iattr(item.conf, 'indego_config'):
                ...
                try:
                    ...
                    self.logger.warning(
                        "Error building config for item '{}' from caller '{}', source '{}' and dest '{}'".format(
                            item, caller, source, dest
                        )
                    )
            ...
            if 'show_uzsu_popup' in item.property.name and item():
        ...
        if self.has_iattr(item.conf, 'indego_command'):
            self.logger.debug(
                "Item '{}' has attribute '{}' triggered {}".format(
                    item, 'indego_command', self.get_iattr_value(item.conf, 'indego_command')
                )
            )
            try:
                ...
                self.logger.warning(
                    "Error sending command for item '{}' from caller '{}', source '{}' and dest '{}'".format(
                        item, caller, source, dest
                    )
                )
        ...
        if self.has_iattr(item.conf, 'indego_function_4_all'):
            ...
            try:
                self.logger.debug(
                    "Item '{}' has attribute '{}' found with {}".format(
                        item, 'indego_plugin_function', self.get_iattr_value(item.conf, 'indego_function_4_all')
                    )
                )
                ...
                self.logger.warning(
                    "failed : Item '{}' has attribute '{}' found with {}".format(
                        item, 'indego_function_4_all', self.get_iattr_value(item.conf, 'indego_function_4_all')
                    )
                )
    ...
    def send_command(self, command, item_name):
        ...
        message, response = self._put_url(
            self.indego_url + 'alms/' + self.alm_sn + '/state', self.context_id, command, 10
        )
    ...
    def _handle_mow_track(self, item):
        if ('visu.mow_track' in item.property.name and self._get_childitem('visu.show_mow_track')) or (
            'visu.show_mow_track' in item.property.name and item()
        ):
            self._create_mow_track()
        elif 'visu.show_mow_track' in item.property.name and not item():
    ...
    def _handle_store_cals(self, item):
        if item.property.name == self.parent_item + '.visu.store_sms_profile' and item():
            self._smart_mow_settings('write')
            self._set_childitem('visu.store_sms_profile', False)

        if item.property.name == self.parent_item + '.calendar_save' and item():
            self._set_childitem('calendar_result', 'speichern gestartet')
            # Now Save the Calendar on Bosch-API
            self.cal_update_count = 0
            self._auto_mow_cal_update()

        if item.property.name == self.parent_item + '.calendar_predictive_save' and item():
    ...
    def _handle_active_mode(self, item):
        if item.property.name == self.parent_item + '.active_mode.kalender' and item():
            self._set_childitem('update_active_mode', True)
            self._set_childitem('active_mode', 1)
            self.items.return_item('indego.active_mode.uzsu.schaltuhr').activate(False)
            self._set_childitem('active_mode.smart', False)
        ...
        if item.property.name == self.parent_item + '.active_mode.aus' and item():
            self._set_childitem('update_active_mode', True)
            self._set_childitem('active_mode', 3)
            self._set_childitem('active_mode.kalender', False)
        ...
        if item.property.name == self.parent_item + '.active_mode.smart' and item():
            self._set_childitem('update_active_mode', True)
            self._set_childitem('active_mode', 2)
            self.items.return_item('indego.active_mode.uzsu.schaltuhr').activate(False)
    ...
    def _create_mow_track(self):
        if self._get_childitem('visu.model_type') == 2:
            ...
            mystrokewidth = '5'
        myMowTrack = {
            'Points': self._get_childitem('visu.mow_track'),
    ...
    def _set_clear_message(self):
        ...
        for message in msg2clear:
            self._delete_url(self.indego_url + 'alerts/{}'.format(message), self.context_id, 10, None)
    ...
    def _get_all_calendars(self):
        if self._get_childitem('wartung.wintermodus') or not self.logged_in:
            return
        if (self._get_childitem('alm_mode') == 'smart') and (
            (self._get_childitem('stateCode') == 513) or (self._get_childitem('stateCode') == 518)
        ):
            return
        ...
        if self._get_childitem('alm_mode') == 'smart':
            try:
                schedule = self._get_url(
                    self.indego_url + 'alms/' + self.alm_sn + '/predictive/schedule', self.context_id
                )
                ...
                self.logger.warning('Problem fetching Calendars: {0}'.format(e))
            my_pred_cal = {'cals': [{'cal': 9, 'days': schedule['exclusion_days']}]}
            my_smMow_cal = {'cals': [{'cal': 9, 'days': schedule['schedule_days']}]}
            # pydevd.settrace("192.168.178.37", port=5678)
    ...
    def _fetch_url(self, url, username=None, password=None, timeout=10, body=None):
        # pydevd.settrace("192.168.178.37", port=5678)
        try:
            myResult, response = self._post_url(
                url, self.context_id, body, timeout, auth=(username, password), nowait=True
            )
        except Exception as e:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            try:
                expiration_timestamp = int(
                    str(response.cookies._cookies['api.indego.iot.bosch-si.com']['/']).split(',')[11].split('=')[1]
                )
            except Exception:
                pass
        else:
    ...
    def _post_url(self, url, contextid=None, body=None, timeout=2, auth='', nowait=True):
        ...
        try:
            ...
            return False
        self.logger.debug(
            'post gesendet an URL: {} context-ID: {} json : {}'.format(url, self.context_id, json.dumps(body))
        )
    ...
    def _put_url(self, url, contextid=None, body=None, timeout=2):
        ...
        try:
            ...
            return False, response
        self.logger.debug(
            'put gesendet an URL: {} context-ID: {} json : {}'.format(url, self.context_id, json.dumps(body))
        )
    ...
    def _set_smart(self, enable=None):
        ...
        result, response = self._put_url(
            self.indego_url + 'alms/' + self.alm_sn + '/predictive', self.context_id, command, 10
        )
    ...
    def _check_state_4_protocoll(self):
        ...
        if (
            ...
            or myActState == 262
            or myActState == 263
        ):  # 769 = fährt zur Station / 772 = Mähzeit beendet / 775 = fertig gemäht
    ...
    def _store_calendar(self, myCal=None, myName=''):
        ...
        if response.status_code == 200:
            ...
            self.logger.info(
                'Error during saving Calendar settings for {} HTTP-Status :{}'.format(myName, response.status_code)
            )
    ...
    def _getrefreshToken(self):
        ...
        mySession.headers['user-agent'] = (
            'Dalvik/2.1.0 (Linux; U; Android 11; sdk_gphone_x86_arm Build/RSR1.201013.001)'
        )
    ...
    def _login_single_key_id(self, user, pwd):
        try:
            ...
            myReqPayload = {
                ...
                'trace': [
                    {'ac': 'T005', 'acST': 1678187316, 'acD': 7},
                    {
                        'ac': 'T021 - URL:https://swsasharedprodb2c.blob.core.windows.net/b2c-templates/bosch/unified.html',
                        'acST': 1678187316,
                        'acD': 119,
                    },
                    {'ac': 'T019', 'acST': 1678187317, 'acD': 44},
                    {'ac': 'T004', 'acST': 1678187317, 'acD': 19},
                    {'ac': 'T003', 'acST': 1678187317, 'acD': 5},
                    {'ac': 'T035', 'acST': 1678187317, 'acD': 0},
                    {'ac': 'T030Online', 'acST': 1678187317, 'acD': 0},
                    {'ac': 'T002', 'acST': 1678187328, 'acD': 0},
                ],
            }
            # Create a session
            mySession = requests.session()
            ...
            url = 'https://prodindego.b2clogin.com/prodindego.onmicrosoft.com/b2c_1a_signup_signin/oauth2/v2.0/authorize?redirect_uri=com.bosch.indegoconnect%3A%2F%2Flogin&client_id={}&response_type=code&state=j1A8L2zQMbolEja6yqbj4w&nonce={}&scope=openid%20profile%20email%20offline_access%20https%3A%2F%2Fprodindego.onmicrosoft.com%2Findego-mobile-api%2FIndego.Mower.User&code_challenge={}&code_challenge_method=S256'.format(
                myClientID, nonce, code_challenge
            )
            ...
            myUrl = 'https://prodindego.b2clogin.com/prodindego.onmicrosoft.com/B2C_1A_signup_signin/client/perftrace?tx=StateProperties={}&p=B2C_1A_signup_signin'.format(
                myNewState
            )
            CollectingCookie = {}
            ...
            postConfirmUrl = myNewUrl[
                myNewUrl.find('postConfirmReturnUrl') : myNewUrl.find('postConfirmReturnUrl') + 300
            ].split('"')
            ...
            RequestVerificationToken = myText[
                myText.find('__RequestVerificationToken') : myText.find('__RequestVerificationToken') + 300
            ].split('"')[4]
            myText[myText.find('ReturnUrl') : myText.find('ReturnUrl') + 1600].split('"')[4]
            myText[myText.find('ReturnUrl') : myText.find('ReturnUrl') + 300].split('"')[4]

            postConfirmUrl = myText[
                myText.find('postConfirmReturnUrl') : myText.find('postConfirmReturnUrl') + 700
            ].split('"')

            myNewUrl = (
                'https://identity-myprofile.bosch.com/ids/api/v1/clients/'
                + myText[myText.find('ciamids_') : myText.find('ciamids_') + 300].split('%2')[0]
            )
            response = mySession.get(myNewUrl, allow_redirects=True)
            self._log_communication('GET   ', myNewUrl, response.status_code)
            ...
            myJson = {'username': user, 'password': pwd, 'keepMeSignedIn': False, 'returnUrl': postConfirmUrl}
            ...
            mySession.cookies.set(
                '.AspNetCore.Antiforgery.085ONM3l57w', mySession.cookies.get('.AspNetCore.Antiforgery.085ONM3l57w')
            )
            ...
            mySession.cookies.set(
                '.AspNetCore.Identity.Application', mySession.cookies.get('.AspNetCore.Identity.Application')
            )
            mySession.headers['accept'] = (
                'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9'
            )
            ...
            step += 1
            CollectingCookie = {}
            CollectingCookie['idsrv.session'] = mySession.cookies.get_dict('identity-myprofile.bosch.com', '/ids')[
                'idsrv.session'
            ]
            CollectingCookie['.AspNetCore.Identity.Application'] = mySession.cookies.get_dict(
                'identity-myprofile.bosch.com', '/ids'
            ...
            mySessionState = myText[myText.find('"session_state"') + 23 : myText.find('"session_state"') + 300].split(
                '"'
            )[0]
            ...
            self._log_communication(
                'LOGIN ',
                'something went wrong during getting Sinlge-Key-ID Login on Step : {} - {}'.format(step, err),
                999,
            )
            self.logger.warning(
                'something went wrong during getting Sinlge-Key-ID Login on Step : {} - {}'.format(step, err)
    ...
    def _build_new_calendar(self, myList=None, type=None):
        ...
        newCal = {}
        emptySlot = {'StHr': '00', 'StMin': '00', 'EnHr': '00', 'EnMin': '00', 'En': False}
        ...
        for myKey in myList:
            ...
            for day in Days.split((',')):
                newSlot = {'StHr': Start[0:2], 'StMin': Start[3:5], 'EnHr': End[0:2], 'EnMin': End[3:5], 'En': True}
                newDay = {'slots': [newSlot], 'day': int(day)}
                dayFound = False
    ...
    def _parse_list_2_cal(self, myList=None, myCal=None, type=None):
        ...
        if type == 'MOW' and len(self.calendar_count_mow) < 5:
            ...
            for myKey in myList:
                ...
                for day in Days.split((',')):
                    if myCal['cals'][myCalNo]['days'][int(day)]['slots'][0]['En']:
                        actSlot = 1
                    else:
                        actSlot = 0
    ...
    def _parse_uzsu_2_list(self, uzsu_dict=None):
        ...
        for myItem in uzsu_dict['list']:
            ...
            if myItem['value'] == '10' and myItem['active']:
                if 'sun' in myItem['time']:
                    if 'calculated' not in myItem:
                        continue
                    else:
                        myItem['time'] = myItem['calculated']
                myKey = '8-' + myItem['time']
                if myKey not in myCal:
        ...
        for myItem in uzsu_dict['list']:
            myDays = myItem['rrule'].split(';')[1].split('=')[1].split(',')
            if myItem['value'] == '20' and myItem['active']:
                if 'sun' in myItem['time']:
                    if 'calculated' not in myItem:
                        continue
                    else:
    ...
    def _parse_cal_2_list(self, myCal=None, type=None):
        ...
        for cal_item in myCal['cals']:
            ...
            if myCalendarNo not in myCalList:
            ...
            for days in cal_item['days']:
                ...
                for slots in days['slots']:
                    ...
                    if myEnabled:
                        ...
                        myDict = {'Key': myKey, 'Start': myStartTime1, 'End': myEndTime1, 'Days': str(myDay)}
                        # pydevd.settrace("192.168.178.37", port=5678)
                        if 'Attr' in slots:
    ...
    def _smart_mow_settings(self, mode=''):
        ...
        if mode == 'write':
            ...
            if self._get_childitem('visu.use_exclude_time_4_sms'):
    ...
    def _check_update(self):
        ...
        if available_updates:
            if available_updates['available']:
    ...
    def _get_weather(self):
        if not self.logged_in:
            return
        try:
            weather = self._get_url(
                self.indego_url + 'alms/' + self.alm_sn + '/predictive/weather', self.context_id, 10
            )
        except Exception:
            return
        if not weather:
        ...
        for i in weather['LocationWeather']['forecast']['intervals']:
            ...
            for x in i:
                ...
                if wertpunkt == 'wwsymbol_mg2008':
                    try:
                        self._set_childitem(
                            'weather.int_' + position + '.' + 'picture', self.path_2_weather_pics + myPictures[wert]
                        )
                    except Exception:
                        # got known Weather-Symbol
                        self.logger.warning('Got unknown Value for Weather-Pic, Value: {}'.format(str(wert)))
                        self._set_childitem(
                            'weather.int_' + position + '.' + 'picture', self.path_2_weather_pics + 'na.png'
                        )
        ...
        if not alert_response:
            ...
            if len(alert_response) == 0:
                ...
                for myAlert in alert_response:
                    if myAlert['alert_id'] not in actAlerts:
    ...
    def _device_data(self):
        ...
        if not device_data_response:
            ...
            self.logger.debug('service_counter :')
            ...
            if alm_mode == 'smart':
                ...
                self._set_childitem('alm_mode.str', 'Übersicht Kalender mähen:')
            elif alm_mode == 'manual' and not self._get_childitem('active_mode.uzsu.schaltuhr.active'):
                self._set_childitem('alm_mode.str', '')
            elif alm_mode == 'manual' and self._get_childitem('active_mode.uzsu.schaltuhr.active'):
                self._set_childitem('alm_mode.str', 'Übersicht mähen nach UZSU:')
            else:
            ...
            if 'alm_firmware_version' in device_data_response:
                ...
                if alm_firmware_version != self.get_sh().indego.alm_firmware_version():
                    ...
                    self.logger.info(
                        'indego updated firmware from {0} to {1}'.format(
                            self.get_sh().indego.alm_firmware_version(), str(alm_firmware_version)
                        )
                    )
    ...
    def _get_state(self):
        ...
        if state_response:
            ...
            try:
                if str(state_code) not in str(self.states) and len(self.states) > 0:
            ...
            if mowed == 0.0 and myLast_percent_mowed > 0.0:
                ...
                #################################
            if (
                state_code == 518 or state_code == 513 or state_code == 515 or state_code == 514
            ):  # 518 = mähe / 513 = schneide Rand / 515 = lade Karte / 514 = mähen, bestimme Ort
                # First run of position detection
                if not self.position_detection:
                    # Now set Position-Detection ON
                    myResult = self._post_url(
                        self.indego_url + 'alms/' + self.alm_sn + '/requestPosition?count=100&interval=7',
                        self.context_id,
                        None,
                        10,
                    )
    ...
    def _load_map(self):
        ...
        garden = self._get_url(
            self.indego_url + 'alms/' + self.alm_sn + '/map?cached=0&showMower=1', self.context_id, 120
        )
        if not garden:
    ...
    def _parse_map(self):
        ...
        if myCustomDrawing is not None and myCustomDrawing != '':
    ...
    def store_credentials_html(self, encoded='', pwd='', user='', store_2_config=None):
        ...
        if store_2_config == 'true':
            ...
            (
                ...
                self.plugin._bearer,
                self.plugin._refresh_token,
                self.plugin.token_expires,
                self.plugin.alm_sn,
            ) = self.plugin._login_single_key_id(self.plugin.user, self.plugin.password)
