...
class AlexaRc4shNG(SmartPlugin):
    ...
    def __init__(self, sh, *args, **kwargs):
        ...
        if ImportPyOTPError:
            self.logger.warning(
                "Plugin '{}': problem during import of pyotp, you will not be able to use MFA-Authentication".format(
                    self.get_fullname()
                )
            )
    ...
    def check_refresh_login(self):
        ...
        if last_update_time + self.LoginUpdateCycle < mytime:
            ...
            self.next_update_time = datetime.fromtimestamp(last_update_time + self.LoginUpdateCycle).strftime(
                '%Y-%m-%d %H:%M:%S'
            )
    ...
    def check_login_state(self):
        try:
            ...
            response = mySession.get(
                'https://' + self.host + '/api/bootstrap?version=0', headers=myHeader, allow_redirects=True
            )
    ...
    def get_list(self, type=''):
        ...
        for mylistItem in myLists['lists']:
            ...
            myListUrl = (
                'https://'
                + self.host
                + '/api/namedLists/{0}/items?startTime=&endTime=&completed=&{1}&_=2'.format(actList, encoded_args)
            )
            ...
            for ListEntry in myListResponse['list']:
                if mylistItem['type'] == type:
                    myReturnList.append(
                        {
                            'value': ListEntry['value'].capitalize(),
                            'completed': ListEntry['completed'],
                            'version': ListEntry['version'],
                            'createdDateTime': ListEntry['createdDateTime'],
                            'updatedDateTime': ListEntry['updatedDateTime'],
                        }
                    )
    ...
    def receive_info_by_request(self, dvName, cmdName, mValue):
        ...
        myUrl = self.parse_url(
            myUrl, mValue, actEcho.serialNumber, actEcho.family, actEcho.deviceType, actEcho.deviceOwnerCustomerId
        )
    ...
    def get_last_alexa(self):
        ...
        mySession.cookies.update(self.cookie)
        response = mySession.get(
            'https://' + self.host + '/api/activities?startTime=&size=10&offset=0',
            headers=myHeader,
    ...
    def send_cmd(self, dvName, cmdName, mValue, path=None):
        ...
        postfields = self.parse_json(
            myDict, mValue, actEcho.serialNumber, actEcho.family, actEcho.deviceType, actEcho.deviceOwnerCustomerId
        )

        try:
            logline = str(self.shtime.now())[0:19] + ' sending command to "{}" payload {}'.format(
                dvName, json.dumps(postfields)
            )
            self._insert_protocoll_entry(logline)
        except Exception:
            pass

        myStatus, myRespHeader, myRespCookie, myContent = self.send_post_request(
            myUrl, myHeaders, self.cookie, postfields
        )
    ...
    def get_devices_by_request(self):
        try:
            ...
            mySession.cookies.update(self.cookie)
            response = mySession.get(
                'https://alexa.amazon.de/api/devices-v2/device?cached=false', headers=myHeader, allow_redirects=True
            )
    ...
    def send_get_request(self, url='', myHeader='', Cookie=''):
        ...
        response = mySession.get(url, headers=myHeader, allow_redirects=True)
    ...
    def send_post_request(self, url='', myHeader='', Cookie='', postdata=''):
        mySession = requests.Session()
        mySession.cookies.update(Cookie)
        response = mySession.post(url, headers=myHeader, data=postdata, allow_redirects=True)
        mySession.close()
    ...
    def auto_login_by_request(self):
        ...
        myStatus, myRespHeader, myRespCookie, myContent, myLocation = self.send_get_request(
            'https://' + self.host + '/spa/index.html', myHeaders
        )
        ...
        myStatus, myRespHeader, myRespCookie, myContent = self.send_post_request(
            newUrl, myHeaders, myCollectionCookie, PostData
        )
        myCollectionTxtCookie = self.parse_response_cookie_2_txt(myRespCookie, myCollectionTxtCookie)
        myCollectionCookie = self.parse_response_cookie(myRespCookie, myCollectionCookie)
        PostData = self.collect_postdata(myContent)
        ...
        myStatus, myRespHeader, myRespCookie, myContent, myLocation = self.send_get_request(
            Url, myHeaders, myCollectionCookie
        )
        myCollectionTxtCookie = self.parse_response_cookie_2_txt(myRespCookie, myCollectionTxtCookie)
        myCollectionCookie = self.parse_response_cookie(myRespCookie, myCollectionCookie)
...
class WebInterface(SmartPluginWebIf):
    ...
    def index(self, reload=None):
        ...
        login_info = (
            self.plugin.last_update_time
            + '<font color="green"><strong> ('
            + self.plugin.next_update_time
            + ')</strong>'
        )
        ...
        if myOrder == 'Step1':
            ...
            if len(myMFA) != 52:
                ...
                try:
                    ...
                    txt_Result['Step'] = myOrder
                    txt_Result['data'] = {
                        'Message': 'OTP could not calculated something seems to be wrong with the MFA<br>Try again'
                    }
