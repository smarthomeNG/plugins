...
class snap7_logo(SmartPlugin):
    ...
    def _read_cycle(self):
        ...
        try:
            # lesen der VM
            self.logger.debug(
                'read_cycle() connected:{0} read addr:{1} len:{2}'.format(
                    self.plc.get_connected(), self._vm, self._vm_len
                )
            )
            ...
            for k, v in self.reads.items():
                ...
                if 'old' in v:  # Variable wurde schon einmal gelesen
                    if v['old'] != new_value:  # Variable hat sich geändert
                        self.logger.debug(
                            'read_cycle():{0} newV:{1} oldV:{2} item:{3} '.format(k, new_value, v['old'], v['item'])
                        )
